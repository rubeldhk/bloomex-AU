...
if (!empty($rows)) {
    ...
    if ($rs->return->error) {
        ...
        $db->insert_sql($q);
        $q = "UPDATE jos_vm_orders SET order_status='K', WHERE order_ID = $order_ID LIMIT 1 ";
        $r = $db->update_sql($q);
    ...
    var_dump($rows);
    echo "</pre>";
    echo "Shipment Void Fail. No fastway ID found in history for order #" . filter_($_REQUEST['order_id']);
}
?>
