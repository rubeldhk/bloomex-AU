function myInterval() {
    jQuery('.pause').css('visibility', 'visible');
    jQuery('div.order > table > tbody').html('').closest('.info').hide();
    jQuery('#company')[0].reset();

    window.callAttemptInterval = setInterval(callAttempt, 5000);
}

function openModal(type, text) {

    var modal_id = 'resultModalNO';

    if (type == 1) {
        modal_id = 'resultModal';
    }

    jQuery('#' + modal_id + ' .modal-body').html(text);
    jQuery('#' + modal_id + '').modal('show');

    setCount();
}



function callAttempt() {
    clearInterval(window.callAttemptInterval);

    jQuery.ajax({
        type: 'POST',
        url: './index.php',
        data: {
            task: 'callAttempt'
        },
        dataType: 'json',
        beforeSend: function (jqXHR) {
            jQuery('.loader').show();
            jQuery('div.order > table > tbody').html('').closest('.info').hide();
            jQuery('.pause').css('visibility', 'hidden');
        },
        success: function (json) {
            if (json.result == true) {
                var html = '<tr><td>Order ID</td><td><a target="_blank" href="../administrator/index2.php?page=order.order_list&show=&option=com_virtuemart&order_id_filter=' + json.obj.order_id + '">' + json.obj.order_id + '</a></td></tr>';
                html += '<tr><td>Last purchase</td><td>' + json.obj.cdate + '</td></tr>';
                html += '<tr><td>Occasion</td><td>' + json.obj.customer_occasion + '</td></tr>';
                // html += '<tr><td>Card Message</td><td>' + json.obj.customer_note + '</td></tr>';
                html += '<tr><td>Current status of order</td><td>' + json.obj.order_status_name + '</td></tr>';
                html += '<tr><td>Order total</td><td>' + json.obj.order_total + '</td></tr>';
                html += '<tr><td>Order products</td><td>' + json.obj.products + '</td></tr>';
                html += '<tr><td>Buyer name</td><td>' + json.obj.first_name + '</td></tr>';
                html += '<tr><td>Recipient name</td><td>' + json.obj.recipient_name + '</td></tr>';
                html += '<tr><td>Company</td><td>' + json.obj.company + '</td></tr>';
                html += '<tr><td>City</td><td>' + json.obj.city + '</td></tr>';
                html += '<tr><td>Email</td><td>' + json.obj.email + '</td></tr>';
                html += '<tr><td>Phone</td><td>' + json.obj.phone + '</td></tr>';
                html += '<tr><td>New order</td><td><a target="_blank" href="../administrator/index2.php?option=com_phoneorder&order_call_type=MDayCalls&user_id=' + json.obj.user_id + '">Link</a></td></td>';

                jQuery('#id').val(json.obj.id);
                jQuery('div.order > table > tbody').html(html).closest('.info').css('display', 'flex');
            } else {
                myInterval();
            }
            jQuery('.loader').hide();
        },
        error: function (jqXHR, textStatus, errorThrown) {
            alert(jqXHR.responseText + textStatus + errorThrown);
            jQuery('.loader').hide();
            myInterval();
        },
        timeout: 30000
    });
}

function setPause() {
    jQuery('.loader').hide();
    clearInterval(window.callAttemptInterval);
    openModal(2, 'Have a nice pause =)');

    jQuery('a.pause').hide();
    jQuery('a.play').show();
}

function setPlay() {
    jQuery('.loader').show();

    jQuery('a.play').hide();
    jQuery('a.pause').show();
    myInterval();
}

function setCount() {
    jQuery.ajax({
        type: 'POST',
        url: './index.php',
        data: {
            task: 'setCount'
        },
        dataType: 'json',
        context: this,
        beforeSend: function (jqXHR) {
        },
        success: function (json) {
            if (json.result == true) {
                jQuery('#all_count').text(json.all);
                jQuery('#ext_count').text(json.ext);
                jQuery('#all_ext_count').text(json.all_ext);
            }
        },
        error: function (jqXHR, textStatus, errorThrown) {
            console.log(jqXHR.responseText);
        },
        timeout: 10000
    });
}

jQuery(document).ready(function () {

    if (needAttempt == true) {
        myInterval();
        setCount();
    }

    jQuery('#btn_disturb').click(function (e) {
        e.preventDefault();

        jQuery('span.input-group-append').trigger('click');
    });

    jQuery('#btn_bad_number').click(function (e) {
        e.preventDefault();

        jQuery.ajax({
            type: 'POST',
            url: './index.php',
            data: {
                task: 'badNumber',
                comment: jQuery('#comment').val(),
                id: jQuery('#id').val()
            },
            dataType: 'json',
            context: this,
            beforeSend: function (jqXHR) {
                jQuery(this)
                    .prop('disabled', true)
                    .find('.spinner-border').removeClass('d-none');
            },
            success: function (json) {
                if (json.result != true) {
                    openModal(1, json.error);
                }
                setCount();

                jQuery(this)
                    .prop('disabled', false)
                    .find('.spinner-border').addClass('d-none');

                jQuery('.loader').show();
                myInterval();
                jQuery(this).removeClass('need');
            },
            error: function (jqXHR, textStatus, errorThrown) {
                openModal(1, jqXHR.responseText);

                jQuery(this)
                    .prop('disabled', false)
                    .find('.spinner-border').addClass('d-none');
            },
            timeout: 20000
        });
    });

    jQuery('#btn_disturb').click(function (e) {
        e.preventDefault();

        jQuery.ajax({
            type: 'POST',
            url: './index.php',
            data: {
                task: 'doNotDisturb',
                comment: jQuery('#comment').val(),
                id: jQuery('#id').val()
            },
            dataType: 'json',
            context: this,
            beforeSend: function (jqXHR) {
                jQuery(this)
                    .prop('disabled', true)
                    .find('.spinner-border').removeClass('d-none');
            },
            success: function (json) {
                if (json.result != true) {
                    openModal(1, json.error);
                }
                setCount();

                jQuery(this)
                    .prop('disabled', false)
                    .find('.spinner-border').addClass('d-none');

                jQuery('.loader').show();
                myInterval();
                jQuery(this).removeClass('need');
            },
            error: function (jqXHR, textStatus, errorThrown) {
                openModal(1, jqXHR.responseText);

                jQuery(this)
                    .prop('disabled', false)
                    .find('.spinner-border').addClass('d-none');
            },
            timeout: 20000
        });
    });

    jQuery('#btn_sale').click(function (e) {
        e.preventDefault();

        jQuery.ajax({
            type: 'POST',
            url: './index.php',
            data: {
                task: 'setSale',
                comment: jQuery('#comment').val(),
                id: jQuery('#id').val()
            },
            dataType: 'json',
            context: this,
            beforeSend: function (jqXHR) {
                jQuery(this)
                    .prop('disabled', true)
                    .find('.spinner-border').removeClass('d-none');
            },
            success: function (json) {
                if (json.result != true) {
                    openModal(1, json.error);
                }
                setCount();

                jQuery(this)
                    .prop('disabled', false)
                    .find('.spinner-border').addClass('d-none');

                jQuery('.loader').show();
                myInterval();
                jQuery(this).removeClass('need');
            },
            error: function (jqXHR, textStatus, errorThrown) {
                openModal(1, jqXHR.responseText);

                jQuery(this)
                    .prop('disabled', false)
                    .find('.spinner-border').addClass('d-none');
            },
            timeout: 20000
        });
    });

    jQuery('#btn_voicemail').click(function (e) {
        e.preventDefault();

        jQuery.ajax({
            type: 'POST',
            url: './index.php',
            data: {
                task: 'setVoicemail',
                comment: jQuery('#comment').val(),
                id: jQuery('#id').val()
            },
            dataType: 'json',
            context: this,
            beforeSend: function (jqXHR) {
                jQuery(this)
                    .prop('disabled', true)
                    .find('.spinner-border').removeClass('d-none');
            },
            success: function (json) {
                if (json.result != true) {
                    openModal(1, json.error);
                }
                setCount();

                jQuery(this)
                    .prop('disabled', false)
                    .find('.spinner-border').addClass('d-none');

                jQuery('.loader').show();
                myInterval();
                jQuery(this).removeClass('need');
            },
            error: function (jqXHR, textStatus, errorThrown) {
                openModal(1, jqXHR.responseText);

                jQuery(this)
                    .prop('disabled', false)
                    .find('.spinner-border').addClass('d-none');
            },
            timeout: 20000
        });
    });

    jQuery('#resultModal').on('hide.bs.modal', function (e) {
        jQuery('.loader').show();
        myInterval();

        jQuery(this).removeClass('need');
    });

});