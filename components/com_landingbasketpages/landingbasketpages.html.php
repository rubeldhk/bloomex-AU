 ...
 * @subpackage Contact
 */
class HTML_LandingBasketPages {

    function viewPage($Page) {
        global $database, $Itemid, $mosConfig_live_site, $mosConfig_absolute_path, $mm_action_url, $my, $VM_LANG, $mainframe, $mosConfig_lang;

        $mainframe->setPageTitle2(str_replace($healthy, $yummy, $Page->meta->title));
        $mainframe->addMetaTag2('keywords', str_replace($healthy, $yummy, $Page->meta->keywords));
        $mainframe->addMetaTag2('description', str_replace($healthy, $yummy, $Page->meta->description));
        ...
        $href = array();
        $sProductRandom = '';
        $sql = " SELECT href FROM jos_vm_landing_page_gift where lang='" . $direng[$mosConfig_lang] . "' AND position='categ_land';";
        $database->setQuery($sql);
        $check_categ_land = $database->loadResult();
        $sql = " SELECT href FROM jos_vm_landing_page_gift where lang='" . $direng[$mosConfig_lang] . "' AND position='categ_product';";
        $database->setQuery($sql);
        $check_categ_product = $database->loadResult();
        if ($check_categ_product == '1') {
            $sql = " SELECT sku FROM jos_vm_landing_products_gift where lang='" . $direng[$mosConfig_lang] . "' ORDER BY num;";
            $database->setQuery($sql);
            ...
            $sql = " SELECT href FROM jos_vm_landing_page_gift where lang='" . $direng[$mosConfig_lang] . "' AND position='body' ORDER BY num;";
            $database->setQuery($sql);
            $rows = $database->loadObjectList();
            if ($rows) {
                foreach ($rows as $row) {
        ...
        <script>
            ...
            jQuery(document).ready(function() {
                ...
                document.getElementById("map_canvas").style.width = (document.getElementById("new-main-banner-banner").offsetWidth) + "px";
                document.getElementById("map_canvas").style.styleFloat = "right";
                document.getElementById("map_canvas").style.position = "absolute";
                jQuery('.banner-size').css('position', 'absolute');
                jQuery('.banner-size').css('z-index', '2');
                jQuery('.banner-size').css('width', '650px');
                initialize();
            });
        </script>
        <?php
        ...
        if ($check_categ_land == '1') {
            ...
            if ($db->num_rows() > 0) {
                ?>
                <table width="100%" border="0" cellpadding="0" cellspacing="0" id="product-top-cell">
                    <?php
                    ...
                    if ($max_items == 1) {
                        ...
                        for ($i = 0; $i < $max_items; $i++) {
                            ...
                            if ($i == 0)
                                echo "<tr>\n";
                            echo "<td align=\"center\"><div id=\"productlist\">";
            ...
            if ($check_categ_product == '0') {
                ...
                <table width="100%" border="0" cellpadding="0" cellspacing="0" id="product-top-cell">
                    <tr>
                        ...
                        $step = 0;
                        for ($i = 0; $i < $max_items; $i++) {
                            $restitlehref = explode('[--1--]', $href[$i]);
                            if ($restitlehref[0] != '') {
                                echo "<td align=\"center\">";
                                echo '<a href="' . $restitlehref[0] . '" title="' . $restitlehref[1] . '">';
                                echo '<strong>';
                                echo $restitlehref[1];
                                echo '</strong>';
                                echo '   <img width="100%" style="margin: 0px 0px 0px 1px" src="' . $mosConfig_live_site . '/images/' . $direng[$mosConfig_lang] . '/landing_mid_gift_' . $direng[$mosConfig_lang] . $i . '.png" alt=" " border="0">  ';
                                echo '  </a>  ';
                                echo "</td>";
                                $step++;
                                if ($step == $products_per_row && $step < $max_items) {
                                    $step = 0;
                                    echo "</tr><tr>";
                                }
                            }
                        }
                        if ($step > 0 && $step < $products_per_row)
                            echo "<td colspan='" . ( $products_per_row - $step ) . "'></td>";
                        ?>
                    </tr>
                    `</table>
                <?php
