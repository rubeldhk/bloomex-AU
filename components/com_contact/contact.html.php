...
class HTML_contact {
    ...
    static function displayTile ($rows) {
        global $Itemid, $mosConfig_live_site, $hide_js, $mosConfig_google_captcha_front;
        /*<script src="https://maps.googleapis.com/maps/api/js?sensor=false&key=AIzaSyCDlnvxmoLg7G0t1AHz5DZ9vIlDnRD-Mxg"></script>*/

        $help = "Need Help? Contact Us Here";
        $phone = "Phone";
        $email = "Email";
        $name = "Name";
        $message = "Subject";
        ...
        ?>
        <script>
            function initialize(address, key) {
                var geocoder;
                var map;
                ...
                geocoder.geocode({'address': address}, function(results, status) {
                    if (status == google.maps.GeocoderStatus.OK) {
                        ...
                        map = new google.maps.Map(document.getElementById("map_canvas" + key), mapOptions);
    ...
    <div class="container padding0">
        <div class="row">
            <div class="col-md-6 col-md-offset-3 contact_wrapper">
                <h3><?= $help ?></h3>
                <div class="card contact_inner" style="padding: 30px;">
                    <form method="post" enctype="multipart/form-data" action="">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name="order_number" id="order_number" placeholder="Enter your order number (optional)">
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <small>Max 10 files, each up to 15MB.</small>
                        </div>
                        <script>
                            (function(){
                                ...
                                if (input) {
                                    input.addEventListener('change', function(){
                                        ...
                                        var maxBytes = 15 * 1024 * 1024; // 15MB
                                        for (var i = 0; i < files.length; i++) {
                                            if (files[i].size > maxBytes) {
                                                alert('File "' + files[i].name + '" exceeds 15MB.');
                                                this.value = '';
                                                return;
                                            }
                                        }
                        ...
                        <div class="form-group">
                            <div class="g-recaptcha" data-sitekey="<?= ($mosConfig_google_captcha_front ?? '6LcfVQYaAAAAANTgSUbCcBi0xb9E4SzHVLK3JbyF') ?>"></div>
                        </div>
                        <div class="form-group">
                            <input type="checkbox" name="policy" id="policy" value="1" style="transform: scale(1.2); margin-right: 8px;">
                            <span class="label_text2">
                                    By checking this box, I agree to receive Customer Care and Marketing SMS messages from Bloomex at the phone number provided above.
                                    The SMS frequency may vary. Data rates may apply. For assistance Reply HELP, Reply STOP to opt out of receiving SMS messages.
                                    For more information please visit our
                                    <a style="color: blue;" href="<?= $mosConfig_live_site ?>/privacy-policy/" target="_blank">Privacy Policy</a> page and
                                    <a style="color: blue;" href="<?= $mosConfig_live_site ?>/terms-and-conditions/" target="_blank">Terms of Use</a> page.
                                  </span>
                        </div>
                        <button type="submit" class="btn-help md-12"><?= $submit ?></button>
                        <input type="hidden" name="option" value="com_contact">
                        <input type="hidden" name="Itemid" value="<?= (int)$Itemid ?>">
                        <input type="hidden" name="task" value="need_help">
                    </form>
                </div>
            </div>
        </div>
            ...
            foreach ($rows as $key => $row) {
                ...
                <script>
                    jQuery(document).ready(function() {
                        document.getElementById("map_canvas<?php echo $key; ?>").style.display = "block";
                        document.getElementById("map_canvas<?php echo $key; ?>").style.height = (document.getElementById("contact_google_map<?php echo $key; ?>").offsetHeight)+"px";
                        document.getElementById("map_canvas<?php echo $key; ?>").style.width = (document.getElementById("contact_google_map<?php echo $key; ?>").offsetWidth)+"px";
                        document.getElementById("map_canvas<?php echo $key; ?>").style.styleFloat="right";
                        document.getElementById("map_canvas<?php echo $key; ?>").style.position="absolute";
                        document.getElementById("map_canvas<?php echo $key; ?>").style.backgroundSize="cover";
                        document.getElementById("map_canvas<?php echo $key; ?>").style.backgroundPosition="center";

                        var address = "<?php echo urlencode($row->address . ", " . $row->postcode . ", " . $row->country); ?>";
                        document.getElementById("map_canvas<?php echo $key; ?>").style.backgroundImage= 'url(<?php echo signQuery('https://maps.googleapis.com/maps/api/staticmap?center='.urlencode($row->address . ", " . $row->postcode . ", " . $row->country).'&markers=color:purple|label:B|'.urlencode($row->address . ", " . $row->postcode . ", " . $row->country).'&zoom=12&size=241x200&key=AIzaSyCYT3iYKoWNQvYcP46pdjt9Ro_pevOksvI', 'ODzJ-le6_6EHn3j734noQNPW9Zs='); ?>)';
                        var key = <?php echo $key; ?>;
                        /*initialize(address, key);*/
                    });
                </script>     
                <div class="col-xs-12 col-sm-4 col-md-3 col-lg-3 contact_wrapper">
                    <div class="contact_inner">
                        <div class="data">
                            <span class="subtitle"><?php echo $row->name; ?></span><br/>
                            Attn. Customer Service<br/>
                            <?php  $str = explode(',', $row->address); echo $str[0]; ?>
                            <?php echo $row->suburb . ', ' . $row->state; ?><br/>
                            <?php echo $row->postcode; ?>
                        </div>
                        <div class="contact_google_map" id="contact_google_map<?php echo $key; ?>">
                            <div id="map_canvas<?php echo $key; ?>"></div>                                
                        </div>
                    </div>
                </div>     
                <?php
