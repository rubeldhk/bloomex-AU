...
if (!defined('VM_BROWSE_ORDERBY_FIELD') && file_exists(sh404SEF_ADMIN_ABS_PATH . 'com_virtuemart/virtuemart.cfg.php')) {
    ...
    $defaultOrderField = 'product_name';
...
switch ($page) {
    case 'shop.browse':
        ...
        if (!empty($root)) {
            ...
            if (($sefConfig->shVMInsertCategories && !empty($category_id)) || (!$sefConfig->shVMInsertCategories && empty($product_id))) {
        ...
        if (empty($product_id) && empty($category_id) && empty($manufacturer_id) && empty($Search) && empty($root)) {
    ...
    case 'shop.product_details':
        ...
        if (@count($rows) > 0) {
            ...
            if ($sefConfig->shVMInsertCategories) {
                $cat = mosGetParam($_REQUEST, 'category_id', $row->category_id);
                $title = array_merge($title, vm_sef_get_category_array($database, $cat, $option, $shLangName));
            }
    ...
    case 'checkout.index': // note: this is not currently used, as VM 1.0.10 misses some calls to sefRelToAbs()
        if (!$sefConfig->shForceNonSefIfHttps) {
            ...
            if ((!empty($checkout_this_step)) || (!empty($ship_to_info_id)) || (!empty($shipping_rate_id)) || (!empty($payment_method_id)) || (!empty($first_payment_method_id)) || (!empty($payment_method_id)) || (!empty($checkout_next_step)))
                $dosef = false;
        } else
            $dosef = false;
        break;
