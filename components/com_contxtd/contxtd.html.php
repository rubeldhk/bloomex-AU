...
* ==================================================================
* @version $Id: contxtd.html.php,v 1.2 2005/09/30 17:21:14 cubalibre Exp $
* @package Mambo
* @subpackage Contact
...
class HTML_contxtd {
	...
	function displaylist( &$categories, &$rows, $catid, $currentcat=NULL, &$params, $tabclass ) {
		...
		if ( ( !$params->get( 'page_title' )) && ( $currentcat->name ) ) {
			/** @CubaLibre: changed page title to category name and implemented displayheader() instead */
			?>
		...
		<tr>
			...
			if ( count( $rows ) ) {
				/** @CubaLibre: changed class name */
				HTML_contxtd::showTable( $params, $rows, $catid, $tabclass );
			}
			?>
			</td>
		</tr>
		...
		<tr>
			...
			if ( ( $params->get( 'type' ) == 'category' ) && $params->get( 'other_cat' ) ) {
				/** @CubaLibre: changed class name */
				HTML_contxtd::showCategories( $params, $categories, $catid );
	...
	function showTable( &$params, &$rows, $catid, $tabclass ) {
		...
		if ( $params->get( 'headings' ) ) {
			...
			<tr>
				...
				<?php  
				/**
				 *  @CubaLibre: display company info
				 */ 
				if ( $params->get( 'company' ) ) {
					?>
					<td height="20" class="sectiontableheader<?php echo $params->get( 'pageclass_sfx' ); ?>">
					<?php echo _CONTACT_HEADER_COMPANY; ?>
					</td>
					<?php
				}
				?>
				...
				<?php
				/**
				 * @CubaLibre: display mobile phone
				 */
				if ( $params->get( 'mobile' ) ) {
					?>
					<td height="20" class="sectiontableheader<?php echo $params->get( 'pageclass_sfx' ); ?>">
					<?php echo _CONTACT_HEADER_MOBILE; ?>
					</td>
					<?php
				}
				?>
		...
		foreach ($rows as $row) {
			...
			?>
			<tr>
				<td width="20%" height="20" class="<?php echo $tabclass[$k]; ?>">
				...
				if ( $params->get( 'position' ) ) {
					?>
					<td width="10%" class="<?php echo $tabclass[$k]; ?>">
	...
	*/
	function showCategories( &$params, &$categories, $catid ) {
		...
		foreach ( $categories as $cat ) {
			if ( $catid == $cat->catid ) {
				...
				$link = 'index.php?option=com_contxtd&amp;catid='. $cat->catid .'&amp;Itemid='. $Itemid;
				?>
				<li>
					<a href="<?php echo sefRelToAbs( $link ); ?>" class="category<?php echo $params->get( 'pageclass_sfx' ); ?>">
					<?php echo $cat->title;?>
	...
	static function viewcontact( &$contact, &$params, $count, &$list, &$menu_params, $task ) {
		...
		function ViewCrossReference( selSelectObject ){
			...
			for ($i = 0; $i < $n; $i++) {
			   echo "\nlinks[".$list[$i]->value."]='"
					. sefRelToAbs( 'index.php?option=com_contxtd&task=view&contact_id='. $list[$i]->value .'&Itemid='. $Itemid )
					. "';";
			}
			?>
		...
		if ( $menu_params->get( 'page_title' ) ) {
			/**
			* @CubaLibre: disabled display of header, created an independent function instead
			*
			* ?>
		...
		// displays Page Title
		/** @CubaLibre: changed class name */
		HTML_contxtd::_writePageTitle( $params );

		// displays Contact Select box
		/** @CubaLibre: changed class name */
		HTML_contxtd::_writeSelectContact( $contact, $params, $count );

		// displays Name & Position
		/** @CubaLibre: changed class name */
		HTML_contxtd::_writeContactName( $contact, $params, $hide_js );
		?>
		<tr>
			<td>
				...
				<tr>
					...
					// displays Image
					/** @CubaLibre: changed class name */
					HTML_contxtd::_writeImage( $contact, $params );
					?>
					</td>
				</tr>
				<tr>
					...
					/** @CubaLibre: added function call */
					HTML_contxtd::_writeContactCompany( $contact, $params );

					// displays Address
					/**  @CubaLibre: changed class name */
					HTML_contxtd::_writeContactAddress( $contact, $params );

					// displays Email & Website
					/** @CubaLibre: added function call */ 
					HTML_contxtd::_writeContactWeb( $contact, $params );
					
					// displays Telephone & Fax
					/** @CubaLibre: chaged class name */
					HTML_contxtd::_writeContactPhone( $contact, $params );

					// displays Misc Info
					/** @CubaLibre: changed class name */
					HTML_contxtd::_writeContactMisc( $contact, $params );
					?>
					</td>
				</tr>
				</table>
		...
		// displays Email Form
		/** @CubaLibre: changed class name */
		HTML_contxtd::_writeVcard( $contact, $params );
		
		// displays Email Form
		/** @CubaLibre: changed class name */
		HTML_contxtd::_writeEmailForm( $contact, $params, $sitename, $task );
		?>
		</table>
		<?php
	...
	function _writeSelectContact( &$contact, &$params, $count ) {
		if ( ( $count > 1 )  && !$params->get( 'popup' ) && $params->get( 'drop_down' ) ) {
			...
			<tr>
				...
				<br />
				<form action="<?php echo sefRelToAbs( 'index.php?option=com_contxtd&amp;Itemid='. $Itemid ); ?>" method="post" name="selectForm" target="_top" id="selectForm">
		...
		if ( $contact->name ||  $contact->con_position ) {
			if ( $contact->name && $params->get( 'name' ) ) {
				...
				<tr>
					...
					<?php
					// displays Print Icon
					$print_link = $mosConfig_live_site. '/index2.php?option=com_contxtd&amp;task=view&contact_id='. $contact->id .'&amp;Itemid='. $Itemid .'&amp;pop=1';
	...
	*/
	...
	/**
	 * @CubaLibre: Writes Company Info
	 */
	function _writeContactCompany( &$contact, &$params ) {
		if ( $contact->company && $params->get( 'company' ) ) {
			global $mosConfig_live_site;
			?>
			<table width="100%" cellpadding="0" cellspacing="0" border="0">
			<tr>
				<td rowspan="2" valign="top" width="<?php echo $params->get( 'column_width' ); ?>" align="left">
				<?php
				echo $params->get( 'marker_company' );
				?>
				</td>
				<td valign="top">
				<?php
	...
	*/
	...
	/**
	* @CubaLibre: Writes Contact Info Web
	*/
	function _writeContactWeb( &$contact, &$params ) {
		/** @CubaLibre: added website check */
		if ( $contact->email_to || $contact->website ) {
			global $mosConfig_live_site;
			?>
			<table width="100%" cellpadding="0" cellspacing="0" border="0">
			<?php
			...
			/** @CubaLibre: displays website */
			if ( $contact->website && $params->get( 'website' ) ) {
				?>
				<tr>
					<td width="<?php echo $params->get( 'column_width' ); ?>" align="left">
					<?php
					...
					<td>
					<a href="http://<?php echo $contact->website; ?>" target="_new">
					<?php 
	...
	/**
	* @CubaLibre: Writes Contact Info Phone
	*/
	function _writeContactPhone( &$contact, &$params ) {
		/** @CubaLibre: added mobile phone check */
		if ( $contact->telephone1  || $contact->fax || $contact->mobile1 ) {
			global $mosConfig_live_site;
			?>
			<table width="100%" cellpadding="0" cellspacing="0" border="0">
			<?php
			/** @CubaLibre: modified telephone */
			if ( $contact->telephone1 && $params->get( 'telephone1' ) ) {
				?>
				<tr>
				<?php
				/**
				* @CubaLibre: check for second phone number to decide table layout 
				*/
				if ( $contact->telephone2 && $params->get( 'telephone2' ) ) { 
					?>
					<td rowspan="2" valign="top" width="<?php echo $params->get( 'column_width' ); ?>" align="left">
					<?php
					} else {
					...
					<td>
					<?php
					echo $contact->telephone1;
					?>
					</td>
				</tr>
				<?php
			}
			/** @CubaLibre: added second phone number */
			if ( $contact->telephone2 && $params->get( 'telephone2' ) ) {
				?>
				<tr>
			...
			/** @CubaLibre: added first mobile phone */
			if ( $contact->mobile1 && $params->get( 'mobile1' ) ) {
				?>
				<tr>
				<?php
				/**
				* @CubaLibre: check for second mobile phone to decide table layout 
				*/
				if ( $contact->mobile2 && $params->get( 'mobile2' ) ) { 
					?>
					<td rowspan="2" valign="top" width="<?php echo $params->get( 'column_width' ); ?>" align="left">
					<?php
					} else {
					...
					<td>
					<?php
					echo $contact->mobile1;
					?>
					</td>
				</tr>
				<?php
			}
			/** @CubaLibre: added second mobile phone */
			if ( $contact->mobile2 && $params->get( 'mobile2' ) ) {
				?>
				<tr>
					<td>
					<?php
					echo $contact->mobile2;
					?>
					</td>
				</tr>
				<?php
	...
	*/
	function _writeContactMisc( &$contact, &$params ) {
		/** @CubaLibre: added check of second misc info */
		if ( ($contact->misc1 && $params->get( 'misc1' )) || ($contact->misc2 && $params->get( 'misc2' )) ) {
			global $mosConfig_live_site;
			?>
			<table width="100%" cellpadding="0" cellspacing="0" border="0">
			<?php
			/** @CubaLibre: modified misc */
			if ( $contact->misc1 && $params->get( 'misc1' ) ) {
				?>
				<tr>
			...
			if ( $contact->misc2 && $params->get( 'misc2' ) ) {
				?>
				<tr>
					<td width="<?php echo $params->get( 'column_width' ); ?>" valign="top" align="left">
					<br>
					<?php
					echo $params->get( 'marker_misc2' );
					?>
					</td>
					<td>
					<br>
	...
	*/
	function _writeVcard( &$contact, &$params ) {
		if ( $params->get( 'vcard' ) ) {
			...
			<tr>
				...
				<a href="index2.php?option=com_contxtd&amp;task=vcard&amp;contact_id=<?php echo $contact->id; ?>&amp;no_html=1">
				<?php echo(_VCARD);?>
				</a>
				</td>
			</tr>
	...
	/**
	* Writes Email form
	* @CubaLibre: added input field for cat_id to define redirection after sending of email
	*/
	function _writeEmailForm( &$contact, &$params, $sitename, $task ) {
		global $Itemid;
				
		if ( $contact->email_to && !$params->get( 'popup' ) && $params->get( 'email_form' ) ) {
		
			/** @CubaLibre: added blog check */
			if ( $task == 'blog' ) {
				?>
				<tr>
					<td colspan="2">
					<br />
					<a href="<?php echo sefRelToAbs( 'index.php?option=com_contxtd&amp;task=view&amp;contact_id='. $contact->id .'&amp;Itemid='. $Itemid ); ?>"><?php echo(_CONTACT_BLOG_EMAIL); ?></a>
  					<br /><br />
					</td>
				</tr>
				<?php
				...
				<tr>
					...
					<br /><br />
					<form action="<?php echo sefRelToAbs( 'index.php?option=com_contxtd&amp;Itemid='. $contact->id ); ?>" method="post" name="emailForm" target="_top" id="emailForm">
					...
					<input type="hidden" name="option" value="com_contxtd" />
					<input type="hidden" name="con_id" value="<?php echo $contact->id; ?>" />
					<input type="hidden" name="cat_id" value="<?php echo $contact->catid; ?>" />
					<input type="hidden" name="sitename" value="<?php echo $sitename; ?>" />
					<input type="hidden" name="email_redirect" value="<?php echo $params->get( 'email_redirect' ); ?>" />
	...
	static function nocontact( &$params ) {
		?>
		<br /><br />
