...
class HTML_content {
	...
	*/
	function showContentList( $title, &$items, &$access, $id=0, $sectionid=NULL, $gid, &$params, &$pageNav, $other_categories, &$lists, $order ) {
		...
		$meta_title = isset($title->en_meta_title) ? $title->en_meta_title : '';
		$meta_description = isset($title->en_meta_title) ? $title->en_meta_description : '';
		$meta_keyword = isset($title->en_meta_title) ? $title->en_meta_keyword : '';

		if($meta_title){
			$mainframe->setPageTitle2($meta_title);
		}
		if($meta_description){
			$mainframe->addMetaTag2('description',$meta_description);
		}
		if($meta_keyword){
			$mainframe->addMetaTag2('keywords',$meta_keyword);
		}
	...
	*/
	static function show( $row, $params, $access, $page=0 ) {
	...
	* determines the link and `link text` of the readmore button & linked title
	*/
	static function _linkInfo( &$row, &$params ) {
		global $my;
	...
	* Writes Title
	*/
	static function Title( &$row, &$params, &$access ) {
		if($row->title){
	...
	* Writes Section
	*/
	static function Section( &$row, &$params ) {
	...
	* Writes Category
	*/
	static function Category( &$row, &$params ) {
		if ( $params->get( 'category' ) ) {
			?>
			<span>
				<?php
	...
	*/
	static function ReadMore ( &$row, &$params ) {
