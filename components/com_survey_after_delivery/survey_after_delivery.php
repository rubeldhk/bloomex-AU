...
function viewPage() {
    ...
    $q = "SELECT id from tbl_cron_survey_send 
      where order_id='" . $database->getEscaped($order_id) . "' and type='delivery'  and survey_page_open_datetime is null ";
    $database->setQuery($q);
    ...
    $q = "SELECT id from tbl_survey_after_delivery where user_id='" . $user_id . "' and order_id='" . $order_id . "'";
    $database->setQuery($q);
    $res = $database->loadResult();
    if ($res) {
        joomAlert::add_alert('you have already filled this survey', 'danger');
        header("Location: $mosConfig_live_site");
    }

    if ($user_id && $order_id) {
        HTML_Survey_after_delivery::viewPage($user_id, $order_id);
    } else {
        joomAlert::add_alert('sorry, we cannot find your survey page', 'danger');
        header("location: $mosConfig_live_site");
    }
}
...
function savePage() {
    ...
    if ($capcha && $user_id) {
        ...
        $how_would_you_rate_the_freshness_and_appearance_of_your_items = trim(mosGetParam($_POST, "how_would_you_rate_the_freshness_and_appearance_of_your_items"));
        ...
        $q = "INSERT INTO `tbl_survey_after_delivery` ( 
                        `user_id`,
                        `comments`,
                        `how_would_you_rate_the_freshness_and_appearance_of_your_items`,
                        `survey_date`,
                        `book_datetime`,
                        `order_id`,
    ...
    VALUES (";
        $q .= $user_id . ",";
        $q .= "'" . $comments . "',";
        $q .= "'" . $how_would_you_rate_the_freshness_and_appearance_of_your_items . "',";
        ...
        $body .= "Survey Type: Receiver \r\n\r\n";
        $body .= "Comment: " . $comments . "\r\n\r\n";
        mosMail($mosConfig_mailfrom, $mosConfig_fromname, $survey_notification_email_address, $subject, $body);

        $q = "SELECT id from tbl_cron_survey_send 
        where order_id='" . $database->getEscaped($order_id) . "' and type='delivery'  and survey_send_datetime is null ";
        $database->setQuery($q);
        $res = $database->loadResult();
        if ($res) {
        ...
        if (
                in_array($how_would_you_rate_your_delivery_experience, ['4', '5']) &&
                in_array($how_was_your_experience_overall, ['4', '5']) &&
                in_array($how_would_you_rate_the_freshness_and_appearance_of_your_items, ['4', '5'])
        ) {
            HTML_Survey_after_delivery::savePage(true);
        } else {
            HTML_Survey_after_delivery::savePage(false);
        }
    } else {
