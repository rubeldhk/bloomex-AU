...
switch( $task ) {
    ...
    case 'subscribe':
        subscribeUser();
        break;
    case 'setOverlayCapture':
        setOverlayCapture();
        break;

    case 'closeOverlayCapture':
        closeOverlayCapture();
        break;
	default:
		HTML_user::frontpage();
...
function closeOverlayCapture()
{
    global $mosConfig_offset;

    $_SESSION['guest']['overlay_capture_close'] = true;
    $_SESSION['guest']['time'] = time() + ($mosConfig_offset * 60 * 60);
...
{
    global $database, $mosConfig_limit_sms_sender_AccountKey;

    $_SESSION['guest']['overlay_email'] = $_POST['email'];
    $couponCode = "BLX-" . strtoupper(genRandomString(6));
    try {
        $query = "SELECT
            `c`.`coupon_id` 
        ...
        $result_coupon = $database->loadResult();
        if (!$result_coupon) {
            $couponCode = "BLX-" . strtoupper(genRandomString(6));
        }

        $query = sprintf("INSERT INTO jos_vm_overlay_capture_client (
                email, 
                first_name, 
                phone, 
                ip, 
        ...
        $sql = "INSERT INTO #__vm_coupons(coupon_code, percent_or_total , coupon_type, coupon_value,expiry_date )
						VALUES('$couponCode', 'percent', 'gift', '10.00','$futureDate')";
        $database->setQuery($sql);
        $couponAdded =  $database->query();

        $message = json_encode(['message' => 'Coupon not created.']);

        if($couponAdded) {

            $promoMessage = sprintf("Your Bloomex Discount Code is: %s | Call 1-800-905-147 for assistance placing your order.",
                $couponCode
            );

            $client = new SoapClient('http://smsgateway.ca/SendSMS.asmx?WSDL');
            $parameters = new SMSParam;
            $parameters->CellNumber = formatMobileNumber($_POST['phone']);
            $parameters->AccountKey = $mosConfig_limit_sms_sender_AccountKey;
            $parameters->MessageBody = $promoMessage;
            ...
            $query = sprintf("UPDATE jos_vm_overlay_capture_client 
                    SET is_email = '%s', 
                    is_sms = '%s', 
                    updated_at = NOW()
                    WHERE `email` = '%s' OR `ip` = '%s' OR `phone` = '%s'",
                1,
                1,
                $_POST['email'],
                $_POST['ip_address'],
...

{
    ...
    global $database, $mosConfig_mailfrom_noreply, $mosConfig_fromname;


    $email = $_POST['email'];

    $query = sprintf("SELECT * FROM `jos_vm_users_subscription` WHERE `email` = '%s'", $database->getEscaped($email));
    ...
    if ($result) {
        ...
        echo json_encode(['message' => 'Email already subscribed','result'=>false]);
    ...
    $query = sprintf("INSERT INTO `jos_vm_users_subscription` (`email`, `created_at`) VALUES ('%s', '%s')", $database->getEscaped($email), $created_at);
    ...
    $subject = 'Bloomex subscription';

    $message = '<html><body><p>Hi,</p><p>Thank you for subscribing to special offers.</p></body></html>';

    mosMail($mosConfig_mailfrom_noreply, $mosConfig_fromname, $email, $subject, $message, 1);


    echo json_encode(['message' => 'Thank you for subscribing to special offers.','result'=>true]);
