...
if ($task == 'save_keyword') {
	global $database;
	//==================== MR DUY ANH /====================
	$searchword = mosGetParam($_REQUEST, 'keyword', '');
	
	if( !empty($searchword) ) {
		$keywords	= htmlentities( $searchword, ENT_QUOTES );
	
		$sExistKeywords	= "SELECT COUNT(*) FROM tbl_search_log WHERE keywords = '$keywords' ";
		$database->setQuery($sExistKeywords);
		$bExist			= intval($database->loadResult());
		
		if( empty($bExist) ) {			
			$sInsertKeywords	= "INSERT INTO tbl_search_log(keywords, hits, cdate)  VALUES('$keywords', 1, '". date("Y-m-d") ."') ";
			$database->setQuery($sInsertKeywords);
			$database->query();
		}else {
			$sUpdateKeywords	= "UPDATE tbl_search_log SET hits = hits + 1 WHERE keywords = '$keywords' ";
			$database->setQuery($sUpdateKeywords);
			$database->query();
		}			
	}
	//==================== MR DUY ANH /====================
	echo $bExist;//."=============".$sExistKeywords."------------".$sInsertKeywords."==----------==".$sUpdateKeywords;
	exit(0);
}
