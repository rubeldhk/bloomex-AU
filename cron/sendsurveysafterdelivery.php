...
FROM 
  ...
  LEFT JOIN `tbl_cron_survey_send` AS `c` ON `c`.`order_id`=`o`.`order_id` AND `c`.type='delivery'
  LEFT JOIN `jos_vm_api2_orders` AS `d` ON `d`.`order_id`=`o`.`order_id`
WHERE 
  STR_TO_DATE(o.ddate, '%d-%m-%Y') BETWEEN NOW() - INTERVAL 5 DAY AND NOW() - INTERVAL 2 DAY
  AND `ui`.`address_type` = 'ST' AND `d`.id IS NULL
...
echo "results: " . $result->num_rows . "<hr>";

while ($obj = $result->fetch_object()) {
    $query = "INSERT INTO tbl_cron_survey_send (order_id,email,date,type) VALUES ( '{$obj->order_id}','{$obj->user_email}',NOW(),'delivery')";
    $result2 = $mysqli->query($query);
    
    $query = "INSERT INTO `jos_vm_order_history` (`order_id`,`order_status_code`,`comments`,`date_added`,`user_name`) VALUES ('{$obj->order_id}','{$obj->order_status}','sent survey after delivery','" . $datetime . "','Cron')";
    $mysqli->query($query);

    $cache = base64_encode('user_id=' . $obj->user_id . '&order_id=' . $obj->order_id. '&type=delivery');
    $nothing = 0;
...
<div>
        <span style=\"font-size:14px;\">In order to continue providing excellent service to our customers, we would appreciate if you would take a few minutes to complete this very brief <a href=\"". $mosConfig_live_site."/survey-after-delivery/?data={$cache}&utm_source=survey-after-delivery&utm_medium=email-survey&utm_campaign=order-id\"  target=\"_self\">survey </a>.</span></div><br>
<div>
