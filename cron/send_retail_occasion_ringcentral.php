...
global $mosConfig_host, $mosConfig_user, $mosConfig_password, $mosConfig_db, $mosConfig_ringcentral_client_id,
       ...
       $mosConfig_ringcentral_campaign_id_retail_occasion, $mosConfig_logger_file_path, $occasion_not, $timeZone,
       $selectLimitOccasion, $mosConfig_live_site;
...
function checkTodayCall($number, $mysqli): bool
{
    $query = sprintf("SELECT *
        FROM `jos_vm_retail_occasion_calls`
...
if ($occasionCalls->num_rows > 0) {
    ...
    try {
        ...
        $params = [
            "campaignIds" => [$mosConfig_ringcentral_campaign_id_retail_occasion],
            "leadStates" => ["READY"]
        ];
        ...
        if (count($occasionCallIds) > 0) {
            $queryUpdate = sprintf("UPDATE `jos_vm_retail_occasion_calls` 
            ...
            if (!$resultUpdate) {
                echo "Update jos_vm_retail_occasion_calls failed: " . $mysqli->error . PHP_EOL;
                $logger->error("Update jos_vm_retail_occasion_calls failed: " . $mysqli->error . PHP_EOL);
            }
        }
        ...
        if (count($leadIds) > 0) {
            foreach ($leadIds as $leadId) {
                $url = "admin/accounts/{$mosConfig_ringcentral_account_id}/campaignLeads/{$leadId}?duplicateHandling=REMOVE_ALL_EXISTING&campaignId={$mosConfig_ringcentral_campaign_id_retail_occasion}";
...
if (count($need_states) > 0) {
    ...
    $queryNumbers = sprintf("SELECT 
        ...
        FROM `jos_vm_api2_orders` AS ao
        INNER JOIN `jos_vm_orders` AS o ON o.order_id = ao.order_id  
        ...
        LEFT JOIN jos_vm_retail_occasion_calls AS oc ON oc.order_id = o.order_id  AND DATE(oc.created_at) BETWEEN CURDATE() - INTERVAL 6 DAY AND CURDATE()
        LEFT JOIN jos_vm_order_occasion AS oo on oo.order_occasion_code = o.customer_occasion
        WHERE
            ao.order_type <> 'pos' AND
            (
                (
                    (o.`customer_occasion` NOT IN ('%s') AND STR_TO_DATE(DATE_FORMAT( FROM_UNIXTIME(o.cdate) ,  '%%Y-%%m-%%d'),  '%%Y-%%m-%%d') >= STR_TO_DATE(DATE_FORMAT( DATE_SUB( NOW( ) , INTERVAL 1 YEAR ) ,  '%%Y-%%m-%%d'),  '%%Y-%%m-%%d'))
                        OR
                    (o.`customer_occasion` NOT IN ('%s') AND STR_TO_DATE(DATE_FORMAT( FROM_UNIXTIME(o.cdate) ,  '%%Y-%%m-%%d'),  '%%Y-%%m-%%d') >= STR_TO_DATE(DATE_FORMAT( DATE_SUB( NOW( ) , INTERVAL 2 YEAR ) ,  '%%Y-%%m-%%d'),  '%%Y-%%m-%%d')) 
                        OR
                    (o.`customer_occasion` NOT IN ('%s') AND STR_TO_DATE(DATE_FORMAT( FROM_UNIXTIME(o.cdate) ,  '%%Y-%%m-%%d'),  '%%Y-%%m-%%d') >= STR_TO_DATE(DATE_FORMAT( DATE_SUB( NOW( ) , INTERVAL 3 YEAR ) ,  '%%Y-%%m-%%d'),  '%%Y-%%m-%%d'))   
                )
            AND 
                (
                    ou.call_customer='DEFAULT' AND  ou.address_type = 'BT' AND ou.phone_1 !=  ''
                )
            )
            AND o.`customer_occasion` NOT IN ('%s')
    ...
    if (!$result) {
        $logger->error('SELECT jos_vm_api2_orders error: ' . $mysqli->error);
        die('SELECT jos_vm_api2_orders error: ' . $mysqli->error);
    }

    if ($result->num_rows > 0) {
        $leads = [
            'description' => 'Bloomex retail occasion calls',
        ...
        foreach ($rows as $index => $out) {
            ...
            if (
                ...
                if (mb_strlen($only_d) > 8) {
                    ...
                    $linkSetOrder = $mosConfig_live_site . '/administrator/index2.php?option=com_phoneorder&user_id=' . $out['user_id'] . '&order_call_type=occasion&products_filter=nz';
                    ...
                    $queryInsert = sprintf("INSERT INTO `jos_vm_retail_occasion_calls`(
                            `order_id`, 
                            `user_id`,
                            `first_name`,
                            `last_name`,
                    ...
                    if (!$resultOccasionCalls) {
                        $logger->error("INSERT jos_vm_retail_occasion_calls failed: " . $mysqli->error . PHP_EOL);
                        die("INSERT jos_vm_retail_occasion_calls failed: " . $mysqli->error . PHP_EOL);
                    }
...
if ($leads && count($leads['uploadLeads']) > 0) {
    try {
        ...
        $url = "admin/accounts/{$mosConfig_ringcentral_account_id}/campaigns/{$mosConfig_ringcentral_campaign_id_retail_occasion}/leadLoader/direct";
