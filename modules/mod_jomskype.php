...
if ($oktocontinue)
	...
	if ($useskypesimages=="0")
		...
		if ($showcallalways==1)
			echo '<a href="skype:'.$skypeid.'?call" onclick="return skypeCheck();"><img src="https://bloomex.ca/modules/jomskype/'.$statusImage.'" border=0></a>';
		else 
			{
			if ($showcallalways!=2)
				{
				if ($status->getNum()==2 || $status->getNum()==7)
					...
					echo '<img src="https://bloomex.ca/modules/jomskype/'.$statusImage.'" border=0>';
				}
			}
		}
	else
...
function drawSelectBox($fields = array (),$showcall,$showsendfile,$showchat,$showadd,$showprofile,$skypeid,$showvoicemail,$imagesArray,$useimages,$statusImage) {
	...
	foreach ($fields as $field) 
		...
		if ($show==true)
			...
			$theField=$field[0];
			if ($useimages==1)
				echo '<a href="skype:'.$skypeid.'?'.$field[0].'" onclick="return skypeCheck();"><img src="https://bloomex.ca/modules/jomskype/'.$imagesArray[0][$theField].'" border=0></a><br/>';
			else
