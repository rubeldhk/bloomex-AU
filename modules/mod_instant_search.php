...
<?php
if( !empty($_REQUEST['keyword_product']) ) {
	$search_field = '<input alt="search" class="inputbox'. $moduleclass_sfx .'" type="text" name="searchword" size="'. $width .'" value="'. $_REQUEST['keyword_product'] .'"  onblur="if(this.value==\'\') this.value=\''. $text .'\';" onfocus="if(this.value==\''. $text .'\') this.value=\'\';" onkeyup="if (this.value.length < max_chars) { prepareSearch(this.value); } else { this.value = this.value.substring(0, max_chars - 1); }" />';
}else{
...
if ( $button ) {
	$search_button = '<input type="button" value="'. $button_text .'" class="button'. $moduleclass_sfx .'" onclick="searchProduct();" />';
} else {
	$search_button = '';
}
...
<form name="productSearchForm" method="post" action="index.php?page=shop.browse&option=com_virtuemart&orderby=product_list&DescOrderBy=ASC">
	<input type="hidden" value="<?php echo ( !empty($_REQUEST['keyword_product']) ? $_REQUEST['keyword_product'] : "/Search" ); ?>" size="15" name="keyword_product" class="inputbox" alt="search">
</form>
<script type="text/javascript">
	function searchProduct() {
		document.productSearchForm.keyword_product.value	= document.instantSearchForm.searchword.value;
...
<form action="<?php echo sefRelToAbs("index.php"); ?>" method="post" name="instantSearchForm" onsubmit="return searchProduct();">
