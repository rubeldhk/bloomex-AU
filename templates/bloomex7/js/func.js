...
var select_bouquet = $j("[name='select_bouquet']").is(':checked')?$j("[name='select_bouquet']:checked").val():'standard';
        var extra_touches =$j('input[name="extra_products"]').is(':checked')?$j('input[name="extra_products"]:checked').serialize():0; 
        ...
        $j.post("index.php",
                {
                    ...
                    ajaxSend: function () {
                        $j("#div_" + formId).html('<img src="' + sImgLoading + '" align="absmiddle" height="53"/>&nbsp;&nbsp;' + sVM_UPDATING);
                    }
                },
        function (data) {
            aData = data.split("[--1--]");
            //     console.log(aData[0]);
            //       console.log(aData[0] == "success");
            if (aData[0] == "success") {
                if (sLang == 'french') {
                    sLangClass = "-fr";
                    sShortLang = "fr";
                ...
                //					if( bMember ) {
                //						sCartLink	= sSecurityUrl + "index.php?page=checkout.index&option=com_virtuemart&Itemid=80&lang=" + sShortLang;
                //					}else {
                sCartLink = sSecurityUrl + "index.php?page=shop.cart&option=com_virtuemart&Itemid=80&lang=" + sShortLang;
                //}	

                $j("#div_" + formId).html(sVM_ADD_PRODUCT_SUCCESSFUL + "<br/><center><a class='show-cart-now" + sLangClass + "' href='" + sCartLink + "'>&nbsp;</a></center>");

                $j("#modCartItem").html(aData[1]);
            } else {
                $j("#div_" + formId).html(sVM_ADD_PRODUCT_UNSUCCESSFUL);
            }
        }
        );
    });
    ...
    $j(".delete-cart-item").click(function () {
        ...
        $j.post("index.php",
                {
                    ...
                    product_id: product_id_value,
                    ajaxSend: function () {
                        $j("#msgReportBlock_" + product_id_value).html('<img src="' + sImgLoading + '" align="absmiddle"/><br/>' + sVM_DELETING);
                    }
                },
        function (data) {
            aData = data.split("[--1--]");
       //     console.log(aData[0]);
            if (aData[0] == "success") {
                if (current_page != "shop.cart") {
                    location.href = sSecurityUrl + "index.php?page=checkout.index&option=com_virtuemart&Itemid=80&reload=1&cart_msg=" + sVM_DELETE_SUCCESSFUL;
                } else {
                    location.href = sSecurityUrl + "index.php?page=shop.cart&option=com_virtuemart&Itemid=80&cart_msg=" + sVM_DELETE_SUCCESSFUL;
                }
            } else {
                $j("#msgReport").css('display', 'none');
                $j("#msgReportBlock_" + product_id_value).html(sVM_DELETE_UNSUCCESSFUL);
            }
        }
        );
    });
        ...
        $j.post("index.php",
                {
                    ...
                    quantity: quantity_value,
                    ajaxSend: function () {
                        $j("#msgReportBlock_" + product_id_value).html('<img src="' + sImgLoading + '" align="absmiddle"/><br/>' + sVM_UPDATING);
                    }
                },
        function (data) {
            aData = data.split("[--1--]");
            if (aData[0] == "success") {
                if (current_page != "shop.cart") {
                    location.href = sSecurityUrl + "index.php?page=checkout.index&option=com_virtuemart&Itemid=80&reload=1&cart_msg=" + sVM_UPDATE_CART_ITEM_SUCCESSFUL;
                } else {
                    location.href = sSecurityUrl + "index.php?page=shop.cart&option=com_virtuemart&Itemid=80&cart_msg=" + sVM_UPDATE_CART_ITEM_SUCCESSFUL;
                }
            } else if (aData[0] == "error") {
                $j("#msgReportBlock_" + product_id_value).html(aData[1]);
            } else {
                $j("#msgReportBlock_" + product_id_value).html(sVM_UPDATE_CART_ITEM_UNSUCCESSFUL);
            }
        }
        );
    });
    ...
    $j("#displayUpdateBillingInfo").click(function () {
        ...
        function (data) {
            ...
            if ($j.trim(aData[0]) == "success") {
                ...
                $j("input[name='address_suite_billing']").val(aData2[18]);
                $j("input[name='address_street_number_billing']").val(aData2[19]);
                $j("input[name='address_street_name_billing']").val(aData2[20]);
                $j('#updateBillingInfoForm').modal({
                    onOpen: modalOpen,
                    position: ["25%", "25%"]
                });
            } else {
    ...
    $j("#btnSelectDeliveryOption").click(function () {
        //zip_billing
        if ($j("input[name='delivery_date_2']").val() != '')
        {
            var delivery_date =  $j("input[name='delivery_date_2']").val();
        }
        else
        {
             var delivery_date = nMonthsValid + "/" + nDaysValid + "/" + nYearsValid;
        }
        ...
        $j.post("index.php",
                {
                    ...
                    delivery_default_date: nMonthsValid + "/" + nDaysValid + "/" + nYearsValid,
                    ...
                    subtotalprice: $j("input[name='sub_total_price']").val(),
                    user_info_id: $j("input[name='ship_to_info_id']:checked").val()
                },
        function (data) {
            $j('#selectDeliveryOption').html(data);
            $j("#selectDeliveryOption").css('display', 'block');
            $j('#selectDeliveryOption').modal({
                onOpen: modalOpen,
                onClose: closeCalendar(),
                position: ["25%", "25%"],
                containerId: 'selectDeliveryOptionContainer',
                containerCss: 'selectDeliveryOptionContainer'
            });
        }
        );
    });
    ...
    function closeCalendar() {
        $j("#selectDeliveryOption").css('display', 'none');
    }
    ...
    $j("#updateBillingInfo").click(function () {
        ...
        var address_suite = $j("input[name='address_suite_billing']").val();
        var address_street_number = $j("input[name='address_street_number_billing']").val();
        var address_street_name = $j("input[name='address_street_name_billing']").val();
        ...
        if (address_street_number == "") {
            ...
            $j('#address_street_number_po').css("color", "red");
            return;
        } else {
            $j('#address_street_number_po').css("color", "");
        }
        if (address_street_name == "") {
            alert('Please enter street name');
            $j('#address_street_name_po').css("color", "red");
            return;
        } else {
            $j('#address_street_name_po').css("color", "");
        }
        if (city_value == "") {
        ...
        if (!isValidZipCode(zip_value)) {
            alert(sVM_CONFIRM_VALID_ZIP_CODE);
            $j('#zip_field').css("color", "red");
            return;
        } else {
            $j('#zip_field').css("color", "");
        ...
        if (state_value == "" || state_value == "NA") {
            alert(sVM_CONFIRM_STATE);
        ...
        if (phone_1_value == "") {
            alert(sVM_CONFIRM_PHONE_NUMBER);
        ...
        $j.post("index.php",
                {
                    ...
                    ajaxSend: function () {
                        ...
                        $j("#msgReportUpdateBilling").html('<img width="24" src="' + sImgLoading + '" align="absmiddle"/>&nbsp;&nbsp;' + sVM_UPDATING_BILLING_INFO);
                    }
                },
        function (data) {
            ...
            if (data == "success") {
                ...
                $j("#name_field_value").html(first_name_value + ' ' + last_name_value);
                var str = '';
                if (address_suite.length > 0) {
                    str = address_suite + ',';
                }
                $j("#address_field_value").html(str + address_street_number + ' ' + address_street_name + ' ' + city_value + ', ' + state_value + ', ' + country_value);
                $j("#phone_field_value").html(phone_1_value);
                $j("#email_field_value").html(email_value);
                $j("#msgReportUpdateBilling").css('display', 'block');
                $j("#msgReportUpdateBilling").html(sVM_UPDATE_BILLING_INFO_SUCCESSFUL);
                $j.modal.close();
            } else {
                $j("#msgReportUpdateBilling").css('display', 'block');
                $j("#msgReportUpdateBilling").html(sVM_UPDATE_BILLING_INFO_UNSUCCESSFUL);
            }
        }
        );
    });
    ...
    $j(".add-shipping-address").click(function () {
        $j("select[name='address_type2']").val("R");
        ...
        $j("input[name='address_suite']").val("");
        $j("input[name='address_street_number']").val("");
        $j("input[name='address_street_name']").val("");

        $j("input[name='city_shipping']").val("");
        $j("input[name='zip_shipping']").val("");
        $j("select[name='country_shipping']").val("AUS");
        changeStateList("state_shipping", "country_shipping");
        $j("select[name='state_shipping']").val("AT");
        $j("input[name='phone_1_shipping']").val("");
        $j("input[name='phone_2_shipping']").val("");
        $j("input[name='email_shipping']").val("");
        $j("input[name='func_shipping']").val("userAddressAdd");
        sCurrentZipChecked = $j("input[name='ship_to_info_id']:checked").val();
        $j('#addShippingInfoForm').modal({
            onOpen: modalOpen,
            position: ["25%", "25%"]
        });
        $j("#updateShippingInfo").attr("value", sVM_ADD_ADDRESS);
    });



    $j("#updateShippingInfo").click(function () {
        var address_type2 = $j("select[name='address_type2']").val();
        ...
        var address_suite = $j("input[name='address_suite']").val();
        var address_street_number = $j("input[name='address_street_number']").val();
        var address_street_name = $j("input[name='address_street_name']").val();
        ...
        var email_shipping_value = $j("input[name='email_shipping']").val();


        if (address_type2 == "") {
            alert(sVM_CONFIRM_ADDRESS_TYPE_2);
            $j('#address_type2').css("color", "red");
            return;
        } else {
            $j('#address_type2').css("color", "");
        }

        /*if( address_type_name_shipping_value == "" ) {
         alert(sVM_CONFIRM_ADD_NICKNAME);
        ...
        if (address_street_name == "") {
            alert(sVM_CONFIRM_ADDRESS);
            $j('#address_street_name').css("color", "red");
            return;
        } else {
            $j('#address_street_name').css("color", "");
        }
        if (address_street_number == "") {
            alert(sVM_CONFIRM_ADDRESS);
            $j('#address_street_number').css("color", "red");
            return;
        } else {
            $j('#address_street_number').css("color", "");
        }
        ...
        if (state_shipping_value == "" || state_shipping_value == "NA") {
            alert(sVM_CONFIRM_STATE);
        ...
        $j.post("index.php",
                {
                    ...
                    email: email_shipping_value,
                    current_zip_checked: sCurrentZipChecked,
                    ajaxSend: function () {
                        $j("#msgReportUpdateShipping").html('<img width="24" src="' + sImgLoading + '" align="absmiddle"/>&nbsp;&nbsp;' + sVM_UPDATING_DELIVER_INFO);
                    }
                },
        function (data) {
            aData = data.split("[--3--]");
            if (aData[0] == "success") {
                ...
                if ($j("input[name='func_shipping']").val() == "userAddressUpdate") {
                    $j("#msgReportDeliverInfo").html(sVM_UPDATE_DELIVER_INFO_SUCCESSFUL);
                } else {

                    $j("#msgReportDeliverInfo").html(sVM_ADD_DELIVER_INFO_SUCCESSFUL);
                    $j("select[name='address_type2']").val("");
                    ...
                    $j("input[name='last_name_shipping']").val("");
                    $j("input[name='company_shipping']").val("");
                    $j("input[name='suite']").val("");
                    $j("input[name='street_number']").val("");
                    $j("input[name='street_name']").val("");
                ...
                changDeliver(oForm.zip_checked.value, aData[2], aData[3]);
                $j.modal.close();
                $j("#mainCheckOutForm").css("display", "block");
                $j("#msgReportShippingAddressRadio").css("display", "none");
            }
            else if (aData[0] == 'undeliver')
            {
                $j("#msgReportUpdateShipping").html('');
                alert(sCannotDeliver);
                return;
            } else {
                if ($j("input[name='func_shipping']").val() == "userAddressUpdate") {
                    $j("#msgReportUpdateShipping").html(sVM_UPDATE_DELIVER_INFO_UNSUCCESSFUL);
                } else {
                    $j("#msgReportUpdateShipping").html(sVM_ADD_DELIVER_INFO_UNSUCCESSFUL);
                }
            }
        }
        );
        ...
        $j.post("index.php",
                {
                    option: "com_ajaxorder",
                    task: "validCart",
                    product_number_items: $j("input[name='product_number_items']").val(),
                    ajaxSend: function () {
                        $j("#msgCheckoutReport").html('<img src="' + sImgLoading + '" align="absmiddle"/>&nbsp;&nbsp;' + sVM_VALIDATING_CART);
                    }
                },
        function (data) {
            if (data == "reload") {
                location.href = sSecurityUrl + "index.php?page=checkout.index&option=com_virtuemart&Itemid=80&reload=1";
            } else {
                CalculateOrderPrice();
                $j("#msgCheckoutReport").html("");
            }
        }
        );
        //Ticket #4969 END
        ...
        $j.post("index.php",
                {
                    ...
                    product_number_items: $j("input[name='product_number_items']").val(),
                    ajaxSend: function () {
                        $j("#msgCheckoutReport").html('<img src="' + sImgLoading + '" align="absmiddle"/>&nbsp;&nbsp;' + sVM_VALIDATING_CART2);
                    }
                },
        function (data) {
            if (data == "reload") {
                location.href = sSecurityUrl + "index.php?page=checkout.index&option=com_virtuemart&Itemid=80&reload=1&reload_msg=" + sVM_VALIDATING_CART3;
            } else {
                $j("#msgCheckoutReport").html("");
                ...
                if (!$j.trim($j("#delivery_date_2").val())) {
                ...
                $j.ajax({
                    ...
                    data: ({
                        ...
                        select_bouquet: $j("input[name='select_bouquet']").val(),
                        card_comment: $j("textarea[name='card_comment']").val(),
                        deliver_date: $j("input[name='delivery_date_2']").val(),
                        ...
                        coupon_code_string: $j("input[name='coupon_code_string']").val(),
                        coupon_type_string: $j("input[name='coupon_type_string']").val(),
                        coupon_discount: $j("input[name='coupon_discount']").val(),
                        coupon_code: $j("input[name='coupon_code']").val(),
                        coupon_code_type: $j("input[name='coupon_code_type']").val(),
                        coupon_value: $j("input[name='coupon_value']").val(),
                        product_coupon_string: $j("input[name='product_coupon_string']").val(),
                        vendor_currency_string: $j("input[name='vendor_currency_string']").val(),
                        vendor_currency_string: 		$j("input[name='vendor_currency_string']").val(),
                                free_shipping: $j("input[name='free_shipping']").val(),
                        find_us: $j("input[name='find_us']:checked").val(),
                        ajaxSend: function () {
                            $j("#order-action-button").css('display', 'none');
                            $j("#msgCheckoutReport").html('<img src="' + sImgLoading + '" align="absmiddle"/>&nbsp;&nbsp;' + ((sLang === 'french') ? 'Ordre est le Traitement' : 'Order checkout is processing') + '...');
                        }
                    }),
                    ...
                    success: function (data) {
                        ...
                        } else if (jQuery.trim(aData[0]) == "error") {
                            $j("#order-action-button").css('display', 'block');
                    ...
                    error: function (x, t, m) {
                        if (t == "timeout") {
                            window.location.href = "index.php?page=checkout.thankyou&option=com_virtuemart&Itemid=80&msg=error_order";
                        }
                    }
                });
            }
...
function editDeliver(user_info_id_value, user_id_value) {
    $j("#updateShippingInfo").attr("value", sVM_UPDATE_ADDRESS);
    sCurrentZipChecked = $j("input[name='ship_to_info_id']:checked").val();
    $j.post("index.php",
            {
                option: "com_ajaxorder",
    ...
    function (data) {
        ...
        if ($j.trim(aData[0]) == "success") {
            ...
            if (!aData2[16])
                aData2[16] = "R";
            $j("select[name='address_type2']").val(aData2[16]);
            ...
            $j("select[name='country_shipping']").val(/*aData2[11]*/ 'AUS');
            changeStateList("state_shipping", "country_shipping");
            ...
            $j("input[name='email_shipping']").val(aData2[20]);
            $j("input[name='address_suite']").val(aData2[17]);
            $j("input[name='address_street_number']").val(aData2[18]);
            $j("input[name='address_street_name']").val(aData2[19]);
            ...
            $j("#msgReportDeliverInfo").css('display', 'block');
            $j("#msgReportDeliverInfo").html(sVM_LOAD_DELIVER_INFO_FORM_UNSUCCESSFUL);
        }
    }
    );
}
    ...
    $j.post("index.php",
            {
                ...
                ajaxSend: function () {
                    $j("#msgReportDeliverInfo").css('display', 'block');
                    $j("#msgReportDeliverInfo").html('<img src="' + sImgLoading + '" align="absmiddle"/>&nbsp;&nbsp;' + sVM_DELETING_DELIVER_INFO);
                }
            },
    function (data) {
        aData = data.split("[--3--]");
        if (aData[0] == "success") {
            $j("#msgReportDeliverInfo").css('display', 'block');
            $j("#msgReportDeliverInfo").html(sVM_DELETE_DELIVER_INFO_SUCCESSFUL);
            ...
            $j("#msgReportDeliverInfo").css('display', 'block');
            $j("#msgReportDeliverInfo").html(sVM_DELETE_DELIVER_INFO_UNSUCCESSFUL);
        }
    }
    );
}
...
//=======================================================================================================================
function changDeliver(sOptionString, sCurrentStateTax, zipCode) {
    
    
    if (zipCode)
    {
        $j("#post_code_zip_now").val(zipCode);
    }
    ...
    if ((aOption[3] == 0 && aOption[3] != null && sOptionString) || bIsValidZipCodeFirst == 0) {
        bIsValidZipCode = 0;
        alert(sCannotDeliver);
        return;
    } else {
    ...
    nYearsValid = parseFloat(nYearNow);
    ...
    if (nExtraDayMonth || parseFloat(oForm.daysofmonthnow.value) < nDaysValid) {
        // alert( nMonthsValid + "/" + nDaysValid  + "/" + nYearsValid + "===" + nExtraDayMonth  );              				
        ...
        if (parseFloat(nExtraDayMonth) > 0) {
            nDaysValid = nExtraDayMonth;
        }

        if (parseFloat(oForm.monthnow.value) == 12) {
            nMonthsValid = 1;
            nYearsValid = parseFloat(nYearNow) + 1;
        } else {
            nMonthsValid = parseFloat(oForm.monthnow.value) + 1;
            nYearsValid = parseFloat(nYearNow);
        }

        //alert( nMonthsValid + "/" + nDaysValid  + "/" + nYearsValid  );       
    }
...
function changeDeliverExtraSameDay( ) {
    //	alert( parseFloat(oForm.delivery_month2.value) + "===" + parseFloat(oForm.monthnow.value) + "===" + parseFloat(aDate[1]) + "===" + parseFloat(oForm.daynow.value) + "===" + parseFloat(oForm.cutofftime.value) );
    sCurrentDate = $j("#delivery_date_2").val();
    aDate = sCurrentDate.split("/");

    if (parseFloat(aDate[0]) == parseFloat(oForm.monthnow.value) &&
            parseFloat(aDate[1]) == parseFloat(oForm.daynow.value) &&
        ...
        /*if( parseFloat($j("input[name=deliver_extra_price]").val()) >= parseFloat(nDeliverFeeExtraSameDay) ) {
         $j("input[name=deliver_extra_price]").val(parseFloat(nDeliverExtraPrice));	
         }*/
        $j("input[name=deliver_extra_price]").val(parseFloat(nDeliverExtraPrice));
        $j("input[name=deliver_extra_day]").val(0);
    }

    //	alert(parseFloat(aDate[1]));
    nSpecialDeliverExtraPrice = isSpecialDate(parseFloat(aDate[0]), parseFloat(aDate[1]));
...
//alert(parseFloat($j("input[name=deliver_extra_price]").val()) + "=======" + nSpecialDeliverExtraPrice+ "=======" + parseFloat($j("input[name=deliver_extra_price]").val() + parseFloat(nSpecialDeliverExtraPrice)));
}
...
function noticeDeliver() {
    ...
    //	alert( nDaysValid + "===" + nMonthsValid );

    //if( ( parseFloat(aDate[0]) < nMonthNow  &&  nMonthNow != 12 ) || ( parseFloat(aDate[0]) == nMonthNow && parseFloat(aDate[1]) < nDayNow ) ) {
    if ((parseFloat(aDate[0]) < nMonthsValid && nMonthNow != 12) || (parseFloat(aDate[0]) == nMonthsValid && parseFloat(aDate[1]) < nDaysValid)) {
        alert(sInValidDateTime);
        bIsValidDate = 0;
        return;
    }
    ...
    if (isUnAvailableDate(parseFloat(aDate[0]), parseFloat(aDate[1]))) {
    ...
    //alert(parseFloat(aDate[0]) +"=-===="+ nMonthsValid +"=-===="+ parseFloat(aDate[1])+"=-===="+nDaysValid);
    //if( parseFloat(aDate[0]) < nMonthsValid ||  parseFloat(aDate[1]) < nDaysValid ) {
    if ((parseFloat(aDate[0]) < nMonthsValid && nMonthsValid != 12) || (parseFloat(aDate[0]) == nMonthsValid && parseFloat(aDate[1]) < nDaysValid)) {
        ...
        $j("input[name=deliver_extra_price]").val(0);
        $j("input[name=deliver_extra_day]").val(0);
        bIsValidDate = 0;
    } else {
        $j("#deliver_extra_post_code").css("display", "block");
...
function CalculateOrderPrice() {
    ...
    nTotalItemPrice = parseFloat($j("input[name=sub_total_price]").val());
   
    nTotalDeliveryFee = parseFloat($j("input[name=deliver_fee]").val());
   
    nTotalPrice = nTotalItemPrice + nTotalDeliveryFee;
   ...
   $j("#calcualte-total-items-price").html("$" + formatAsMoney(nTotalItemPrice));
   $j('#calcualte-total-deliver-fee').html('$'+formatAsMoney(nTotalDeliveryFee));
...
function changeUnAvailableDate(nCurrentMonth) {
    if (sUnAvailableDate) {
        ...
        for (i = 0; i < aUnAvailableDate.length; i++) {
            if (aUnAvailableDate[i] != "") {
                ...
                if (nCurrentMonth == aUnAvailableItem[0]) {
                    aItem[k] = aUnAvailableItem[1];
                    k++;
                }
            }
        }

        for (i = 1; i < oForm.delivery_day2.length; i++) {
            oForm.delivery_day2.options[i].text = oForm.delivery_day2.options[i].value;
            oForm.delivery_day2.options[i].style.color = "black";
        }
        ...
        for (i = 0; i < aItem.length; i++) {
            oForm.delivery_day2.options[aItem[i]].text = oForm.delivery_day2.options[aItem[i]].text + " - No delivery service  ";
            oForm.delivery_day2.options[aItem[i]].style.color = "red";
        }
    }
}
...
function validDeliverDate(submit_action) {
    ...
    //alert( parseFloat(aDate[0]) + "====" + nMonthsValid + "====" + parseFloat(aDate[1]) + "====" + nDaysValid + "====" + (parseFloat(aDate[0]) < nMonthsValid) + "====" + (parseFloat(aDate[1]) < nDaysValid ));
    if (parseFloat(aDate[0]) < nMonthsValid || parseFloat(aDate[1]) < nDaysValid) {
        alert(sInValidDateTime);
        return;
    }
...
function changeStateList(state_list_name, element_country_id) {
    ...
    if (element_country_id == "country_shipping") {
        selected_country = "AUS";
    } else if (element_country_id == "AUS") {
        selected_country = "AUS";
    } else {
    ...
    if (!document.getElementById(state_list_name)) {
        state_list_name = 'state';
    }
...
function social_login(social) {
    window.open("/index.php?option=social_login&ok=1&social=" + social);
}
...
function searchRedirect(url, last_keyword) {
    $j.post("index.php",
            {
                option: "com_instant_search",
                task: "save_keyword",
