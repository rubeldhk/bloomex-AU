...
*/
class mosMenuBar {
    static function customIconConfirm($task = '', $icon = '', $alt = '', $listSelect = true, $confirmline = false) {
        $image = mosAdminMenus::ImageCheckAdmin($icon, '/administrator/images/', NULL, NULL, $alt, $task, 1, 'middle', $alt);

        $action = "if (window.confirm('$confirmline')) {submitbutton('$task')}";
        if ($listSelect) {
            $href = "javascript:if (document.adminForm.boxchecked.value == 0){ alert('Please make a selection from the list to $alt');}else{ $action }";
        } else {
	...
	*/
	...
	function customY( $task='', $icon='',$alt='') {
		$image 	= mosAdminMenus::ImageCheckAdmin( $icon, '/administrator/images/', NULL, NULL, '', $task, 1, 'middle', '' );

		?>
		<td>
			<p class="toolbar customy" title="<?php echo $alt;?>">
				<?php echo $image; ?></p>
		</td>
		<style>
			.customy{
	...
	* @param string An override for the alt text
	*/
	function deleteList($func = '', $msg='', $task='remove', $alt='Delete' ) {
	...
	*/
	...
	function customicon( $task='', $icon='', $iconOver='', $alt='',$listSelect=true, $tooltip_img='',$href_attr=true) {
		$icon 	= ( $iconOver ? $iconOver : $icon );
		$image 	= mosAdminMenus::ImageCheckAdmin( $icon, '/administrator/images/', NULL, NULL, $alt, $task, 1, 'middle', $alt );
		$image_tooltip 	= mosAdminMenus::ImageCheckAdmin( $tooltip_img, '/administrator/images/', NULL, NULL, $alt, $task, 1, 'middle', $alt );
		if ($listSelect) {
			$href = "javascript:if (document.adminForm.boxchecked.value == 0){ alert('Please make a selection from the list to $alt');}else{submitbutton('$task')}";
		} else {
			$href = "javascript:submitbutton('$task')";
		...
		if(!$href_attr){
			$href = "javascript:void(0)";
                ...
                if ($task == 'parse_xlsx') {
                    $class = 'upload_xlsx';
                }elseif ($task == 'upload_list') {
                    $class = 'upload_list';
                }elseif ($task == 'upload_desc_list') {
                    $class = 'upload_desc_list';
                } elseif ($task == 'sold_products') {
                    $class = 'sold_products';
                }
                elseif ($task == 'assignorder_xlsx') {
                    $class = '" data-toggle="modal" data-target="#assignorder_xlsx';
                } elseif ($task == 'disableindexing_xlsx') {
                    $class = '" data-toggle="modal" data-target="#disableindexing_xlsx';
                }

		?>
		<td>
			<div class="my_tooltip">
				<a class="toolbar <?php echo $class; ?>" href="<?php echo $href;?>">
					<?php echo $image; ?>
					<br /><?php echo $alt; ?></a>
                                <?php
                                if ($task == 'parse_xlsx') {
                                    ?>
                                    <span class="tooltiptext">Correct Format
                                        <?php echo $image_tooltip; ?>
                                    </span>
                                    <?php
                                }
                                ?>
