...
function showSurvey( $option,$download ) {
    ...
    $query = "SELECT COUNT(*) FROM tbl_survey_after_delivery $where";
    ...
    $query = "SELECT o.*,s.survey_page_open_datetime,s.email_open_datetime,s.date as survey_send_datetime FROM tbl_survey_after_delivery as o left join tbl_cron_survey_send as s on s.order_id=o.order_id and s.type='delivery'  $where $orderBy";
...
if($download){
    ...
    HTML_Survey_after_delivery::showSurvey( $rows, $pageNav, $option, $lists );
}
}
