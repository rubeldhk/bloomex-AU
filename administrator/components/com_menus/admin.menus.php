...
switch ($task) {
	case 'new':
                mosRedirect( 'index2.php?option=com_menus&menutype='.$menutype.'&type=view&task=edit&hidemainmenu=1');
	...
	case 'unpublish':
        ...
        case 'show':
	case 'unshow':
		showMenuSection( $cid, ($task == 'show'), $menutype );
		break;
	...
	case 'saveorder':
		...
		break;
    case 'get_category_alias':
        get_category_alias($option);
        break;
    case 'get_content_alias':
        get_content_alias($option);
        break;
	default:
	...
	foreach ($rows as $v ) {
            ...
            Switch ($v->new_type) {
                case 'vm_category':
                    ...
                    $v->category_publish = $category_obj->category_publish;
                    break;
...
function saveMenu( $option, $task='save' ) {
    ...
    if ($mosConfig_live_site == 'https://bloomex.com.au') {
        $change_table = "jos_menu_history_live";
    } else {
        $change_table = "jos_menu_history_stage";
    }
    if($_POST['published'] == '0'){ $_POST['show']='0'; }

    $newdata=array();
    $newdata['name']=$_POST['name'];
    $newdata['alias']=$_POST['alias'];
    $newdata['new_type']=$_POST['new_type'];
    $newdata['published']=$_POST['published'];
    $newdata['show']=$_POST['show'];

    if($_POST["id"]){
        $query_select_old_data = "SELECT `name`,alias,new_type,published,`show` FROM jos_menu WHERE  id='" . $_POST["id"] . "'";
        $database->setQuery( $query_select_old_data );
        $olddata=$database->loadAssocList();
        if($olddata && $newdata){
            mosChangesNotification('menu_item_update',$olddata[0],$newdata,$_POST["id"]);

            $sql = "INSERT INTO `" . $change_table . "`
                            (`menu_id`, `name`,`old`, `new`,  `username`, `date`) VALUES ";
            if($newdata['name']!=$olddata[0]['name']){
                $sql .="(" . $_POST["id"] . ", 'name',  '" . $olddata[0]['name'] . "',  '" . $newdata['name'] . "',  '" . $my->username . "', DATE_SUB(NOW(), INTERVAL 4 HOUR)),";
            }
            if($newdata['alias']!=$olddata[0]['alias']){
                $sql .="(" . $_POST["id"] . ", 'alias',  '" . $olddata[0]['alias'] . "',  '" . $newdata['alias'] . "',  '" . $my->username . "', DATE_SUB(NOW(), INTERVAL 4 HOUR)),";
            }
            if($newdata['new_type']!=$olddata[0]['new_type']){
                $sql .="(" . $_POST["id"] . ", 'component',  '" . $olddata[0]['new_type'] . "',  '" . $newdata['new_type'] . "',  '" . $my->username . "', DATE_SUB(NOW(), INTERVAL 4 HOUR)),";
            }
            if($newdata['published']!=$olddata[0]['published']){
                $sql .="(" . $_POST["id"] . ", 'published',  '" . ($olddata[0]['published']== '0' ? 'N' : 'Y') . "',  '" . ($newdata['published']== '0' ? 'N' : 'Y') . "',  '" . $my->username . "', DATE_SUB(NOW(), INTERVAL 4 HOUR)),";
            }
            if($newdata['show']!=$olddata[0]['show']){
                $sql .="(" . $_POST["id"] . ", 'show',  '" . ($olddata[0]['show']== '0' ? 'N' : 'Y') . "',  '" . ($newdata['show']== '0' ? 'N' : 'Y') . "',  '" . $my->username . "', DATE_SUB(NOW(), INTERVAL 4 HOUR)),";
            }
            $sql = rtrim($sql,',');
    ...
    if(!$_POST["id"] && $newdata){
        mosChangesNotification('menu_item_added',$olddata[0],$newdata,$row->id);

        $sql = "INSERT INTO `" . $change_table . "`
                            (`menu_id`, `name`,  `username`, `date`) VALUES 
        (" . $row->id . ", 'Created',  '" . $my->username . "', DATE_SUB(NOW(), INTERVAL 4 HOUR))";
        $database->setQuery($sql);
        $database->query();
    }
	...
	$msg = 'Menu item Saved';
        ...
        Switch ($_POST['new_type']) {
            case 'vm_category':
                $query = "UPDATE `jos_vm_category` AS `c`
                SET
                    `c`.`alias`='".$database->getEscaped($_POST['alias'])."',
                    `c`.`category_publish`='".($_POST['published'] == '1' ? 'Y' : 'N')."'
                WHERE 
                    `c`.`category_id`=".(int)$_POST['link']."
                ";
...
function publishMenuSection( $cid=null, $publish=1, $menutype ) {
	...
	foreach ($cid as $id) {
        ...
        if($id){
            $query_select_old_data = "SELECT `name`,alias,new_type,published,`show` FROM jos_menu WHERE  id='" . $id . "'";
            $database->setQuery( $query_select_old_data );
            $olddata=$database->loadAssocList();
            $newdata=array();
            $newdata=$olddata[0];
            if($publish=='0'){
                $newdata['show']=$publish;
            }
            $newdata['published']=$publish;
            if($olddata && $newdata){
                mosChangesNotification('menu_item_update',$olddata[0],$newdata,$id);
            }

            $sql = "INSERT INTO `" . $change_table . "`
                                (`menu_id`, `name`, `old`, `new`, `username`, `date`) VALUES (" . $id . ", 'published', '".($olddata[0]['published'] == '0' ? 'N' : 'Y')."', '".($publish == '0' ? 'N' : 'Y')."', '" . $my->username . "', DATE_SUB(NOW(), INTERVAL 4 HOUR))";
            $database->setQuery($sql);
            $database->query();
        }
		...
		$menu->load( $id );
                ...
                Switch ($menu->new_type) {
                    case 'vm_category':
                        $query = "UPDATE `jos_vm_category`
                        SET
                            `category_publish`='".($publish == '1' ? 'Y' : 'N')."'
                        WHERE `category_id`=".$menu->link."
                        ";
        ...
        if($publish=='0'){
            ...
            $sql = "INSERT INTO `" . $change_table . "`
                                (`menu_id`, `name`, `old`, `new`, `username`, `date`) VALUES (" . $id . ", 'show', '".($olddata[0]['show'] == '0' ? 'N' : 'Y')."', '".($publish == '0' ? 'N' : 'Y')."', '" . $my->username . "', DATE_SUB(NOW(), INTERVAL 4 HOUR))";
            $database->setQuery($sql);
            $database->query();
...
function showMenuSection( $cid=null, $task='show', $menutype ) {
    ...
    if ($mosConfig_live_site == 'https://bloomex.com.au') {
        ...
        $publish = ($task == 'show' ? '1' : '0');

	if (!is_array( $cid ) || count( $cid ) < 1) {
		return 'Select an item to ' . ($task == 'show' ? 'publish' : 'unpublish');
	}
	...
	foreach ($cid as $id) {
        ...
        $sql= "INSERT INTO `" . $change_table . "`
                                (`menu_id`, `name`, `old`, `new`, `username`, `date`) VALUES (" . $id . ", 'show', '".($publish == '0' ? 'Y' : 'N')."', '".($publish == '0' ? 'N' : 'Y')."', '" . $my->username . "', DATE_SUB(NOW(), INTERVAL 4 HOUR))";
        $database->setQuery($sql);
        $database->query();
		...
		if ($menu->type) {
                    $database = &$database;
                    $task = $task == 'show' ? 'publish' : 'unpublish';
                    // $type value is used in *.menu.php
                    $type = $menu->type;
                    require_once( $mosConfig_absolute_path . '/administrator/components/com_menus/' . $type . '/' . $type . '.menu.php' );
		}
...
function TrashMenuSection( $cid=NULL, $menutype='mainmenu' ) {
	...
	$children = array();
	foreach ( $cid as $id ) {
        mosChangesNotification('menu_item_delete','','',$id);
...
*/
function get_category_alias(){
    global  $database;
    $category_id = intval( mosGetParam( $_POST, 'category_id', 0 ) );
    $sql = "SELECT alias
...
function get_content_alias(){
    global  $database;
    $content_id = intval( mosGetParam( $_POST, 'content_id', 0 ) );
    $sql = "SELECT title_alias
                                FROM `jos_content`
