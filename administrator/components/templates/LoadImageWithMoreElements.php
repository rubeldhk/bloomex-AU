 ...
 */
class LoadImageWithMoreElements
{
    ...
    // public __construct
    function LoadImageWithMoreElements( $database_table, $folder_for_images, $names_array, $languages_array, $header, $elements, $image_name )
    {
    ...
    function show() { 
        ...
        for( $i = 0; $i < $count; $i++ )
        {
            ...
            $count_element = count($this->elements);
            
            for( $i2 = 0; $i2 < $count_element; $i2++ )
            {
                $res = '0';
                switch ($i2) {
                    case 0:
                ...
                $html .= '
                        <td>
                           <input type="text" name="'.$this->elements[$i2].$this->names[$i].'" value="'.$res.'"> &nbsp;&nbsp;&nbsp;
                        </td>';
            }
    ...
    function result()
    {
        ...
        if( isset($_POST['upload']) )
        {
            ...
            for( $i = 0; $i < $count; $i++ )
            {
                ...
                $query  = "UPDATE `".$this->table."` SET title='".$_POST['title'.$second_name]."', sku='".$_POST['sku'.$second_name]."', price='".$_POST['price'.$second_name]."', href='".$_POST['href'.$second_name]."' WHERE id='".($i+1)."'";
                $database->setQuery($query);
