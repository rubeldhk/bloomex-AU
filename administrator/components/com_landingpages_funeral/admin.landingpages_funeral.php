...
switch ($act) {		
	...
	case "setup_category":
		switch ($task) {
			...
			default:
				HTML_LandingPages_Funeral::setupCategory( $option );
				break;
		}
		break;
	...
	default:
		switch ($task) {		
			case 'new':
				editLandingPages_Funeral( '0', $option);
				break;
		
			case 'edit':
				editLandingPages_Funeral( intval( $cid[0] ), $option );
				break;
		
			case 'editA':
				editLandingPages_Funeral( $id, $option );
				break;
		
			case 'save':
				saveLandingPages_Funeral( $option );
				break;
		
			case 'remove':
				removeLandingPages_Funeral( $cid, $option );
				break;
				
			case 'cancel':
				cancelLandingPages_Funeral();
				break;
		
			default:
				showLandingPages_Funeral( $option );
				break;
		}
		break;
...
//=================================================== LandingPages OPTION ===================================================
function showLandingPages_Funeral( $option ) {
	...
	$query = "SELECT COUNT(*) FROM tbl_landing_pages_funeral $where";
	...
	$query = "SELECT * FROM tbl_landing_pages_funeral $where ORDER BY city ASC";
	...
	HTML_LandingPages_Funeral::showLandingPages_Funeral( $rows, $pageNav, $option, $lists );
}


function editLandingPages_Funeral( $id, $option ) {
	global $database, $my, $mosConfig_absolute_path;

	$row = new mosLandingPages_Funeral( $database );
	...
	HTML_LandingPages_Funeral::editLandingPages_Funeral( $row, $option, $lists );
}


function saveLandingPages_Funeral( $option ) {
...
function removeLandingPages_Funeral( &$cid, $option ) {
	global $database, $act, $mosConfig_absolute_path;
	
	if (count( $cid )) {		
		foreach ($cid as $value) {			
			$query = "DELETE FROM tbl_landing_pages_funeral WHERE id = $value";
			$database->setQuery( $query );
			if (!$database->query()) {
...
function cancelLandingPages_Funeral() {
	mosRedirect('index2.php?option=com_landingpages_funeral');
}
...
function saveSetupCategory( $option ) {
	...
	if( $category_id ) {
		$sql = "UPDATE tbl_landing_pages_funeral SET category_id = ".$category_id;
