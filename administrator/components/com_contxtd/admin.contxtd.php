...
* ==================================================================
* @version $Id: admin.contxtd.php,v 1.2 2005/10/21 21:35:22 cubalibre Exp $
* @package Mambo
* @subpackage Contact
...
if (!($acl->acl_check( 'administration', 'edit', 'users', $my->usertype, 'components', 'all' )
		| $acl->acl_check( 'administration', 'edit', 'users', $my->usertype, 'components', 'com_contxtd' ))) {
...
function showContacts( $option ) {
	...
	$database->setQuery( "SELECT COUNT(*) FROM #__contxtd_details AS cd $where" );
	...
	/** @CubaLibre: changed option name */
	$lists['catid'] = mosAdminMenus::ComponentCategory( 'catid', 'com_contxtd_details', intval( $catid ), $javascript );
	/** @CubaLibre: changed class name */
	HTML_contxtd::showcontacts( $rows, $pageNav, $search, $option, $lists );
}
...
function editContact( $id, $option ) {
	...
	// build list of categories
	/** @CubaLibre: changed option name */
	$lists['catid'] 			= mosAdminMenus::ComponentCategory( 'catid', 'com_contxtd_details', intval( $row->catid ) );
	...
	/** @CubaLibre: changed component name */
	$file = $mosConfig_absolute_path .'/administrator/components/com_contxtd/contxtd_items.xml';
	$params =new  mosParameters( $row->params, $file, 'component' );

	/** @CubaLibre: changed class name */
	HTML_contxtd::editcontact( $row, $lists, $option, $params );
}
...
function saveContact( $option ) {
	...
	if ($row->default_con) {
		/** @CubaLibre: changed table name */
		$database->setQuery( "UPDATE #__contxtd_details SET default_con='0' WHERE id <> $row->id AND default_con='1'" );
		$database->query();
	}
...
function removeContacts( &$cid, $option ) {
	...
	if (count( $cid )) {
		$cids = implode( ',', $cid );
		/** @CubaLibre: changed table name */
		$database->setQuery( "DELETE FROM #__contxtd_details WHERE id IN ($cids)" );
		if (!$database->query()) {
...
function changeContact( $cid=null, $state=0, $option ) {
	...
	$cids = implode( ',', $cid );
	/** @CubaLibre: changed table name */
	$database->setQuery( "UPDATE #__contxtd_details SET published='$state'"
...
function cancelContact() {
	...
	$row->checkin();
	/** @CubaLibre: changed option name */
	mosRedirect('index2.php?option=com_contxtd');
}
?>