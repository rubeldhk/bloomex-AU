...
/**
*
* @version $Id: account.shipto.php,v 1.4 2007/01/24 06:18:32 paul Exp $
* @package VirtueMart
* @subpackage html
...
if( !$auth["user_id"] ) mosRedirect(sefRelToAbs('index.php?option=com_login&Itemid='.$_REQUEST['Itemid'])); 
...
if( !$db->num_rows()) {	
	...
	$selected_country_code = $db->sf("country");
}
...
<?php 
  ...
  <input type="hidden" class="inputbox" id="address_type_name" name="address_type_name" value=" <?php $db->sp("address_type_name") ?>" maxlength="64" size="16" />

  <div style="float:left;width:30%;text-align:right;<?php if (stristr($missing,'address_type_name')) echo $missing_style ?>">
    <?php echo "<label for=\"address_type_name\">".$VM_LANG->_PHPSHOP_USER_FORM_ADDRESS_TYPE_LABEL."</label>*" ?>:</div>
  <div style="float:left;width:60%;">
    <select class="inputbox" size="1" id="address_type2" name="address_type2">
		<option value="B" <?php if( $db->sf("address_type2") == "B" ) echo "selected='selected'"; ?>>Business</option>
		<option value="R" <?php if( $db->sf("address_type2") == "R" ) echo "selected='selected'"; ?>>Residential</option>
   </select>
  </div>
  <br/><br/>
...
<div style="float:left;width:30%;text-align:right;" >
    <?php echo "<label for=\"suite\">".$VM_LANG->_PHPSHOP_SHOPPER_FORM_SUITE."</label>" ?>:</div>
  <div style="float:left;width:60%;"> 
    <input type="text" id="suite" name="suite" size="40" value="<?php $db->sp("suite") ?>" class="inputbox" />
  </div>
<br/><br/><div style="float:left;width:30%;text-align:right;" >
    <?php echo "<label for=\"street_number\">".$VM_LANG->_PHPSHOP_SHOPPER_FORM_STREET_NUMBER."</label>" ?>:</div>
  <div style="float:left;width:60%;"> 
    <input type="text" id="street_number" name="street_number" size="40" value="<?php $db->sp("street_number") ?>" class="inputbox" />
  </div>
<br/><br/><div style="float:left;width:30%;text-align:right;" >
    <?php echo "<label for=\"street_name\">".$VM_LANG->_PHPSHOP_SHOPPER_FORM_STREET_NAME."</label>" ?>:</div>
  <div style="float:left;width:60%;"> 
    <input type="text" id="street_name" name="street_name" size="40" value="<?php $db->sp("street_name") ?>" class="inputbox" />
  </div>
<br/><br/>
...
<br/><br/>
      ...
      <div style="float:left;width:60%;">
        <?php $ps_html->list_country("country", $selected_country_code, "id=\"country_field\" only=\"AUS\" onchange=\"changeStateList();\"") ?>
      </div>
    <br/><br/>
    <?php 
...
?>
      ...
      echo $ps_html->dynamic_state_lists( "country", "state", $selected_country_code, $db->sf('state') );
      echo "<noscript>\n";
...
<br/><br/>
 ...
 <div style="float:left;width:45%;text-align:right;" >
    ...
    <a href="<?php $sess->purl( SECUREURL."index.php?page=$next_page") ?>" ><u><?php echo _BACK ?></u></a>
  </div>
  </form>
<?php
  if (!empty($user_info_id)) { ?>
