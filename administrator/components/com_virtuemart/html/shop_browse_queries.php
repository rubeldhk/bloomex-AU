...
$fieldnames = "`product_name`,`products_per_row`,`category_browsepage`,`category_flypage`,`#__{vm}_product`.`product_id`, 
			`#__{vm}_category`.`category_id`,`product_full_image`,`product_thumb_image`,`product_s_desc`,`product_parent_id`, 
			`product_publish`,`product_in_stock`,`product_sku`, #__{vm}_product_category_xref.product_list IS NULL AS product_list_null";
...
switch ($mosConfig_lang) {
    case 'french':
        $sSearchProduct = " AND `#__{vm}_product`.`product_id` IN ( SELECT DISTINCT reference_id FROM jos_jf_content 
							WHERE 	language_id = 2 AND  
									reference_table = 'vm_product' AND 
									( (reference_field = 'product_desc' AND `value` LIKE '%" . htmlspecialchars($keyword_product, ENT_QUOTES) . "%') OR
									  (reference_field = 'product_name' AND `value` LIKE '%" . htmlspecialchars($keyword_product, ENT_QUOTES) . "%') OR
									  (reference_field = 'product_sku' AND `value` LIKE '%" . htmlspecialchars($keyword_product, ENT_QUOTES) . "%') )
								      )";
        break;

    case 'english':
        if (!empty($keyword_product)) {
            $keyword1 = $vmInputFilter->safeSQL(urldecode(mosGetParam($_REQUEST, 'keyword_product', null)));
            $fieldnames .= ", CASE
						                         WHEN  `#__{vm}_product`.`product_name` LIKE '%$keyword1%' THEN 1
						                         WHEN  `#__{vm}_product`.`product_s_desc` LIKE '%$keyword1%' THEN 2
						                         WHEN  `#__{vm}_product`.`product_desc` LIKE '%$keyword1%' THEN 3
						                         ELSE 99999
						                 END AS orderby_search";
        }
...
switch ($orderby) {
    ...
    case 'product_list':
        $orderbyField = 'product_list_null ASC, `#__{vm}_product_category_xref`.`product_list` ASC, `#__{vm}_product`.`product_name` ';
        $DescOrderBy = " ASC ";
        break;
    case 'product_name':
        ...
        break;
    /*case 'product_price':
        // withut product discounts
        //$orderbyField = '`#__{vm}_product_price`.`product_price`'; break; 
        $orderbyField = "IF(`#__{vm}_product_discount`.`amount` IS NOT NULL,
                        IF(`#__{vm}_product_discount`.`is_percent`='1',
                        `#__{vm}_product_price`.`product_price`-`#__{vm}_product_price`.`product_price`*`#__{vm}_product_discount`.`amount`,
                        `#__{vm}_product_price`.`product_price`-`#__{vm}_product_discount`.`amount`),`#__{vm}_product_price`.`product_price`)";
        break;
     * 
     */
    case 'product_price':
        // withut product discounts
        //$orderbyField = '`#__{vm}_product_price`.`product_price`'; break; 
        $orderbyField = "`#__{vm}_product_price`.`product_price`-`#__{vm}_product_price`.`saving_price`";
        break;
    case 'product_sku':
...
/** Changed Product Type - End */ elseif (empty($manufacturer_id)) {

    /*     * * GET ALL PUBLISHED PRODUCTS ** */
    $list = "SELECT DISTINCT $fieldnames FROM (`#__{vm}_product`, `#__{vm}_category`, `#__{vm}_product_category_xref`,`#__{vm}_shopper_group`) ";
    $count = "SELECT $count_name FROM (`#__{vm}_product`, `#__{vm}_category`, `#__{vm}_product_category_xref`,`#__{vm}_shopper_group`) ";
    $q = "LEFT JOIN `#__{vm}_product_price` ON `#__{vm}_product`.`product_id` = `#__{vm}_product_price`.`product_id` LEFT JOIN `jos_vm_product_discount` ON `jos_vm_product`.`product_discount_id` = `jos_vm_product_discount`.`discount_id`";
    $q .= "\n WHERE `#__{vm}_product_category_xref`.`category_id`=`#__{vm}_category`.`category_id` ";
    if ($category_id) {
    ...
    if (!empty($keyword_product)) {
        $q .= "\n ORDER BY orderby_search ASC, $orderbyField $DescOrderBy";
    } else {
        $q .= "\n ORDER BY $orderbyField $DescOrderBy";
    }
...
/* * * GET ALL PUBLISHED PRODUCTS FROM THAT MANUFACTURER ** */ elseif (!empty($manufacturer_id)) {
    ...
    if ($perm->is_registered_customer($my->id)) {
        $list .= ",`#__{vm}_shopper_vendor_xref`) LEFT JOIN `#__{vm}_product_price` ON `#__{vm}_product`.`product_id` = `#__{vm}_product_price`.`product_id` 
            LEFT JOIN `jos_vm_product_discount` ON `jos_vm_product`.`product_discount_id` = `jos_vm_product_discount`.`discount_id` WHERE ";
        ...
        $list .= ")  LEFT JOIN `#__{vm}_product_price` ON `#__{vm}_product`.`product_id` = `#__{vm}_product_price`.`product_id` LEFT JOIN `jos_vm_product_discount` ON `jos_vm_product`.`product_discount_id` = `jos_vm_product_discount`.`discount_id` WHERE ";
