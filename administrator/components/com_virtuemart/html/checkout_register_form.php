 ...
 */
$required_fields = Array( 'first_name', 'last_name', 'address_street_number','address_street_name', 'city', 'zip', 'country', 'phone_1' );
...
//echo $shopper_fields[uniqid('fieldset_begin')];
...
$shopper_fields[uniqid('fieldset_begin')] = $VM_LANG->_PHPSHOP_USER_FORM_BILLTO_LBL;
	...
	if (CAN_SELECT_STATES == '1') {
		...
		$required_fields[] = 'state';
             ...
             $shopper_fields['email'] = $VM_LANG->_PHPSHOP_USER_FORM_EMAIL;
...
$shopper_fields['agreed2']	= "";
//$required_fields[] 			= 'agreed2';
...
<script language="javascript" type="text/javascript" src="includes/js/mambojavascript.js"></script>
    <div class="container checkout_login_form">
        <div class="row">
            <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6 wrapper">
            <div class="inner">
                <?php echo $VM_LANG->_PHPSHOP_NO_CUSTOMER ?>
<form action="<?php echo $mm_action_url ?>index.php" method="post" name="adminForm" >
   ...
   foreach( $shopper_fields as $fieldname => $label) {
   		if( stristr( $fieldname, 'fieldset_begin' )) {
   			echo '<fieldset class="checkout-login">
	...
	if( $fieldname != "agreed2") {
   		echo '<div id="'.$fieldname.'_div" class="checkout-login-left" style="';
		...
		echo ' </div><div class="checkout-login-right">'."\n";
       }	
      	/**
      	 ...
      	 */
   		switch( $fieldname ) {
			...
			case 'bank_account_type':
				echo '<select class="form-control" name="bank_account_type">
			...
			case 'agreed':
				echo '<input type="checkbox" id="agreed_field" name="agreed" value="1" class="form-control" />';
				break;
				
			case 'agreed2':
				echo '<div style="dispay:block;text-align:center" id="agreed2_div"><input type="checkbox" id="agreed2_field" name="agreed2" value="1"  /> I\'m interested in recieving promotional emails from Bloomex.</div>';
				break;
				
			case 'password':
			case 'password2':
				echo '<input type="password" id="'.$fieldname.'_field" name="'.$fieldname.'" size="30" class="form-control" maxlength="32" />'."\n";
	   			break;
	   		...
	   		case 'extra_field_4': case 'extra_field_5':
	   			...
	   			break;
                        case 'zip':
                            echo '<input type="text" id="'.$fieldname.'_field" name="'.$fieldname.'" size="30" value="'. mosGetParam( $_REQUEST, $fieldname) .'" class="form-control" maxlength="7" />'."\n";
                            break;
   			default:
		        echo '<input type="text" id="'.$fieldname.'_field" name="'.$fieldname.'" size="30" value="'. mosGetParam( $_REQUEST, $fieldname) .'" class="form-control" maxlength="32" />'."\n";
	   		//if( strpos( $fieldname, 'address' ) !== false ) echo '<br><small><strong>(Suite, StreetNumber, StreetName)</strong></small>';
                            break;
   		}
   		if( $fieldname != "agreed2") {
   		echo '</div>
    ...
    echo '
	<div class="checkout-register-btn">';
    
	if( !$mosConfig_useractivation && VM_SILENT_REGISTRATION != '1') {
		echo '<input type="hidden" name="remember" value="yes" id="remember_login2" checked="checked" />
		<br />';
	}
	else {
	...
	echo '
        <button type="submit" class="btn btn-default" onclick="return( submitregistration());">'. _BUTTON_SEND_REG . '</button><br>
	</div>
