...
$shopper_fields[uniqid('fieldset_begin')] = $VM_LANG->_PHPSHOP_USER_FORM_BILLTO_LBL;
	...
	$shopper_fields['last_name'] = $VM_LANG->_PHPSHOP_SHOPPER_FORM_LAST_NAME;
        $shopper_fields['suite'] = $VM_LANG->_PHPSHOP_SHOPPER_FORM_SUITE;//'Suite/Apt';
	$shopper_fields['street_number'] =$VM_LANG->_PHPSHOP_SHOPPER_FORM_STREET_NUMBER;//'Street number';
	$shopper_fields['street_name'] = $VM_LANG->_PHPSHOP_SHOPPER_FORM_STREET_NAME;//'Street name';
...
<div style="width:90%;">
   ...
   foreach( $shopper_fields as $fieldname => $label) {
      ...
      <div style="'.$field_div_style.'">'."\n";
   		...
   		switch( $fieldname ) {
			...
			case 'street_number':
			case 'street_name':
				echo '<input type="text" id="'.$fieldname.'_field" name="address_'.$fieldname.'" size="40" value="'. $db->sf($fieldname) .'" class="inputbox" />'."\n";
	   			break;
   			default:
   ...
   $sql	= "SELECT gid FROM #__users WHERE id = " . $auth["user_id"];
