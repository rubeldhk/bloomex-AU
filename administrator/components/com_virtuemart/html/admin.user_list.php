...
$list = "SELECT * FROM #__users AS u INNER JOIN #__{vm}_user_info AS ui ON u.id=ui.user_id";
$count = "SELECT COUNT(id) as amount FROM #__users AS u INNER JOIN #__{vm}_user_info AS ui ON u.id=ui.user_id";
$q = "";
if (!empty($keyword)) {
    $q = " WHERE ";
    $q .= " (u.username LIKE '%$keyword%' OR ";
    $q .= "u.email LIKE '%$keyword%' ";
    $q .= ") ";
}


$q .= " and ui.address_type='BT'";

$count .= $q;
$list .= $q . " LIMIT $limitstart, " . $limit;
//we don't really need to count if there is keyword, let's just sayd it's a lot

$db->query($count);
$num_rows_new = $db->f('amount');

// Create the Page Navigation
$pageNav = new vmPageNav($num_rows_new, $limitstart, $limit);
