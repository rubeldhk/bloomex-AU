...
if (!empty($keyword)) {
    ...
    $q = "";
    $list  = "SELECT * FROM #__{vm}_order_status  ";
    $count = "SELECT count(*) as num_rows FROM #__{vm}_order_status  ";
    $q .= " ORDER BY list_order ASC";
    $list .= $q . " LIMIT $limitstart, " . $limit;
    $count .= $q;
}
...
$columns = Array(  "#" => "width=\"20\"", 
    ...
    $VM_LANG->_PHPSHOP_PRODUCT_LIST_PUBLISH => '',
					_E_REMOVE => "width=\"5%\""
				);
...
while ($db->next_record()) { 
    ...
    $tmpcell = "<a href=\"". $sess->url($_SERVER['PHP_SELF']."?page=$modulename.order_status_list&limitstart=$limitstart&keyword=$keyword&order_status_id=".$db->f("order_status_id")."&func=changePublishState");
    ...
    $tmpcell .= vmCommonHTML::getYesNoIcon($publish, "Publish", "Unpublish" );
    $tmpcell .= "</a>";
