...
echo '<pre>';
    print_r($category_tmp);
    print_r($levelcounter);
    print_r($levels);
echo '</pre>';
        

for($n = $limitstart ; $n < $nrows ; $n++) {
        if (sizeof($row_list) == 0) {
            $row_list[$n] = $n;
            $depth_list[$n] = $n;
        }
        //$category_tmp[$row_list[$n]] = (!empty($keyword)) ? $category_tmp[$n] : $category_tmp[$row_list[$n]];
	...
	if( !$downCondition || !$upCondition ) {
		...
		if(isset($levelcounter[$category_tmp[$row_list[$n]]["category_parent_id"]]) AND $levelcounter[$category_tmp[$row_list[$n]]["category_parent_id"]] > $levels[$depth_list[$n]+1] )
			$downCondition = true;
	...
	$listObj->addCell( $pageNav->orderUpIcon( $ibg, $upCondition, 'orderup', 'Order Down', $page, 'reorder' )
						. '&nbsp;'
						.$pageNav->orderDownIcon( $ibg, isset($levelcounter[$category_tmp[$row_list[$n]]["category_parent_id"]]) ? $levelcounter[$category_tmp[$row_list[$n]]["category_parent_id"]] : '', $downCondition, 'orderdown', 'Order Down', $page, 'reorder' )
					);
