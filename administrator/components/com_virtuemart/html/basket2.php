...
$full_image = $product_parent_id!=0 && !$db_data_prod->f("product_full_image") ? $db_d->f("product_full_image") : $db_data_prod->f("product_full_image"); // Change
...
$image_thumb_url = IMAGEURL."product/$product_thumb_image";
$img_article ="<img src=\"$image_thumb_url\" height=100 width=85 onclick=\"javascript: window.open('$imageurl', 'win2', 'status=no,toolbar=no,scrollbars=yes,titlebar=no,menubar=no,resizable=yes,width=$width,height=$height,directories=no,location=no');\" border=\"0\" />";
$product_rows[$i]['product_thumb_pic'] = $img_article;
...
/* WEIGHT CALCULATION */
      ...
      <input type=\"image\" name=\"delete\" title=\"". $VM_LANG->_PHPSHOP_CART_DELETE ."\" src=\"". IMAGEURL ."ps_image/delete_f2.gif\" border=\"0\" alt=\"". $VM_LANG->_PHPSHOP_CART_DELETE ."\" />
		...
		//MMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMM
			...
			//========================================= PHPMAGIC=========================================
			    ...
			    $shipping_total = $shipping_total + $_SESSION["deliver_extra_price"];
  ...
  if (!empty($_REQUEST['ship_to_info_id']) || $auth["show_price_including_tax"] == 1) { 
    ...
    if ($weight_total != 0 or TAX_VIRTUAL=='1') {
        ...
        //$tax_total = $ps_checkout->calc_order_tax($total, $vars);
        $tax_total= $total*$tax/100;
    } else {
        $tax_total = 0;
    }
    $shipping_tax=$shipping_total*$tax/100;
 ...
 //MMMMMMMMMMMMMMMMMMMMM
       $order_total += $tax_total-$coupon_display;
		$total_undiscounted += $tax_total;
	}
	//MMMMMMMMMMMMMMMMMMMMMMM
    $order_total_wtax+= $shipping_total + $total;
	$order_total += $shipping_total + $total+$tax_total;
	...
	if (round($_SESSION['minimum_pov'], 2) > 0.00) {
       ...
       // echo $CURRENCY_DISPLAY->getFullValue($order_total)."aaaaaaaaaaaaaaaaaaa";
        if( $show_basket ) {
