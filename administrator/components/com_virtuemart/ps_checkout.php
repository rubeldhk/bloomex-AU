		...
		*/
		...
		if( $db->num_rows() ) {
			...
			while($db->next_record()) {	
				...
				$sResultString	.= "<td>";
	            if ( $nStep == 1 ) {
					$sResultString	.= "<input type='radio' name='$name' value='" . $db->f("user_info_id") . "' title='" . $db->f("country") ."_". $db->f("state") . "' checked='checked' onclick='changDeliver(\"".$row."\");' >";
					$sResultString	.= "<input type='hidden' name='zip_checked' value='" . $row . "' >";
					$sCurrentStateTax	= $db->f("country") ."_". $db->f("state");
				}else {
					$sResultString	.= "<input type='radio' name='$name' value='" . $db->f("user_info_id") . "' title='" . $db->f("country") ."_". $db->f("state") . "' onclick='changDeliver(\"".$row."\");' >";
				}
				...
				$sResultString	.= "<strong>".$db->f("address_type_name")."</strong>(".$db->f("first_name") . " " . $db->f("middle_name") . " " . $db->f("last_name") . " ).&nbsp;
			...
			$sResultString	= "error[--2--]<input type='hidden' name='zip_checked' value='' >";
			return $sResultString;
		}
	}
...
If you are not agreed with below, please cancel your order by sending email to wecare@bloomex.ca indicating your order number.  \n\n";
		...
		elseif (ORDER_MAIL_HTML == '1') {
			...
			//$html = str_replace('{phpShopOrderSubtotal}',$CURRENCY_DISPLAY->getFullValue($sub_total),$html);
                        $html = str_replace('{phpShopOrderSubtotal}',$CURRENCY_DISPLAY->getFullValue($order_total - $order_shipping),$html);
