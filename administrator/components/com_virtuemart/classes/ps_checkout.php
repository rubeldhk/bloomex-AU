...
//		$this->_subtotal = $this->calc_order_subtotal($vars);
	...
	function ship_to_addresses_radio($user_id, $name, $value) {
		...
		*/
                ...
                if ($user_id != 0) {
                    ...
                    $q .= "WHERE  jos_vm_state.country_id='13' and  user_id = '" . $user_id . "' ";
                    $q .= "AND address_type = 'ST' ";
                    $q .= "ORDER BY cdate LIMIT 0,5";
                    $db->query($q);

                    $sResultString	= "";
                    if( $db->num_rows() ) {
                            $sResultString	= "success[--2--]";
                            $sResultString	.= "<table border='0' width='100%' cellpadding='5' cellspacing='0' border='0'>\n";
                            $i = 2;
                            $nStep		= 0;
                            $nCompare	= 1;
                            while($db->next_record()) {	
                                    ...
                                    $query	= "SELECT postal_code FROM jos_postcode_warehouse WHERE `deliverable`=0 AND published=1 AND postal_code = '".trim($db->f("zip"))."'";
                                    ...
                                    $sResultString	.= "<tr class='sectiontableentry$i'>";
                                    $sResultString	.= "<td>";
                                    /*print_r($row);
                                    echo"<br/><br/>";
                                    print_r($aOptions);
                                    echo $nStep ."==". $nCompare;*/
                                    if ( $nStep == $db->num_rows() ) {
                                            $sResultString	.= "<input type='radio' name='$name' value='" . $db->f("user_info_id") . "' title='" . $db->f("country") ."_". $db->f("state") . "' checked='checked' onclick='changDeliver(\"".$row."\",\"".$db->f("country") ."_". $db->f("state")."\", \"".$db->f("zip")."\");' >";
                                            $sResultString	.= "<input type='hidden' name='zip_checked' value='" . $row . "' ><input type='hidden' name='current_state_tax_tmp' value='" . $db->f("country") ."_". $db->f("state") . "' >";
                                            //$sResultZipNow	= '<input type="hidden" id="post_code_zip_now" name="post_code_zip_now" value="'.$db->f("zip").' />';
                                    }else {
                                            $sResultString	.= "<input type='radio' name='$name' value='" . $db->f("user_info_id") . "' title='" . $db->f("country") ."_". $db->f("state") . "' onclick='changDeliver(\"".$row."\",\"".$db->f("country") ."_". $db->f("state")."\", \"".$db->f("zip")."\");' >";
                                    }
                                    $sResultString	.= "</td><td><div>";
                                    ...
                                    $sResultString	.= "<strong>Name:</strong> ".$db->f("first_name"). " " .$db->f("middle_name"). " " .$db->f("last_name")."<br/>
                                                    <b data-b='1'>".$VM_LANG->_VM_ADDRESS.":</b> ".$sAddress;

                                    $sResultString	.= "<div></td>
                                                    <td width='10%'><input class='new_checkout_register_button edit-deliver' style='width:85px !important;' onclick='editDeliver(\"".$db->f("user_info_id")."\",\"".$db->f("user_id")."\");' value='".$VM_LANG->_VM_EDIT."' type='button'></td>
                                                    <td width='10%'><input class='new_checkout_register_button delete-deliver' style='width:85px !important;' onclick='deleteDeliver(\"".$db->f("user_info_id")."\",\"".$db->f("user_id")."\");' value='".$VM_LANG->_VM_DELETE."' type='button'></td>
                                            </tr>";
                                    if($i == 1) $i++;
                                    elseif($i == 2) $i--;
                            }
                            $sResultStringDefault='';
                            if( $nCompare > $nStep ) {
                                    $bIsValidZipCode 		= 0;
                                    $sResultStringDefault	= "<input type='hidden' name='zip_checked' value='' ><input type='hidden' name='current_state_tax_tmp' value='' >";
                            }else {
                                    $bIsValidZipCode = 1;
                            }					


                            $sResultString	.= "</table>".$sResultStringDefault."[--2--]$bIsValidZipCode";			
                            return $sResultString;
                    }else  {
                            $sResultString	= "error[--2--]<input type='hidden' name='zip_checked' value='' >[--2--]none";
                            return $sResultString;
                    }
                }
                else  {
	...
	***************************************************************************/
    ...
    ////MMMMMMMMMMMMMMMMMMMMMMMMM
     ...
     $order_taxable -= floatval( $payment_discount_untaxed );
		...
		if( $this->_SHIPPING ) {			
			/* sets _shipping */
			$order_shipping_temp 		= round( $this->calc_order_shipping( $d ), 2 );
			$d['order_shipping_tax'] 	= $order_shipping_tax = round($order_shipping_temp*$taxrate , 2 );
			
			if( $_SESSION["deliver_extra_price"] ) {
				$d['order_shipping'] = $order_shipping =  floatval($order_shipping_temp) + floatval($_SESSION["deliver_extra_price"]) + $order_shipping_tax;
			}else {
				$d['order_shipping'] = $order_shipping	= $order_shipping_temp + $order_shipping_tax;
			}
		}
	...
	//	}
        ...
        $d['order_total'] = $order_total = $order_subtotal + $order_tax + $order_shipping + /*$order_shipping_tax - */$coupon_discount - $payment_discount;
		//$d['order_total'] = $order_total =$tmp_subtotal+$order_tax+$order_shipping+$order_shipping_tax-$coupon_discount-$payment_discount;
		...
		if ($d['customer_occasion']=="") $d['customer_occasion']='00000'; 
		...
		$q .= "mdate,customer_note,customer_signature,ddate,customer_occasion,customer_comments,find_us,ip_address,username) \n";
		$q .= "VALUES (";
		$q .= "'" . $auth["user_id"] . "', ";
		$q .= $ps_vendor_id . ", ";
		$q .= "'" . $order_number . "', '";
        ...
        $q .= htmlspecialchars(strip_tags(addslashes($d['customer_signature']))) . "', '";
        $q .= strip_tags($d['delivery_day'])."-".strip_tags($d['delivery_month'])."-".date('Y',time())."', '"; 
        $q .= htmlspecialchars(strip_tags(addslashes($d['customer_occasion']))). "', '"; 
        $q .= htmlspecialchars(strip_tags(addslashes($d['customer_comments']))). "', '"; 
        $q .= htmlspecialchars(strip_tags(addslashes($d['find_us']))). "', '"; 
	    ...
	    */
		...
		//$q .= "'" . $d["order_payment_code"] . "', "; //Prevent CVV from storing to DB
		$q .= "'', ";
    ...
    */
		...
		for($i = 0; $i < $cart["idx"]; $i++) {
			...
			$t = "SELECT * ";
			$t .= "FROM #__{vm}_product_product_type_xref,#__{vm}_product_type WHERE ";
                        $t .= "#__{vm}_product_product_type_xref.product_id='".$cart[$i]["product_id"]."' ";
                        $t .= "AND #__{vm}_product_product_type_xref.product_type_id=#__{vm}_product_type.product_type_id";
			$dbtype1->query($t);

               		while( $dbtype1->next_record() ) {

			$product_type_id = $dbtype1->f("product_type_id");
                        $dbtype2 = new ps_DB;

			$t = "SELECT * ";
			$t .= "FROM #__{vm}_product_type_$product_type_id WHERE ";
                        $t .= "product_id='".$cart[$i]["product_id"]."' ";
			$dbtype2->query($t);
			$dbtype2->next_record();


        		 $dbtype = new ps_DB;
			$q = "INSERT INTO #__{vm}_order_product_type ";
			$q .= "(order_id, product_id, product_type_name, quantity, price) ";
			$q .= "VALUES ('";
        		$q .= $order_id . "', '";
			$q .= $cart[$i]["product_id"] . "', '";
	                $q .= addslashes($dbtype1->f("product_type_name")) . "', ";
	                $q .= $dbtype2->f("quantity") . ", ";
	                $q .= $dbtype2->f("price") . ")";
                        $dbtype->query($q);
			$dbtype->next_record();   
			}
...
################## BEGIN GOOGLE CONVERSION CODE ######MRE 27-06-06###############################################
...
echo   '<!-- Google Code for Purchase Conversion Page -->
			...
			<!--
			var google_conversion_id = "UA-2790740";
			...
			<noscript>
				<img height=1 width=1 border=0 src="https://www.googleadservices.com/pagead/conversion/UA-2790740/?value='.$order_total.'&label=Purchase&script=0">
			</noscript>';
			...
			if (MULTIPLE_TAXRATES_ENABLE != '1' ) {
				...
				if( $this->_SHIPPING ) {
					...
					if( $taxrate ) {
						...
						if( $auth["show_price_including_tax"] == 1 ) {
							/*echo $order_tax_details[$taxrate]."===";
							@$order_tax_details[$taxrate] += $rate - ($rate / ($taxrate+1));							
							echo $rate."===".$taxrate."===".($rate / ($taxrate+1))."===";*/
							/*print_r($order_tax_details);
							die("aaaaaaaaaaa");*/
						}
						else {
	...
	function email_receipt($order_id) {
		...
		if( !empty( $_SESSION['ccdata']['order_payment_name'] )
			&& !empty($_SESSION['ccdata']['order_payment_number'])) {
	  		...
	  		$payment_info_details .= 'adasdfasfasfdasdfasdf';
		...
		$order_status = $dbos->f("order_status_name");
	        $shopper_message .= "Delivery Date: " . $db->f("ddate") . "\n";
...
$shopper_message .= $VM_LANG->_PHPSHOP_CHECKOUT_CUSTOMER_SIGNATURE.":\ " . $db->f("customer_signature") . "\n";
       $shopper_message .= " Special Instructions: " . $db->f("customer_comments") . "\n";
        $shopper_message .= "\n\n------------------------------------------------------------------------\n\n";	
	$shopper_message .= "Please note, that below listed policy is the same for everyone. By receiving this email you confirm, that you have agreed to our terms and conditions as well as to refund and exchange policy. 
If you are not agreed with below, please cancel your order by sending email to wecare@bloomex.com.au indicating your order number.  \n\n";
		...
		elseif (ORDER_MAIL_HTML == '1') {
			...
			while($dboi->next_record()) {
				...
				$order_items .= "<td>".stripslashes($dboi->f("product_name"));
	 ...
	 */
	static function asterisk_pad($str, $display_length, $reversed = false) {
	...
	**************************************************************************/
	 ...
	 * @return string
	 */
	static function show_tax_details( $details ) {
