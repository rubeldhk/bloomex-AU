...
class ps_reviews {
    ...
    function product_reviews($product_id, $limit = 0) {
        ...
        if (PSHOP_ALLOW_REVIEWS == "1") {
            ...
            while ($dbc->next_record()) {
                ...
                $db->next_record();
                $html .= "<div id='new-main-banner-testomonials' style='text-indent: 0px; padding: 0px;'>" . $db->f("name") . "&nbsp;&nbsp;(" . strftime(_DATE_FORMAT_LC, $dbc->f("time")) . ")</div><br />";
                $html .= $VM_LANG->_PHPSHOP_RATE_NOM . ": <img src=\"" . IMAGEURL . "stars/" . $dbc->f("user_rating") . ".gif\" border=\"0\" alt=\"" . $dbc->f("user_rating") . "\" />";
                $html .= "<br /><p class='testimonialDescription'>" . $dbc->f("comment") . "</p>";
            }
            if ($num_rows < 1) {
    ...
    function reviewform($product_id) {
      ...
      //]]></script>
            ...
            <textarea title=\"" . $VM_LANG->_PHPSHOP_REVIEW_COMMENT . "\" class=\"inputboxRev\" id=\"comment\" onblur=\"refresh_counter();\" onfocus=\"refresh_counter();\" onkeypress=\"refresh_counter();\" name=\"comment\" rows=\"10\" cols=\"55\" maxlength=\"2000\"></textarea>
            <br />
            <input class=\"new_checkout_upd_address_button\" type=\"submit\" onclick=\"return( check_reviewform());\" name=\"submit_review\" title=\"" . $VM_LANG->_PHPSHOP_REVIEW_SUBMIT . "\" value=\"" . $VM_LANG->_PHPSHOP_REVIEW_SUBMIT . "\" />
            
            <!-- <div>" . $VM_LANG->_PHPSHOP_REVIEW_COUNT . " -->
            <input type=\"hidden\" value=\"0\" size=\"4\" class=\"inputboxRev\" name=\"counter\" maxlength=\"4\" readonly=\"readonly\" />
            </div>
    ...
    function process_review(&$d) {
        ...
        if (PSHOP_ALLOW_REVIEWS == "1" && !empty($my->id) && ($d["user_rating"] > 2)) {
