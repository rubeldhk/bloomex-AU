...
class ps_order {
    ...
    function order_status_update(&$d) {
        global $mosConfig_offset,$mosConfig_adm_link,$mosConfig_adm_auth,$my,$mosConfig_groups_can_see_hidden_comments,$database;
        $db = new ps_DB;
        $timestamp = time();
        ...
        $curr_order_status = !empty($d["current_order_status"]) ? $d["current_order_status"] : "";

        //MMMMMMMMMMMMMMMMMMMMMM
        $curr_warehouse = @$d["current_warehouse"];
        $curr_priority = @$d["current_priority"];
        $notify_customer = empty($d['notify_customer']) ? "N" : $d['notify_customer'];
        $notify_security = empty($d['notify_security']) ? "N" : $d['notify_security'];
        $notify_warehouse = empty($d['notify_warehouse']) ? "N" : $d['notify_warehouse'];
        $notify_recipient = empty($d['notify_recipient']) ? "N" : $d['notify_recipient'];
        ...
        if($d["order_status"] == 'hc' && !$checkUserCanSeeHiddenComments){
            $hiddenStatus=true;
        }

        $queryPartner= "SELECT id from jos_vm_api2_orders  WHERE order_id = ". $d['order_id'];
        $database->setQuery($queryPartner);
        $checkOrderFromPartner	= $database->loadResult();

        if ($notify_customer == "Y" && $d["order_status"] != "O" && !$hiddenStatus) {
            $notify_customer = 1;
        } else {
            $notify_customer = 0;
        }

        if ($notify_security === "Y") {
            $notify_security = 1;
        } else {
            $notify_security = 0;
        }

        if ($notify_warehouse == "Y" && !$hiddenStatus) {
            $notify_warehouse = 1;
        } else {
            $notify_warehouse = 0;
        }
        if ($notify_recipient == "Y" && !$hiddenStatus) {
            $notify_recipient = 1;
        } else {
            $notify_recipient = 0;
        }
        if ($d["warehouse"] != $d["current_warehouse"]) {

            $q = "SELECT * FROM #__{vm}_warehouse WHERE";
            $q .= " warehouse_code = '".$d["current_warehouse"]."'";
            $db->query($q);
            $db->next_record();
            $current_warehouse = $db->f("warehouse_name")??'No Warehouse Assigned';

            $q = "SELECT * FROM #__{vm}_warehouse WHERE";
            $q .= " warehouse_code = '".$d["warehouse"]."'";
            $db->query($q);
            $db->next_record();
            $new_warehouse = $db->f("warehouse_name")??'No Warehouse Assigned';

            $d['order_comment'] = 'Change Order Warehouse from <strong>'.$current_warehouse.'</strong> to <strong>'.$new_warehouse.'</strong>. ' . $d['order_comment'];
        }
        ...
        if ($d["order_status"] == "T") {
            ...
            $q = "SELECT partner_name, partner_email, partner_phone FROM tbl_local_parthners ";
            $q .= "WHERE partner_id = '" . $d["current_partner"] . "' ";
            $partner = false;
            $database->setQuery($q);
            $database->loadObject($partner);


            $key = md5($db->loadResult() . $d["order_id"]);

            $d['order_comment'] = 'Send to partner: <b>' . $partner->partner_name . '</b>. <br>
             Partner Email: <b>' . $partner->partner_email . '</b>. <br>
             Offer summ: <b>' . $d["partner_price"] .'</b>';

            $notify_partner = $this->notify_partner($d, $partner, $key);

            if ($notify_partner != true) {
                echo 'We could not send email this partner';
                exit(0);
            }


            $q = "INSERT INTO tbl_local_parthners_orders (order_id, partner_id, confirm, price, `key`)";
            $q .= " VALUES (";
            $q .= "" . $d["order_id"] . ", " . $d["current_partner"] . ", 0,  '" . $d["partner_price"] . "', '" . $key . "') 
                ON DUPLICATE KEY UPDATE partner_id=" . $d["current_partner"] . ",confirm=0 , price= '" . $d["partner_price"] . "', `key`='" . $key . "'";
            //$db->setQuery($q);
            $db->query($q);
        ...
        if (/* $curr_order_status == "T" && */$d["order_status"] == "L") {
            $q = "UPDATE tbl_local_parthners_orders set confirm=-1, mtime=NULL where order_id=" . $d["order_id"] . " and  partner_id=" . $d["current_partner"];
            $db->query($q);
        }
        if ($curr_order_status == 'T' || $d["order_status"] == "T" || $d["order_status"] == "L" || $d["order_status"] == "L") {
            $comments = $db->getEscaped($d['order_comment'], ENT_QUOTES);
            $order_status_name = ($d["order_status"] == 'Т') ? 'Partner' : 'Cancel Partner';
            $q = "INSERT INTO 	tbl_local_parthners_orders_history (order_id, partner_id, status, price,comments, time)";
            $q .= " VALUES ('" . $d["order_id"] . "', " . $d["current_partner"] . ", '" . $order_status_name . "',  '" . $d["partner_price"] . "','" . $comments . "', NOW() ) ";
            $db->query($q);
        }
        $q = "UPDATE #__{vm}_orders SET";

        $q .= " order_status='" . $d["order_status"] . "' ";
        $q .= ", warehouse='" . $d["warehouse"] . "' ";
        if (isset($d["color"])) {
            $q .= ", color='" . $d["color"] . "' ";
        }
        $q .= ", priority='" . $d["priority"] . "' ";
        $q .= ", mdate='" . $timestamp . "' ";
        $q .= ", username='" . $_SESSION['session_username'] . "' ";
        $q .= "WHERE order_id='" . $d["order_id"] . "'";
        $db->query($q);

        $notify_iris_inside = mosGetParam($_REQUEST, "notify_iris_inside", "");
        $notify_iris_val = !empty($notify_iris_inside) ? 1 : 0;

        // Update the Order History.
        $q = "INSERT INTO #__{vm}_order_history ";
        $q .= "(order_id,order_status_code,warehouse,priority,date_added,customer_notified,warehouse_notified,security_notified,recipient_notified,comments,user_name) VALUES (";
        $q .= "'" . $d["order_id"] . "', '" . $d["order_status"] . "', '" . $d["warehouse"] . "','" . $d["priority"] . "', '$mysqlDatetime', '$notify_customer', '$notify_warehouse','$notify_security','$notify_recipient',  '" . $db->getEscaped($d['order_comment'], ENT_QUOTES) . "', '" . $d["user_name"] . "')";
        $db->query($q);
        if ($db->error) {
            die($db->error);
        }
        ...
        if ($notify_customer == "1") {
            $this->notify_customer($d,$checkOrderFromPartner);
        }

        if ($notify_security) {
            $this->notify_security($d);
        }

        if ($notify_warehouse == "1") {
            $this->notify_warehouse($d);
        }
        if ($notify_recipient == "1") {
            $this->notify_recipient($d);
        }

        if (intval($_REQUEST["ajax_action"])) {
            global $database;
            $iris_confirm = mosGetParam($_REQUEST, "iris_confirm", "");
            $iris_confirm = !empty($iris_confirm) ? $iris_confirm : "";


            if ($iris_confirm == "success") {
                $query = "INSERT INTO #__vm_order_history (order_id, order_status_code, date_added, customer_notified, warehouse_notified, comments, user_name)  
					  	   VALUES (" . $d['order_id'] . ", 'C', '" . date("Y-m-d H:i:s", time()) . "', 0, 0, 'The order infomartion was sent successful to IRIS'" . "', '" . $d["order_status"] . "' )";
                $database->setQuery($query);
                $database->query();
            }
            ...
            $notify_iris_inside = mosGetParam($_REQUEST, "notify_iris_inside", "");
            $notify_iris_inside = !empty($notify_iris_inside) ? $notify_iris_inside : "";
            $bIrisExist = true;
            if ($notify_iris_inside == "Y") {
                $query = "SELECT warehouse FROM #__vm_orders WHERE order_id = " . $d['order_id'];
                $database->setQuery($query);
                $warehouse = $database->loadResult();

                if (!empty($warehouse) && $warehouse == 'IRIS') {
                    $query = "SELECT FloristOrderNo FROM iris_order_tracking WHERE PartnerReference = " . $d['order_id'];
                    $database->setQuery($query);
                    $FloristOrderNo = $database->loadResult();

                    if (!empty($FloristOrderNo)) {
                        $url = "http://irisadvantage.com/XMLDataFeed/IssueResponse.aspx";

                        $post_string = '<?xml version="1.0" ?>
									<IssueResponse>
									<AccountID>55699</AccountID>
									<FloristOrderNo>' . $FloristOrderNo . '</FloristOrderNo>
									<PartnerReference>' . $d['order_id'] . '</PartnerReference>
									<IssueRespSubject>There was an issue</IssueRespSubject>
									<IssueRespDetails><![CDATA[' . $d['order_comment'] . ']]></IssueRespDetails>
									<IssueRespDateTime>' . date("m/d/Y H:i:s A", $timestamp) . ' CST</IssueRespDateTime>
									</IssueResponse>';
            ...
            if ($notify_iris_inside == "Y" && !$bIrisExist) {
                echo "iris_none";
            } else {
                echo (isset($d['rel']) ? $d['rel'] : '') . "-success";
            }
            exit(0);
        }
    ...
    private function notify_security(array $d)
    {
        global $mosConfig_securityEmail,$mosConfig_mailfrom, $mosConfig_fromname;
        if(!isset($mosConfig_securityEmail)){
            return;
        }
        require_once CLASSPATH . 'ps_comemails.php';

        $ps_comemails = new ps_comemails();
        $emailObj = $ps_comemails->getEmailTextByType(13, 3);

        if (!empty($d['include_comment']) && !empty($d['order_comment'])) {
            $emailObj->email_html = str_replace('{UpdateStatusComment}', strip_tags($d['order_comment']), $emailObj->email_html);
        } else {
            $emailObj->email_html = str_replace('{UpdateStatusComment}', '', $emailObj->email_html);
        }
        vmMail($mosConfig_mailfrom, $mosConfig_fromname, $mosConfig_securityEmail, $ps_comemails->setVariables((int) $d["order_id"], $emailObj->email_subject), $ps_comemails->setVariables((int) $d["order_id"], $emailObj->email_html), '', true, null, null);
    }

    /*     * ************************************************************************
     * name: notify_recipient
     * created by: YI
     * description: notifies the recipient that about any changes
     * parameters: $d
     * returns: true
     * ************************************************************************ */
    function notify_recipient(&$d) {

        global $mosConfig_live_site, $mosConfig_absolute_path, $sess, $mosConfig_mailfrom, $mosConfig_fromname,
        ...
        if (isset($_FILES['images'])) {
            ...
            foreach ($images as $image) {
                $EmbeddedImages[] = array('path' => $image['tmp_name'],
                    'name' => $image['name'],
                    'filename' => $image['name'],
                    'encoding' => "base64",
                    'mimetype' => $image['type']);
        ...
        WHERE `order_id`=" . (int) $d["order_id"] . " AND `address_type`='ST'";
        ...
        vmMail($mosConfig_mailfrom, $mosConfig_fromname, $user_info_obj->user_email, $ps_comemails->setVariables((int) $d["order_id"], $confirmation_obj->email_subject), $ps_comemails->setVariables((int) $d["order_id"], $confirmation_obj->email_html), '', true, null, null, $EmbeddedImages);
        //!NEW CONFIRMATION
    }
    /*     * ************************************************************************
    ...
    function notify_customer(&$d,$checkOrderFromPartner=false) {
        global $mosConfig_live_site, $mosConfig_absolute_path, $sess, $mosConfig_mailfrom, $mosConfig_fromname,
        $VM_LANG, $mosConfig_smtpauth, $mosConfig_mailer, $vmLogger,
        $mosConfig_smtpuser, $mosConfig_smtppass, $mosConfig_smtphost, $database;
        ...
        try {
            require_once CLASSPATH . 'ps_comsms.php';
            $ps_comsms = new ps_comsms;
            $ps_comsms->send($d['order_id'], $d['order_status']);
        }catch (Exception $e) {}
        ...
        $query = "SELECT `email_subject`, `email_html` FROM `jos_vm_emails` WHERE `email_type`='2' AND `recipient_type`='1' AND `order_status_code`='" . $database->getEscaped($d["order_status"]) . "' $addIntoWhere ";
        $confirmation_obj = false;
        $database->setQuery($query);
        $database->loadObject($confirmation_obj);

        if (!$confirmation_obj) {
            $query = "SELECT `email_subject`, `email_html` FROM `jos_vm_emails` WHERE `email_type`='2' AND `recipient_type`='1' $addIntoWhere";
            $confirmation_obj = false;
            $database->setQuery($query);
            $database->loadObject($confirmation_obj);
        }
        if ($confirmation_obj) {
            ...
            if (!empty($d['include_comment']) && !empty($d['order_comment'])) {
                $confirmation_obj->email_html = str_replace('{UpdateStatusComment}', $d['order_status'] === 'hc' ? 'You are not allowed to see this comment.' : htmlspecialchars($d['order_comment']), $confirmation_obj->email_html);
            } else {
                $confirmation_obj->email_html = str_replace('{UpdateStatusComment}', '', $confirmation_obj->email_html);
            }
    ...
    function notify_partner(&$d,$partner, $key = '') {
        global $mosConfig_live_site, $mosConfig_absolute_path, $sess,$mosConfig_fromname,
               $database,  $mosConfig_mailfrom, $mosConfig_limit_sms_sender_AccountKey;



        $q = "SELECT OC.order_occasion_name FROM jos_vm_orders AS O "
            . "LEFT JOIN jos_vm_order_occasion AS OC  ON O.customer_occasion = OC.order_occasion_code "
            . "WHERE O.order_id='" . $d["order_id"] . "'";
        $occasion = false;
        $database->setQuery($q);
        $database->loadObject($occasion);

        $query = "SELECT `email_subject`, `email_html` FROM `jos_vm_emails` WHERE `email_type`='14' AND `recipient_type`='4' AND `order_status_code`='T'  ";
        $confirmation_obj = false;
        $database->setQuery($query);
        $database->loadObject($confirmation_obj);
        ...
        if ($confirmation_obj) {
            ...
            $html = str_replace('{lifesite}', $mosConfig_live_site, $confirmation_obj->email_html);
            $html = str_replace('{Price}', date($d["partner_price"]), $html);
            $html = str_replace('{Key}', $key, $html);
            $html = str_replace('{Occasion}', nl2br($occasion->order_occasion_name??''), $html);

           $emailSent = vmMail($mosConfig_mailfrom, $mosConfig_fromname, $partner->partner_email, $ps_comemails->setVariables((int)$d["order_id"], $confirmation_obj->email_subject), $ps_comemails->setVariables((int)$d["order_id"], $html), '', true, null, null);

           if(!$emailSent)
               return false;

            $phone = formatMobileNumber($partner->partner_phone);
            if ($phone != false) {
                $client = new SoapClient('http://smsgateway.ca/SendSMS.asmx?WSDL');
                $parameters = new SMSParams;
                $parameters->CellNumber = $phone;
                $parameters->AccountKey = $mosConfig_limit_sms_sender_AccountKey;
                $parameters->MessageBody = 'You got an order from Bloomex. Please check your email for details. Thank you for your great work!';
    ...
    function notify_warehouse(&$d) {
        ...
        $url = $mosConfig_live_site . "/order-details/?order_id=" . $d["order_id"] . "&Itemid=$Itemid";
        $EmbeddedImages=[];
        if ($_FILES['images']) {
            $images = $this->reArrayFiles($_FILES['images']);
            foreach ($images as $image) {
        ...
        $q = "SELECT warehouse_name,warehouse_email,order_status_name FROM #__{vm}_warehouse,#__{vm}_priority,#__{vm}_orders,#__{vm}_order_status ";
        $q .= "WHERE #__{vm}_orders.order_id = '" . $d["order_id"] . "' ";
        $q .= "AND #__{vm}_orders.warehouse = #__{vm}_warehouse.warehouse_code ";
        $q .= "AND #__{vm}_orders.order_status = #__{vm}_order_status.order_status_code ";
        $db->query($q);
        $db->next_record();

        /* MAIL BODY */
        $message = _HI . " WAREHOUSE " . $db->f("warehouse_name") . ",\n\n";
        ...
        $message .= "<b>" . $db->f("order_status_name") . "</b>\n";
        ...
        $result = vmMail($mosConfig_mailfrom, $mosConfig_fromname, $db->f("warehouse_email"), $mail_Subject, $mail_Body, '', true, null, null, $EmbeddedImages);

        /* Send the email */
        if ($result) {
            $vmLogger->info($VM_LANG->_PHPSHOP_DOWNLOADS_SEND_MSG . " WAREHOUSE " . $db->f("warehouse_name") . ", " . $db->f("warehouse_email"));
        } else {
            $vmLogger->warning($VM_LANG->_PHPSHOP_DOWNLOADS_ERR_SEND . ' WAREHOUSE ' . $db->f("warehouse_name") . ", " . $db->f("warehouse_email") . " (" . $result->ErrorInfo . ")");
        }
    }
    ...
    function list_order($order_status = 'A', $secure = 0) {
        global $VM_LANG, $CURRENCY_DISPLAY, $sess,$sef, $limit, $limitstart, $keyword, $mm_action_url, $my, $mosConfig_live_site;
...
//                $listObj->writeSearchHeader('', '', 'account', 'index');
            ...
            while ($db->next_record()) {
                ...
                $tmp_cell = "<a href=\"/" . "account/order-details/" . $db->f("order_id") . "/\">\n";
                ...
                $tmp_cell .= "<br /><strong>" . $VM_LANG->_PHPSHOP_ORDER_PRINT_TOTAL . ":</strong> " . LangNumberFormat::number_format($db->f("order_total"), 2); //$CURRENCY_DISPLAY->getFullValue($db->f("order_total"));
                $listObj->addCell($tmp_cell);
                ...
                $tmp_cell = '<a target="_blank" class="float-end account_button" href="' . $mosConfig_live_site . '/order-tax-invoice/' . $db->f("order_id") . '/">';
                $tmp_cell .= "<input type='button' class='btn btn-primary  account_button' value='" . $VM_LANG->_PHPSHOP_ORDER_PRINT_TAX_INVOICE . "'></a>";
                $listObj->addCell($tmp_cell);
            }
            $listObj->writeTable();
            $listObj->endTable();
    ...
    function delete_record($record_id, &$d) {
        ...
        if ($this->validate_delete($record_id)) {
            ...
            $q = "DELETE from #__{vm}_vm_order_product_type  where order_id='$record_id'";
            $db->query($q);
            $db->next_record();
