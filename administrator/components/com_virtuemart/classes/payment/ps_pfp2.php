...
class ps_pfp2 {
	...
	var $payment_code = "PFP2";
	var $classname = "ps_pfp2";
	...
	?>
	  <table>
	    <tr>
	      ...
	      <td>
                <select name="PFP2_TEST_REQUEST" class="inputbox" >
                <option <?php if (PFP2_TEST_REQUEST == 'TRUE') echo "selected=\"selected\""; ?> value="TRUE"><?php echo $VM_LANG->_PHPSHOP_ADMIN_CFG_YES ?></option>
                <option <?php if (PFP2_TEST_REQUEST == 'FALSE') echo "selected=\"selected\""; ?> value="FALSE"><?php echo $VM_LANG->_PHPSHOP_ADMIN_CFG_NO ?></option>
                </select>
	      </td>
	    ...
	    <tr>
	      ...
	      <td>
                <input type="text" name="PFP2_PARTNER" class="inputbox" value="<?php echo PFP2_PARTNER; ?>" />
	      </td>
	      <td><?php echo $VM_LANG->_PHPSHOP_ADMIN_CFG_PFP_PARTNER_EXPLAIN ?></td>
	    </tr>
	    <tr>
	      <td><strong><?php echo $VM_LANG->_PHPSHOP_ADMIN_CFG_PFP_USERNAME ?>(Vendor)</strong></td>
	      <td>
                <input type="text" name="PFP2_LOGIN" class="inputbox" value="<?php echo PFP2_LOGIN; ?>" />
	      </td>
	      <td><?php echo $VM_LANG->_PHPSHOP_ADMIN_CFG_PFP_USERNAME_EXPLAIN ?></td>
	    </tr>
		  <tr>
	      <td><strong><?php echo $VM_LANG->_PHPSHOP_ADMIN_CFG_PFP_USERNAME ?>(User)</strong></td>
	      <td>
                <input type="text" name="PFP2_USER" class="inputbox" value="<?php echo PFP2_USER; ?>" />
	      </td>
	      <td><?php echo $VM_LANG->_PHPSHOP_ADMIN_CFG_PFP_USERNAME_EXPLAIN ?></td>
	    </tr>
	    <tr>
	    ...
	    <tr>
	      ...
	      <td>
                <select name="PFP2_CHECK_CARD_CODE" class="inputbox">
                <option <?php if (PFP2_CHECK_CARD_CODE == 'YES') echo "selected=\"selected\""; ?> value="YES">
                <?php echo $VM_LANG->_PHPSHOP_ADMIN_CFG_YES ?></option>
                <option <?php if (PFP2_CHECK_CARD_CODE == 'NO') echo "selected=\"selected\""; ?> value="NO">
	    ...
	    <tr>
	      ...
	      <td>
		<select name="PFP2_TYPE" class="inputbox">
                <option <?php if (PFP2_TYPE == 'A') echo "selected=\"selected\""; ?> value="A">Authorize</option>
                <option <?php if (PFP2_TYPE == 'S') echo "selected=\"selected\""; ?> value="S">Sale</option>
                </select>
	      </td>
	    ...
	    <tr>
	      ...
	      <td>
                <select name="PFP2_VERIFIED_STATUS" class="inputbox" >
                <?php
                    ...
                    for ($i = 0; $i < sizeof($order_status_code); $i++) {
                      ...
                      if (PFP2_VERIFIED_STATUS == $order_status_code[$i]) 
		...
		<?php echo $VM_LANG->_PHPSHOP_ADMIN_CFG_PFP_VERIFIED_STATUS_EXPLAIN ?>
	      ...
	      <td>
                <select name="PFP2_INVALID_STATUS" class="inputbox" >
                <?php
                    for ($i = 0; $i < sizeof($order_status_code); $i++) {
                      echo "<option value=\"" . $order_status_code[$i];
                      if (PFP2_INVALID_STATUS == $order_status_code[$i]) 
                         echo "\" selected=\"selected\">";
                      else
                         echo "\">";
                      echo $order_status_name[$i] . "</option>\n";
	...
	function write_configuration( &$d ) {
		...
		$my_config_array = array(
		    "PFP2_TEST_REQUEST" => $d['PFP2_TEST_REQUEST'],
		    "PFP2_PARTNER" => $d['PFP2_PARTNER'],
		    "PFP2_LOGIN" => $d['PFP2_LOGIN'],
		    "PFP2_USER" => $d['PFP2_USER'],
		    "PFP2_TYPE" => $d['PFP2_TYPE'],
		    "PFP2_CHECK_CARD_CODE" => $d['PFP2_CHECK_CARD_CODE'],
		    "PFP2_VERIFIED_STATUS" => $d['PFP2_VERIFIED_STATUS'],
		    "PFP2_INVALID_STATUS" => $d['PFP2_INVALID_STATUS'],
		    "PFP2_CERT_PATH" => $d['PFP2_CERT_PATH']);
		$config = "<?php\n";
	...
	function process_payment($order_number, $order_total, &$d) {
		...
		$database->query( "SELECT payment_passkey as passkey " .
...
//speedhack
	...
	function capture_payment( &$d ) {
		...
		if (PFP2_TEST_REQUEST == 'TRUE') {
			...
			$host = 'https://payflowpro.verisign.com/transaction';
			$vmLogger->debug("Using real site: " . $host);
		}
...
//speedhack
	...
	function void_authorization(&$d) {
		...
		/* If a test request, use "https://pilot-payflowpro.verisign.com/transaction" */
		if (PFP2_TEST_REQUEST == 'TRUE') {
			$host = 'https://pilot-payflowpro.verisign.com/transaction';
			$vmLogger->debug("Using test site: " . $host);
		} else {
