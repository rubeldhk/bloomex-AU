...
class payflow_pro {
	...
	*/
	...
	function process_payment($order_number, $order_total, &$d) {
		...
		$database->query( "SELECT payment_passkey as passkey FROM #__{vm}_payment_method WHERE payment_class='".$this->classname."' AND shopper_group_id='".$auth['shopper_group_id']."'" );
	...
	function capture_payment( &$d ) {
		...
		$database->query( "SELECT payment_passkey as passkey FROM #__{vm}_payment_method WHERE payment_class='".$this->classname."'" );
		$transaction = $database->record[0];
		if( empty($transaction->passkey)) {
