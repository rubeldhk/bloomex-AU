...
*************************************************************************/
  ...
  ***************************************************************************/  
   function validate_add($d) {
     ...
     if ($d["discount_type"] == "coupon") {
       if (!$d["coupon"]) {
	       $this->error = "ERROR:  You must enter an coupon code for the Discount.";
	       return False;	
	     }	
     }
  ...
  ***************************************************************************/   
  ...
  function add(&$d) {
    ...
    $q = "INSERT INTO #__{vm}_product_discount (discount_type, coupon_code, amount, is_percent, start_date, end_date)";
    $q .= " VALUES ('";
    $q .= $d["discount_type"] . "','";
    $q .= $d["coupon"] . "','";
    $q .= $d["amount"] . "','";
  ...
  function update(&$d) {
    ...
    $q .= "discount_type='" . $d["discount_type"]."',";
    $q .= "coupon_code='" . $d["coupon"]."',";
    $q .= "amount='" . $d["amount"]."',";
  ...
  function discount_list( $discount_id='' ) {
    ...
    $db = new ps_DB;
    $html = "";
    $db->query( "SELECT * FROM #__{vm}_product_discount WHERE discount_type != 'coupon' OR discount_type IS NULL" );
  ...
  function coupon_discount_list( $coupon_discount = '' ) {  	
    global $VM_LANG, $option;
    $db = new ps_DB;
    $html = "";
    $db->query( "SELECT * FROM #__{vm}_product_discount WHERE discount_type = 'coupon'" );
    
    if($db->num_rows() > 0) {
      $html = "<select name=\"product_coupon_discount\" class=\"inputbox\" >\n";
      ...
      while( $db->next_record() ) {
		...
		if( $coupon_discount == $db->f("coupon_code") ) {
			 $html .= "<option value=\"".$db->f("coupon_code")."\" selected>".$db->f("coupon_code") . " - $amount";
		}else {
			 $html .= "<option value=\"".$db->f("coupon_code")."\">".$db->f("coupon_code") . " - $amount";
		}
        $html .= "</option>\n";
      }
      $html .= "</select>\n";
