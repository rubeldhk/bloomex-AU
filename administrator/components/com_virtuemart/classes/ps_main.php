...
function validate_image(&$d, $field_name, $table_name) {
    ...
    if (isset($d['product_sku'])) {
        if (!is_dir($path . $d['product_sku'])) {
            mkdir($path . $d['product_sku'], 0777);
            $vmLogger->debug('Had to create the directory ' . $path  . $d['product_sku']);
        }
    }
    ...
    $tmp_field_name_action = str_replace("thumb", "full", $field_name);
    $tmp_field_name_action = str_replace("_fr", "", $tmp_field_name_action);
    $tmp_field_name = str_replace("thumb", "full", $field_name);
    $pos_field_name = strpos($field_name, "thumb");
    $fullImageFileInfo = pathinfo(isset($_FILES[$field_name]["name"]) ? $_FILES[$field_name]['name'] : "" );
    //	Class for resizing Thumbnails
    require_once( CLASSPATH . "class.img2thumb.php");
    if (@$d[$tmp_field_name_action . '_action'] == 'auto_resize' && ($pos_field_name === false)) {
        ...
        $ext = '.' . $fullImageFileInfo["extension"];
        $noimgif = "";
        if (is_file($full_file)) {
            ...
            //$to_file_thumb = generation_image_name( $_FILES[$field_name]['name'], $path, $field_name ); 
            $pos_field_name_folder = strpos($field_name, "category");
            $folder_path = ( $pos_field_name_folder === false ) ? "product/" : "category/"; //"product/resized/"
            $to_file_thumb = $d[$tmp_field_name . '_new_name_hidden'];
            $fileout = IMAGEPATH . $folder_path . $to_file_thumb . "_" . PSHOP_IMG_WIDTH . "x" . PSHOP_IMG_HEIGHT . $noimgif . $ext;
            $fileout = IMAGEPATH . $folder_path . $d['product_sku'] . "/" . $to_file_thumb . $noimgif . $ext;
    ...
    if (strpos("thumb", $field_name) !== false ) {
        $image_type = "thumbnail image";
    } elseif (strpos("full", $field_name) !== false) {
    ...
    if (isset($d[$field_name . "_action"]) AND $d[$field_name . "_action"] == "delete") {
        ...
        if (!empty($curr_file)) {
            ...
            if (PSHOP_IMG_RESIZE_ENABLE == "1" && $image_type == "thumbnail image") {
                ...
                $resizedfilename = $pathinfo["dirname"] . "/" . $d['product_sku'] . "/" . $filehash . "." . $pathinfo["extension"];
    ...
    /* If upload fails */ elseif ($orig_file and $temp_file == "none") {
    ...
    // Generate Image Destination File Name
    //$to_file = generation_image_name( $_FILES[$field_name]['name'], $path, $field_name ); //md5(uniqid("VirtueMart"));
    $to_file = $d[$field_name . '_new_name_hidden'];
    /* Check image file format */
    if ($orig_file != "none") {
        if ($to_file == '') {
            $vmLogger->err('Bad File Name.');
            return false;
        }
    ...
    $d["image_commands"][] = "\$ret = copy(\"" . addslashes($convertedImage ?: $temp_file ) . "\", \"" . $path . $to_file . "\");";
...
function include_class($module) {
    ...
    switch ($module) {
        ...
        case "order" :
            ...
            require_once(CLASSPATH . 'ps_warehouse.php' );
            require_once(CLASSPATH . 'ps_priority.php' );
            require_once(CLASSPATH . 'ps_order_occasion.php' );
            ...
            $ps_warehouse = new ps_warehouse;
            $ps_priority = new ps_priority;
            $ps_order_occasion = new ps_order_occasion;
            break;
...
function vmCreateMail($from = '', $fromname = '', $subject = '', $body = '') {
    ...
    global $mosConfig_smtpport, $mosConfig_smtpprotocol;
    ...
    if ($mosConfig_mailer == 'smtp') {
        ...
        $mail->Protocol = $mosConfig_smtpprotocol;
    } else
