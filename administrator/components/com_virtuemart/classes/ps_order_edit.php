...
class ps_order_edit {
	...
	**************************************************************************/
	...
	function pane_content($tab) {
		...
		<table class="adminlist"><tr><th><?php echo($VM_LANG->_PHPSHOP_ORDER_EDIT_EDIT_ORDER); ?></th></tr></table>
		...
		<th width="5%" align="left" colspan="2"><?php echo $VM_LANG->_PHPSHOP_ORDER_EDIT_ACTIONS ?></th>
		</tr>
		<?php
		$dbt = new ps_DB;
		$db = new ps_DB;
		...
		src="<?php echo IMAGEURL ?>ps_image/edit_f2.gif" border="0"  alt="<?php echo $VM_LANG->_PHPSHOP_UPDATE ?>" />
		...
		<form method="post" action="<?php echo $_SERVER['PHP_SELF'] ?>">
		<input type="image" title="<?php echo $VM_LANG->_PHPSHOP_DELETE ?>" 
		src="<?php echo IMAGEURL ?>ps_image/delete_f2.gif" border="0"  alt="<?php echo $VM_LANG->_PHPSHOP_DELETE ?>" />
		...
		if(preg_match('/^standard_shipping/', $db->f('ship_method_id'))) {
		?>
		<tr>
		<td colspan="2"><div align="right"><strong><?php echo $VM_LANG->_PHPSHOP_ORDER_PRINT_SHIPPING ?>: </div></strong></td>
		<td></td>
		</tr>
		...
		while ($dbs->next_record()){
			...
			print " ".($dbs->f('shipping_rate_value'));
		...
		<tr>
		<td colspan="2"><div align="right"><strong><?php echo $VM_LANG->_PHPSHOP_ORDER_PRINT_SHIPPING_TAX ?>: </div></strong></td>
		<td>
		<form method="post" action="<?php echo $_SERVER['PHP_SELF'] ?>">
		...
		?>
		<tr>
		<td colspan="2"><div align="right"><strong><?php echo $VM_LANG->_PHPSHOP_COUPON_DISCOUNT ?>: </div></strong></td>
		<td>
		<form method="post" action="<?php echo $_SERVER['PHP_SELF'] ?>">
		...
		<tr>
		<td colspan="2"><div align="right"><strong><?php echo $VM_LANG->_PHPSHOP_PAYMENT_METHOD_LIST_DISCOUNT ?>: </div></strong></td>
		<td>
		<form method="post" action="<?php echo $_SERVER['PHP_SELF'] ?>">
	...
	function update_standard_shipping() {
		...
		$shipping_method = "standard_shipping|$shipping_carrier|$shipping_name|".round($shipping_total)."|$shipping";
	...
	function delete_product() {
		...
		// On supprime le produit de la commande
		$q = "DELETE FROM #__{vm}_order_product_type  ";
                $q .= "WHERE order_id = '" . $this->order_id . "' ";
		...
		$vmLogger->info( $VM_LANG->_PHPSHOP_ORDER_EDIT_PRODUCT_DELETED );
	}
	...
	function update_quantity() {
		...
		if(!is_numeric($quantity) || $quantity < 1) {
			$vmLogger->err( $VM_LANG->_PHPSHOP_ORDER_EDIT_ERROR_QUANTITY_MUST_BE_HIGHER_THAN_0 );
			return;
		}
		...
		$vmLogger->info( $VM_LANG->_PHPSHOP_ORDER_EDIT_QUANTITY_UPDATED );
	...
	function add_product() {
		...
		// On peux insèrer le produit à la commande
		...
		if ($add_product_validate == 1) {
         ...
         $dbp = new ps_DB;
			...
			// On récupère le prix exact du produit
			...
			$t .= "FROM #__{vm}_product_product_type_xref,#__{vm}_product_type WHERE ";
                        $t .= "#__{vm}_product_product_type_xref.product_id='".$product_id."' ";
			...
			$vmLogger->info( $VM_LANG->_PHPSHOP_ORDER_EDIT_PRODUCT_ADDED );
		}
	...
	function display_form_add_product() {
		...
		$html_entete .= '<br /><br /><table class="adminlist"><tr><th>' . $VM_LANG->_PHPSHOP_ORDER_EDIT_ADD_PRODUCT . '</th></tr></table>';
		...
		<input type="submit" value="' . $VM_LANG->_PHPSHOP_ORDER_EDIT_RETURN_PARENTS . '" />
		...
		$html_entete .= '<th>' . $VM_LANG->_PHPSHOP_ORDER_PRINT_SKU . '</th>';		
		...
		$html_entete .= '<th align="left">Action</th></tr>';
       	        $html_table .= '<td>' . $ps_product_attribute->list_sku_attribute($product_id) . '</td>';
		...
		$html_table .= '<td><input type="submit" value="' . $VM_LANG->_PHPSHOP_ORDER_EDIT_ADD . '" /></td></tr></table>';
	...
	function list_products($product_id) {
		...
		$display .= '<option value="-1">SELECT A PRODUCT</option>';
