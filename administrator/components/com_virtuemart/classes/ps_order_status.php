...
class ps_order_status {
    ...
    function list_order_warehouse($order_status_code, $extra = "") {
        echo $this->getWarehouse($order_status_code, $extra);
    }

    function getOrderStatus($order_status_code, $extra = "") {
        $db = new ps_DB;

        $q = "SELECT order_status_id, order_status_code, order_status_name FROM #__{vm}_order_status where publish='1' ORDER BY list_order";
        $db->query($q);
        $html = "<select name=\"order_status\" class=\"inputbox form-control nopadding\" $extra>\n";
    ...
    function list_order_priority($order_status_code, $extra = "") {
        echo $this->getPriority($order_status_code, $extra);
    }
    ...
    function getPriority($priority_code, $extra = "") {
        ...
        $db->query($q);
        $html = "<select name=\"priority\" class=\"inputbox form-control nopadding\" $extra>\n";
        while ($db->next_record()) {
    ...
    function getWarehouse($id, $extra = "") {
        $db = new ps_DB;
        //$qryval=mysql_fetch_array(mysql_query("select * from jos_vm_orders order by order_id desc"));
        //$id=$qryval[order_warehouse];
        $q = "SELECT id, name from jos_vm_warehouse order by id";
        $db->query($q);
        $html = "<select name=\"select_warehouse\" class=\"inputbox\" $extra>\n";
        while ($db->next_record()) {
            $html .= "<option value=\"" . $db->f("id") . "\"";
            if ($id == $db->f("id"))
                $html .= " selected=\"selected\">";
            else
                $html .= ">";
