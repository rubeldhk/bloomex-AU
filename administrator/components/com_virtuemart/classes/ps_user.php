...
class ps_user {
    ...
    function add(&$d) {
        ...
        // Insert billto
        $q = "INSERT INTO #__{vm}_user_info(user_info_id, user_id, address_type, address_type_name, company, title, last_name, first_name, middle_name, phone_1, phone_2,
			fax, address_1, address_2, district,city, state, country, zip, user_email, extra_field_1, extra_field_2, extra_field_3, extra_field_4, extra_field_5, cdate, mdate, perms,
			bank_account_nr, bank_name, bank_sort_code, bank_iban, bank_account_holder,suite,street_number,street_name, bank_account_type) 
				VALUES (";
...
//		die($q);
        ...
        $vmLogger->info("New User was added successfully.");
    ...
    function update(&$d) {
        global $my, $VM_LANG, $perm, $vmLogger, $database;
        ...
        // Joomla User Information stuff
        $this->saveUser($d);
        $db->query("SELECT user_id FROM #__{vm}_user_info WHERE user_id='" . $d["user_id"] . "' AND address_type='BT' ");
        if ($db->num_rows() < 1) {
            $hash_secret = "VirtueMartIsCool";
            $q = "INSERT INTO #__{vm}_user_info(user_info_id, user_id, address_type, address_type_name, company, title, last_name, first_name, middle_name, phone_1, phone_2,
			fax,  district,city, state, country, zip, user_email, extra_field_1, extra_field_2, extra_field_3, extra_field_4, extra_field_5, cdate, mdate, perms,
			bank_account_nr, bank_name, bank_sort_code, bank_iban, bank_account_holder,suite,street_number,street_name, bank_account_type)
				VALUES (";
            $q .= "'" . md5(uniqid($hash_secret)) . "',";
            $q .= "'" . $d["user_id"] . "',";
...
//		die($q);
        ...
        $db->query("SELECT user_id FROM #__{vm}_shopper_vendor_xref WHERE  user_id='" . $d["user_id"] . "'");
        if ($db->num_rows() < 1) {
            // Insert Shopper -ShopperGroup - Relationship
            $q = "INSERT INTO #__{vm}_shopper_vendor_xref ";
            $q .= "(user_id,vendor_id,shopper_group_id,customer_number) ";
        ...
        //RATE
        $query = "SELECT `rate` FROM `#__vm_users_rating` WHERE `user_id`=" . (int) $d["user_id"] . "";
        $database->setQuery($query);
        $pre_rate = $database->loadResult();

        if ($pre_rate != (int) $d["user_rate"]) {
            $q = "UPDATE `#__{vm}_users_rating` SET `rate`=" . (int) $d["user_rate"] . " WHERE `user_id`=" . (int) $d["user_id"] . "";
            $db->query($q);
            ...
            if ((int) $pre_rate == 1 AND (int) $d["user_rate"] > 1) {
                $query = "SELECT `mask` FROM `jos_vm_user_ccards` WHERE `user_id`=" . (int) $d["user_id"] . "";
                $database->setQuery($query);
        ...
        if ($d['receive'] == '0' AND $d["phone_1"] != '') {
            $q = "INSERT INTO `tbl_not_receive` (`number`,`email`) VALUES ('" . $db->getEscaped($d["phone_1"]) . "','" . $db->getEscaped($d["email"]) . "')";

            $db->query($q);
        } elseif ($d['receive'] == '1') {
            $q = "DELETE FROM `tbl_not_receive` WHERE `number`='" . $db->getEscaped($d["phone_1"]) . "' OR `email`='" . $db->getEscaped($d["email"]) . "'";

            $db->query($q);
        }
        if ($d['corp_stakeholder'] == '1') {
            $q = "INSERT INTO `jos_vm_user_options` (`corp_stakeholder`,`user_id`) VALUES ('" . $d["corp_stakeholder"] . "','" . $d["user_id"] . "')";

            $db->query($q);
        } elseif ($d['corp_stakeholder'] == '0') {
            $q = "DELETE FROM `jos_vm_user_options`  WHERE `user_id`='" . $d["user_id"] . "'";
        ...
        WHERE `uc`.`user_id`=" . (int) $d['user_id'] . "";
        ...
        $credits = floatval($d['user_credits']);
        ...
        $vmLogger->info("User was updated successfully.");
    ...
    function delete(&$d) {
        ...
        $vmLogger->info("User(s) was deleted successfully.");
...
//		die($user_group_id);
        ...
        /* $query = "SELECT id FROM tbl_pass_log WHERE user_id = $user_id";
          $database->setQuery( $query );
          $isExist	= $database->loadResult();

          if( $isExist )  {
          $query = "UPDATE tbl_pass_log SET last_updated = '".date("Y-m-d  H:i:s")."' WHERE user_id = $user_id";
          $database->setQuery( $query );
          $database->query() or die( $database->stderr() );
          }else {
          $query = "INSERT INTO tbl_pass_log(last_updated, user_id) VALUES('".date("Y-m-d  H:i:s")."', $user_id )";
          $database->setQuery( $query );
          $database->query() or die( $database->stderr() );
          } */
