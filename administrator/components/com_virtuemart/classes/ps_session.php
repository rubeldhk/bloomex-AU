...
class ps_session {
    ...
    function initSession() {
        ...
        if (empty($_SESSION) || session_id() == '') {
            ...
            if (!empty($_SESSION) && !empty($_COOKIE[$this->_session_name])) {
                ...
                if (@$_REQUEST['option'] == 'com_virtuemart') {
                    // most modern browsers accept cookies anyway, this crushes seo
                    //   $this->doCookieCheck(); // Introduced to check if the user-agent accepts cookies
                    setcookie('VMCHECK', 'OK', time() + 24 * 3600, '/','',true,true);
                }
    ...
    function doCookieCheck() {
        ...
        if ($doCheck) {
            ...
            if ($mm_action_url[strlen($mm_action_url) - 1] == '/') {
                $app = '';
            } else {
                $app = '/';
            }
            setcookie('VMCHECK', 'OK', time() + 24 * 3600, '/','',true,true);
            mosRedirect($mm_action_url . $app . 'index.php?' . $_SERVER['QUERY_STRING'] . '&vmcchk=1');
        }
    }
