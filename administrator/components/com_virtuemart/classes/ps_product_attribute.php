...
class ps_product_attribute {
	...
	*/
	 ...
	 * @return unknown
	 */
	function list_sku_attribute($product_id) {
		global $mosConfig_secret;
		$db = new ps_DB;

		$q = "SELECT product_id, product_sku from #__{vm}_product WHERE product_id='$product_id'";
		$db->query($q);
		$db->next_record();

		$product_sku_list=$db->f("product_sku");
		if ($product_sku_list) {
			$has_product_sku=1;
			$fields=explode(";",$product_sku_list);
			$html = "";
			foreach($fields as $field)
			{
				$titlevar=str_replace(" ","_",$field);
				$title=ucfirst($field);
				$html .= "<div style=\"width:50%;float:left;text-align:right;margin:3px;\">";
				$html .= "<label for=\"".$titlevar."_field\">$title</label></div>";
			}
		}
   ...
   * @return array $result
   */
	static function cartGetAttributes( &$d ) {
