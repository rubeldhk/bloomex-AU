...
require_once( $mosConfig_absolute_path . '/includes/joomla.php' );
     ...
     $q = "SELECT * FROM tbl_db_connect_data";
     //getAffectedRows
     $database->setQuery($q);
        $database->query();
        ...
        foreach ($result as $row) {
            ...
            $link = mysql_connect( $row['host'],  $row['login'], $password);
            if (!$link) {
                die('Could not connect: ' . mysql_error());
            }

            // Which database?
            if (!mysql_select_db( $row['db_name'] , $link)) {
                echo 'Could not select database';
                exit;
            }
            // Build query
            $sql = "SELECT `order_id`,`user_id` FROM `jos_vm_orders` WHERE `cdate`>(select `last_date_timest` from `tbl_last_loading`) order by `cdate`";
	...
	if( $bTestMode ) { //check test mode 
		$query 			= "INSERT INTO jos_vm_orders( user_id, 
					   	   ...
					   	   VALUES( 	$user_id, 
							   	   	...
							   	   	'".strip_tags($occasion)."', 
							   	   	'".strip_tags($card_comment)."', 
							   	   	'$find_us', 
							   	   	'".strip_tags($sPartnerName)."' )";
                 $database->setQuery($query);
                     $database->query(); 
		...
		foreach ($rows as $row) { 
			...
			foreach ($rows2 as $row2) { 
				...
				$query = "INSERT INTO jos_vm_order_product_type( order_id, 
							...
							VALUES ( {$order_id}, 
									 ".$row['product_id']."', 
									 '" . addslashes($item2[0]['product_type_name']) . "', 
									 " . $item2[0]['quantity'] . ", 
									 ".$item2[0]['price']. ")";
	           	$db->insert_sql($query);
