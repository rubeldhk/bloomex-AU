 ...
 */
function loadfunc($wf='',$dfrom='',$dto='',$norder='',$ordersrc=''){
     ...
     foreach($files as $file){
         if(($file!='.')&&($file!='..')){
       $params_connect_db[$file]=array();  
    foreach(file($path."/".$file) as $line)
    {
        list($key, $val) = explode("=", $line);
        $params_connect_db[$file][$key] = $val;     
    }
         }
     }
     ...
     $countOrd=0;
     if($wf!=''){
         $q="update tbl_last_load AS a set `block`=1,  `ident_uniq`='".$ident_uniq."' where `block`=0 and exists (select b.id from tbl_db_connect_data AS b where (b.`interval`*60+a.last_time)<UNIX_TIMESTAMP(NOW()) AND a.db_id=b.id )  ";
         $database->setQuery($q);
         $database->query();
         $rr=$database->getAffectedRows();
         if($rr>0){
           $start_load=true;  
         }
      ...
      if($ordersrc!=''){
         $q="update tbl_last_load AS a set block=1,  `ident_uniq`='".$ident_uniq."' where block=0 and exists (select b.id from tbl_db_connect_data AS b where b.id=".$ordersrc." and a.db_id=b.id ) ";
         $database->setQuery($q);
         $database->query();
         $rr=$database->getAffectedRows();
         if($rr>0){
     ...
     if($wf==''&& $ordersrc==''){
          $q="update tbl_last_load AS a set block=1 ,  `ident_uniq`='".$ident_uniq."' where block=0   ";
         $database->setQuery($q);
         $database->query();
         $rr=$database->getAffectedRows();
         if($rr>0){
     ...
     $timestamp 			= time() + ($mosConfig_offset*60*60);
           $q = "SELECT * FROM tbl_db_connect_data where id in (select db_id from tbl_last_load where block=1 and `ident_uniq`='".$ident_uniq."' )";
        $database->setQuery($q);
        $database->query();
        ...
        foreach ($result as $row) {
            $host=trim($params_connect_db[$row['title']]['host']);
            $login=trim($params_connect_db[$row['title']]['login']);
            $db_name=trim($params_connect_db[$row['title']]['db_name']);
            $password=trim($params_connect_db[$row['title']]['password']);//encode( rtrim($row['password']), $key);
            ...
            if($dfrom!='' || $dto!=''){
                if($dfrom!=''){
                    $cond1="`cdate`> UNIX_TIMESTAMP( '".$dfrom."')";
                }
                if($dto!=''){
                    $cond2="`cdate`< UNIX_TIMESTAMP( '".$dto."')";
                }
                $and=($cond1!='' && $cond2!='')? " AND ":" ";
              $sql = "SELECT `order_id`,`user_id` FROM `jos_vm_orders` WHERE ".$cond1." ".$and." ".$cond2." order by ddate";
            }
             if($norder!='' ){   
              $sql = "SELECT `order_id`,`user_id` FROM `jos_vm_orders` WHERE `order_id`=".$norder;
            }
            if($dfrom=='' && $dto=='' && $norder==''){
            $sql = "SELECT `order_id`,`user_id` FROM `jos_vm_orders` WHERE `cdate`>IFNULL( (SELECT `last_date_timest` FROM `tbl_last_loading`), 0 )  order by `ddate`";
            }
            
            $result_ol = mysql_query($sql, $link);
            
            while ($rowDB = mysql_fetch_assoc($result_ol)) {
                $user_id_old=$rowDB['user_id'];
                $order_id_old=$rowDB['order_id'];
                $title=$row['title'];
                $sql = "SELECT `id`, `name`, `username`, `email`, `email2`, `password`, `usertype`, `block`, `sendEmail`, `gid`, `registerDate`, `lastvisitDate`, `activation`, `params` FROM `jos_users` WHERE `id`=".$user_id_old;
                
                 $user_res = mysql_query($sql, $link);
                 ...
                 if(count($result_u)>0){
                   $user_id_new=$result_u[0]['id'];
                     ...
                     $q.="'".$user['name']."','".$user['username']."','".$user['email']."',NULL,'".$user['password']."','".$user['usertype']."',".$user['block'].",".$user['sendEmail'].",".$user['gid'].",'".$user['registerDate']."','".$user['lastvisitDate']."','".$user['activation']."','".$user['params']."')";
                     $database->setQuery($q);
                     $database->query(); 
                 ...
                 $result_ui = $database->loadAssocList();
                  if(!$result_ui){    
                     $sql="SELECT `user_id`, `address_type`, `address_type_name`, `company`, `title`, `last_name`, `first_name`,
                         `middle_name`, `phone_1`, `phone_2`, `fax`, `address_1`, `address_2`, `city`, `state`, `country`, `zip`, `user_email`, `extra_field_1`, 
                         `extra_field_2`, `extra_field_3`, `extra_field_4`, `extra_field_5`, `cdate`, `mdate`, `perms`, `bank_account_nr`, 
                         `bank_name`, `bank_sort_code`, `bank_iban`, `bank_account_holder`, `bank_account_type`, `address_type2`
                         FROM `jos_vm_user_info` where `address_type`='BT' and `user_id`=".$user_id_old;
                        
                          $user_info_res = mysql_query($sql, $link);
                          $user_info= mysql_fetch_assoc($user_info_res);
                     ...
                     $q.="'".$user_info_id."','".$user_id_new."','"
                            ...
                            .$user_info['address_type_name']."','"
                             ...
                             .$user_info['cdate']."','"
                             .$user_info['mdate']."','','"
                              .$user_info['bank_account_nr']."','"
                              .$user_info['bank_name']."','"
                              .$user_info['bank_sort_code']."','"
                              .$user_info['bank_iban']."','"
                              .$user_info['bank_account_holder']."','"
                              .$user_info['bank_account_type']."','"
                              .$user_info['address_type2']."',NULL,'','"
                             ."')";
                    
                     $database->setQuery($q);
                     $database->query(); 
                  }
                 $sql="select O.`order_id`, 	O.`ddate`, 	O.`customer_occasion`, 	O.`coupon_code`, O.`cdate`,O.`customer_note`,  O.`customer_comments`,	O.`customer_signature`, OUI.`last_name`, OUI.`middle_name`,OUI.`first_name` , 
			OUI.`phone_1`, OUI.`address_1`, OUI.`address_2`, OUI.`city`, OUI.`state`, OUI.`zip`, OUI.`country`, OUI.`fax`, OUI.`user_email`
			from `jos_vm_orders` AS O, `jos_vm_order_user_info` AS OUI
			where O.`order_id`=OUI.`order_id`
			and O.`user_id`=OUI.`user_id`
			and OUI.`address_type`='ST' and O.`order_id`='".$order_id_old."'";
               
                 
                 $order_det = mysql_query($sql, $link);
                 $order= mysql_fetch_assoc($order_det);
                 $aOrderItem 							= array();
                 $aOrderItem['Mode']						= 1;
		 $aOrderItem['PartnerEmail']				= $user['email'];
                 	 $aOrderItem['PartnerName']				=$title;
		 ...
		 $sql="SELECT `order_item_sku`, `product_quantity` FROM `jos_vm_order_item` WHERE `order_id`=".$order_id_old;
                  $order_items = mysql_query($sql, $link);
                  $aOrderItem['ProductSKU']		= "";
                  $aProductItem					= array();
                  $bCart							= false;
                  $sProductQ="";
                   while ($order_item = mysql_fetch_assoc($order_items)) {
                      $aProductItem[]				 = trim($order_item['order_item_sku'] );
			$sProductItem			 	.= trim($order_item['order_item_sku']) . ",";
                        $sProductQ                              .=$order_item['order_item_sku'].",".$order_item['product_quantity'].";";
                       $aOrderItem['ProductSKU']	.= trim($order_item['order_item_sku'] ) . '[--1--]' . trim($order_item['product_quantity']) . '[--2--]';
                       $bCart	= true;
                   }
                   $sError		= "";
		   $bCheck		= true;
			...
			$bCheck	= false;
			$sError	.= 'Order ID must be require';
		   }else {
                        $query = "delete from `tbl_load_history` where `new_number` not in (select order_id from jos_vm_orders)";
                        $database->setQuery($query);
                        $database->query(); 
                        $query = "select * from `tbl_load_history` where `new_number`<>0 and `old_number`=".$order_id_old." and `from`='".$title."'";
                        $database->setQuery($query);
                        $database->query(); 
                        $result_hist = $database->loadAssocList();				
			if( count($result_hist)>0 ){
				$bCheck	= false;
				$sError	.= 'Order ID ('.$order_id_old.') must be unique.';
			}
		  }
                  if( !$bCart ){
			$bCheck	= false;
			$sError	.= 'No orders to process.';
		 }
						
		if( count($aProductItem) ) {
			foreach ($aProductItem as $product) {
                        $query = "SELECT product_id FROM jos_vm_product WHERE product_sku = '$product' ";
                        $database->setQuery($query);
                        $database->query(); 
                        $bProduct = $database->loadAssocList();						
			if( !$bProduct ){
				$bCheck	= false;
				$sError	.= 'Product sku '.$product.' could not be found ';
			}
			}	
		}   
            ...
            $sql = "insert into `tbl_last_loading` (`last_date_timest`) VALUES(".$last_date.")";
            $result = mysql_query($sql, $link);
            }
             $last_date=0;     
        }
         if(($wf!='')||($dfrom=='' && $dto=='' && $norder=='')){
         $q="update tbl_last_load AS a set block=0, last_time=UNIX_TIMESTAMP(NOW()) ,  `ident_uniq`='' where block=1 ";
         }
         else{
             $q="update tbl_last_load AS a set block=0 ,  `ident_uniq`='' where block=1 ";
         }
         $database->setQuery($q);
         $database->query();
         $rr=$database->getAffectedRows();
	...
	if( $bTestMode ) { //check test mode 
		...
		foreach ($rows as $row) { 
			...
			$order_items .= '<tr>
								...
								<td>'.LangNumberFormat::number_format( $row['product_price'], 2, ".", " " ).'</td>
								<td>'.LangNumberFormat::number_format( $nTaxTemp, 2, ".", " " ).'</td>
								<td>'.LangNumberFormat::number_format( (($row['product_price'] * $nTaxTemp ) + $row['product_price']), 2, ".", " " ).'</td>
								<td>'.intval($aQuantity[$row['product_sku']]).'</td>
								<td>'.LangNumberFormat::number_format( (($row['product_price'] * $nTaxTemp) + $row['product_price'])*intval($aQuantity[$row['product_sku']]), 2, ".", " " ).'</td>
							</tr>';
		...
		$shopper_footer_html 	= "<br /><br />Thank you for your patronage.<br />"
            ...
            ."<br/> <b>Please Note: Orders placed for deliveries outside of Sydney, Melbourne, Brisbane and Perth may be delayed. We will contact you via email if there is an issue.</b>";
