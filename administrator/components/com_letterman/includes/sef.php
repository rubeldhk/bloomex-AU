...
function sefRelToAbs( $string ) {
	...
	if ($mosConfig_sef && !preg_match("/^(([^:\/?#]+):)/i",$string) && !strcasecmp(substr($string,0,9),"index.php")) {
		...
		*/
		...
		$sefstring = "";
		if ( (preg_match("/option=com_content/i",$string) || preg_match("/option=content/i",$string) ) && !preg_match("/task=new/i",$string) && !preg_match("/task=edit/i",$string) ) {
			/*
			Content
			...
			*/
			...
			if (preg_match("/&lang=/i",$string)) {
				$temp =explode("&lang=", $string);
				$temp =explode("&", $temp[1]);

				$sefstring .= "lang,".$temp[0]."/";
			}
			$string = $sefstring;
		} else if (preg_match("/option=com_/i",$string) && !preg_match("/task=new/i",$string) && !preg_match("/task=edit/i",$string)) {
			/*
			Components
