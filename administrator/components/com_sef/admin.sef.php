 ...
 * @package     sh404SEF
 * @version     $Id: admin.sef.php 365 2008-04-09 20:27:19Z silianacom-svn $
...
function saveSEF( $option ) {
    ...
    if($_POST['can_not_be_deleted'])
        $_POST['can_not_be_deleted']=1;
    if (!$row->bind( $_POST )) {
    ...
    if ($check === true) { 
    	...
    	$query = "SELECT newurl, rank, id,can_not_be_deleted FROM #__redirection WHERE oldurl = '".$row->oldurl."' ORDER BY rank ASC";
		$database->setQuery($query);
		...
		if (count($dbUrlList) > 0) {   // there are URL with same SEF URL
			if (!$sefConfig->shRecordDuplicates) {  // we don't allow duplicates : reject this URL
				...
				foreach ($dbUrlList as $urlInDB) {  // same SEF, but is the non-sef in this list of URl with same SEF ?
					if ($urlInDB->newurl == $row->newurl && $urlInDB->can_not_be_deleted == $row->can_not_be_deleted)
				...
				if ($mosMosg != _COM_SEF_URLEXIST) {  // we have not the same non-sef in the db, so this is an existing URL
					...
					$query = "UPDATE #__redirection SET oldurl='".$row->oldurl."', newurl='"
						.$row->newurl."', rank='".$shNewMaxRank."', dateadd='".$row->dateadd."',can_not_be_deleted='".$row->can_not_be_deleted."' WHERE id = '".$row->id."'";  // update DB
						$database->setQuery($query);
						$database->query(); 
