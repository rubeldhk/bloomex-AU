...
class EditGiftCategory {
    ...
    function show() {
        ...
        $l = $langs[0];
        $query = "SELECT * FROM jos_vm_landing_page_gift ";
        $database->setQuery($query);
        $result = $database->loadObjectList();

        $res = EditGiftCategory::result();
        ...
        $html = '<!--ERROR FORM--> </form>' . $res . '
             ...
             <div style="display:none">
              <input type="checkbox" name="categ_land_en" value="categ_land" '.EditGiftCategory::selectContentBodyCateg($langs[0]).'>Do not use caterory from landing manager<br></div>
              <input type="radio" name="categ_product_en" value="category" '.EditGiftCategory::selectContentBody($langs[0],'categ').'>Category<br>
        ...
        $html .= EditGiftCategory::center_content_categ('English Category', $langs[0]);
        $html .= '
            </table>

 <input type="radio" name="categ_product_en" value="product" '.EditGiftCategory::selectContentBody($langs[0],'product').'>Products<br>';
       $html .= ' <table class="adminlist">
         ...
         $html .= EditGiftCategory::center_content_product('English Products', $langs[0]);
  $html .= '
            </table>
 ...
 function center_content_product($name, $l){
     ...
     $html .= '
      ...
      $q = "SELECT sku,num FROM jos_vm_landing_products_gift where lang='" . $l . "' ORDER BY num ASC";
        $database->setQuery($q);
        $result = $database->loadObjectList();
    ...
    function selectContentBody($l,$type){
      global $database;
      $ret='';
   $query = "SELECT * FROM jos_vm_landing_page_gift WHERE lang='" . $l . "' AND position='categ_product'";
        $database->setQuery($query);
        $result = $database->loadObjectList();
   ...
   function selectContentUpper($l){
      global $database;
      $ret='';
   $query = "SELECT * FROM jos_vm_landing_page_gift WHERE lang='" . $l . "' AND position='check_upper_banner'";
        $database->setQuery($query);
        $result = $database->loadObjectList();
        if($result[0]->href=='1'){
            $ret='checked';
  ...
  function selectContentBodyCateg($l){
      global $database;
      $ret='';
   $query = "SELECT * FROM jos_vm_landing_page_gift WHERE lang='" . $l . "' AND position='categ_land'";
        $database->setQuery($query);
        $result = $database->loadObjectList();
        if($result[0]->href=='0'){
            $ret='checked';
      ...
      function top_content($name, $l) {
        global $database;
        $html .= '';
        $query = "SELECT * FROM jos_vm_landing_page_gift WHERE lang='" . $l . "' AND position='top' ORDER BY num ASC";
        $database->setQuery($query);
        $result = $database->loadObjectList();
        $k = 0;
        for ($i = 0; $i < 1; $i++) {
    ...
    function upper_content($name, $l) {
        ...
        $html .= '';
        $query = "SELECT * FROM jos_vm_landing_page_gift WHERE lang='" . $l . "' AND position='upper' ORDER BY num ASC";
        $database->setQuery($query);
        $result = $database->loadObjectList();
        $k = 0;
        for ($i = 0; $i < 2; $i++) {
    ...
    function center_content_categ($name, $l) {
        global $database;
        $html .= '';
        $query = "SELECT * FROM jos_vm_landing_page_gift WHERE lang='" . $l . "' AND position='body' ORDER BY num ASC";
        $database->setQuery($query);
        $result = $database->loadObjectList();
        $k = 0;
        for ($i = 0; $i < 9; $i++) {
            $k = ( $i == ((int) ($i / 2) * 2) ) ? 1 : 0;
             $restitlehref=explode('[--1--]',$result[$i]->href);
            $html .= '
                <tr class="row' . $k . '">
                    <td>
                      ' . $name . '
                    ...
                    <td>
                        <input type="text" name="href_' . $l . $i . '" value="' . $restitlehref[0] . '">
                            <input type="text" name="ttitlehref_' . $l . $i . '" value="' . $restitlehref[1] . '">
                    </td>
                </tr>';
        }
        return $html;
            ...
            switch ($_POST['type']) {
                case "center":
                    ...
                    $checked_fr=($categ_product_fr=='category')?'0':'1';
                     $query = "UPDATE `jos_vm_landing_page_gift` SET href='" .$checked_fr . "' WHERE num='0' AND lang='fr' AND position='categ_product'";
                            $database->setQuery($query);
                            $database->query();

                     $query = "UPDATE `jos_vm_landing_page_gift` SET href='" .$checked_en . "' WHERE num='0' AND lang='en' AND position='categ_product'";
                            $database->setQuery($query);
                            $database->query();
                    ...
                    $checked_fr=($categ_land_fr=='categ_land')?'0':'1';
                     $query = "UPDATE `jos_vm_landing_page_gift` SET href='" .$checked_fr . "' WHERE num='0' AND lang='fr' AND position='categ_land'";
                            $database->setQuery($query);
                            $database->query();


                     $query = "UPDATE `jos_vm_landing_page_gift` SET href='" .$checked_en . "' WHERE num='0' AND lang='en' AND position='categ_land'";
                            $database->setQuery($query);
                            $database->query();
                    ...
                    for ($i = 0; $i < 9; $i++) {
                        for ($i2 = 0; $i2 < 2; $i2++) {
                            ...
                            $image_name = 'landing_mid_gift_' . $l . $i . '.png';
                            ...
                            $query = "UPDATE `jos_vm_landing_page_gift` SET href='" . $_POST['href' . $second_name].'[--1--]'. $_POST['ttitlehref' . $second_name] . "' WHERE num='" . ($i + 1) . "' AND lang='" . $l . "' AND position='body'";
                            $database->setQuery($query);
                            if ($database->query())
                                $html .= "Updated. categ";
                            $q = "UPDATE jos_vm_landing_products_gift SET sku='{$_POST['sku'. $second_name_p]}' WHERE num='".( $i + 1 )."' AND lang='" . $l . "'";
                            $database->setQuery($q);
                             if ($database->query())
                ...
                case "upper":
                    ...
                    $checked_fr=($upper_banner_fr=='upper')?'1':'0';
                     $query = "UPDATE `jos_vm_landing_page_gift` SET href='" .$checked_fr . "' WHERE num='0' AND lang='fr' AND position='check_upper_banner'";
                            $database->setQuery($query);
                            if ($database->query())
                                    $html = "Updated. type fr ";
                     $query = "UPDATE `jos_vm_landing_page_gift` SET href='" .$checked_en . "' WHERE num='0' AND lang='en' AND position='check_upper_banner'";
                            $database->setQuery($query);
                            if ($database->query())
                    ...
                    for ($i = 0; $i < 2; $i++) {
                        for ($i2 = 0; $i2 < 2; $i2++) {
                            $l = ( $l == $langs[0] ) ? $langs[1] : $langs[0];
                            $second_name = '_' . $l . $i;
                            $image_name = 'landing_top_gift_' . $l . $i . '.png';
                            ...
                            $query = "UPDATE `jos_vm_landing_page_gift` SET href='" . $_POST['uhref' . $second_name] . "' WHERE num='" . ($i + 1) . "' AND lang='" . $l . "' AND position='upper'";
                            $database->setQuery($query);
                            if ($database->query())
                                $html = "Updated.";
                        }
                    ...
                    for ($i = 0; $i < 1; $i++) {
                        for ($i2 = 0; $i2 < 2; $i2++) {
                            $l = ( $l == $langs[0] ) ? $langs[1] : $langs[0];
                            $second_name = '_' . $l . $i;
                            $image_name = 'landing_main_gift_' . $l . $i . '.png';
                            ...
                            $query = "UPDATE `jos_vm_landing_page_gift` SET href='" . $_POST['thref' . $second_name] . "' WHERE num='" . ($i + 1) . "' AND lang='" . $l . "' AND position='top'";
                            $database->setQuery($query);
                            if ($database->query())
                                $html = "Updated.";
                        }
    ...
    function create($message = '') {
        $tmpl = & EditGiftCategory::createTemplate();
        $tmpl->setAttribute('body', 'src', 'gift_landding_users_manager.html');
        $tmpl->addVar('body', 'domainlist', EditGiftCategory::show());
