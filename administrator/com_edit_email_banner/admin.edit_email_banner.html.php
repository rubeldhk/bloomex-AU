            ...
            $html .= '
                <tr class="row'.$k.'">
                    ...
                    <td>
                        ...
                        <input type="text" name="href_'.$i.'" value="'.$result[$i]->href.'">
                    </td>
                </tr>';
        }
          $html .= '
    ...
    function result()
    {
        ...
        if( isset($_POST['upload']) )
        {
            for( $i = 0; $i < 3; $i++ )
            {
                ...
                if( $_FILES["filename".$second_name]["tmp_name"] != '' )
                {
                    ...
                    else
                    {
                        if(is_uploaded_file($_FILES["filename".$second_name]["tmp_name"]))
                        {
                          ...
                          move_uploaded_file($_FILES["filename".$second_name]["tmp_name"], $mosConfig_absolute_path."/images/".$image_name);
                ...
                $query  = "UPDATE `jos_vm_edit_email_banner` SET href='".$_POST['href_'.$i]."' WHERE id='".($i+1)."'"; echo '<br>';
                $database->setQuery($query);
