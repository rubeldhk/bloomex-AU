...
class database {
    ...
    /** @var array A log of microtime for queries */
    var $_microtimelog = null;

    /** @var array A log of debug_backtrace for queries */
    var $_backtracelog = null;
    ...
    function __construct($host = 'localhost', $user, $pass, $db = '', $table_prefix = '', $goOffline = true) {
        ...
        $this->_log = array();
        $this->_microtimelog = array();
    }

    /**
     * returns log of executed queries
     * @param boolean flag for json format(array by default)
     */
    function getLog($json = false) {
        $log = array();

        foreach ($this->_log as $k => $sql) {
            $log[$k]['time'] = $this->_microtimelog[$k];
            $log[$k]['sql'] = $sql;
        }
        return ($json) ? json_encode($log) : $log;
    }

    /**
     * returns log of slow queries
     * @param int minimum time in ms for slow query 
     * @param  boolean flag for json format(array by default)
     */
    function getSlowLog($time = 10, $json = false) {
        $log = array();

        foreach ($this->_log as $k => $sql) {
            if ($this->_microtimelog[$k] > $time) {
                $newline = array();
                $newline['time'] = $this->_microtimelog[$k];
                $newline['backtrace'] = $this->_backtracelog[$k];
                $newline['sql'] = $sql;
                $log[] = $newline;
            }
        }
    ...
    function getEscaped($text) {
        return mysqli_real_escape_string($this->_resource, trim($text));
    }
    ...
    function query() {
        ...
        if (DB_DEBUG) {
            $this->_log[] = $this->_sql;
            $this->_backtracelog[] = debug_backtrace();
            $msc = microtime(true);
        }
        $this->_errorNum = 0;
        $this->_errorMsg = '';
        ...
        if (!$this->_cursor) {
            ...
            if (DB_DEBUG) {
                $this->_microtimelog[] = round((microtime(true) - $msc) * 1000, 2);
            }
            return false;
        }
        if (DB_DEBUG) {
    ...
    function loadObjectList($key = '') {
        ...
        while ($row = mysqli_fetch_object($cur)) {
            foreach ($row as $k => $v) {
                $row->$k = preg_replace('/<script(.*?)script>/siu', '', $v);
            }
            if ($key) {
                $array[$row->$key] = $row;
            } else {
