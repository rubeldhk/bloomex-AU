...
* @return object
*/
	static function parse( $txt, $process_sections = false, $asArray = false ) {
		if (is_string( $txt )) {
			$lines = explode( "\n", $txt );
			...
			if (!is_object( $this->_xmlElem )) {
				...
				if ($xmlDoc->loadXML( $this->_path, false, true )) {
					...
					$isParamsFile = ($tagName == 'mosinstall' || $tagName == 'mosparams');
                                        ...
                                        if (method_exists($root, 'getAttribute') == true) {
                                            $root_type = $root->getAttribute( 'type' );
                                        }
                                        
					if ($isParamsFile) {
                                            if (isset($root_type) AND $root->getAttribute( 'type' ) != $this->_type) {
...
*/
	function renderParam( &$param, $control_name='params' ) {
		$result = array();
        $checkMethodExist = method_exists($param,'getAttribute');
       if(!$checkMethodExist){
           return false;
       }
	...
	*/
	function _form_list( $name, $value, &$node, $control_name ) {
		...
		foreach ($node->childNodes as $option) {
            $checkMethodExist = method_exists($option,'getAttribute');
            if(!$checkMethodExist){
                continue;
            }
