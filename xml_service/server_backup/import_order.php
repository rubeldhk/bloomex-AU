...
while ( $row = $db->get_row($users, 'MYSQL_ASSOC') ) {	
	...
	if(  $ftp->file_size( "/httpdocs/orders.xml" ) >= 0 && $ftp->get( $rootPath . "orders.xml", "/httpdocs/orders.xml" ) ) {	
		if( is_file($rootPath . "orders.xml") ) {		
			...
			if( $ftp->renamefile( "/httpdocs/orders.xml", "/httpdocs/order_processed/order_".date("m_d_y_h_i").".xml" ) ) {
				
				if( $ftp->put_string( "/httpdocs/order_result_" . date("m_d_y_h_i") . ".xml",  $sOrderResult ) ) {	
...
//		die($result);
	...
	$payment_number = ereg_replace(" |-", "", $credit_card_number);	
