...
while ( $row = $db->get_row($users, 'MYSQL_ASSOC') ) {
	...
	//echo $row['ftp_username']."==========".$row['ftp_password']."==========".$row['domain_name'] . "<br/><br/>";
	
	$sProductData	= getProductFiler($row['product_filter_id'], $row['product_updated_time']);
	
	
	$tempFile = getcwd() . "/tmp/products_temp.xml";
	
	$fh = fopen($tempFile, 'w') or die("can't open file");
	fwrite($fh, $sProductData);
	fclose($fh);
	
	if( $ftp->put( date( "Ymd" , time())."_products.xml",  $tempFile ) ) {	
	...
	$query 		= "SELECT * FROM tbl_partners WHERE partner_name = '" . $row['partner_name'] . "'";
	$row		= $db->select($query);
	$oUpdate	= $db->get_row( $row, 'MYSQL_ASSOC');
	
	if( $oUpdate['id'] ) {
		$sDateTime	= '<html>
							<body bgcolor="#FFFFFF">';
		
		if( strtotime( $oUpdate['product_updated_time'] ) ) {			
			$sDateTime .= 'Product Last Updated: '. date('Y-m-d H:s', strtotime($oUpdate['product_updated_time']));
		}
		
		if( strtotime( $oUpdate['status_updated_time'] ) ) {			
			$sDateTime .= '<br/><br/>Order Status Last Updated: '. date('Y-m-d H:s', strtotime($oUpdate['status_updated_time']));
		}
		
		if( strtotime( $oUpdate['import_updated_time'] ) ) {			
			$sDateTime .= '<br/><br/>Import Order Last Updated: '. date('Y-m-d H:s', strtotime($oUpdate['import_updated_time']));
		}
		
		if( strtotime( $oUpdate['delivery_updated_time'] ) ) {			
			$sDateTime .= '<br/><br/>Delivery Options Last Updated: '. date('Y-m-d H:s', strtotime($oUpdate['delivery_updated_time']));
		}
		...
		$fh = fopen("/tmp/index.txt", 'w') or die("can't open file");
		@fwrite($fh, $sDateTime);
		@fclose($fh);
		$ftp->put( "index.html", "/tmp/index.txt" );		
	}
