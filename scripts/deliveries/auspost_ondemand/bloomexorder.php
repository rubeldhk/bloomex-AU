        ...
        switch ($this->_WH) {
            case "WH12":
                if (substr($_SERVER['HTTP_HOST'], 0, 4) == 'dev.' || substr($_SERVER['HTTP_HOST'], 0, 6) == 'stage.') {
                    $Warehouse['username'] = '507bb323-a42a-41bf-ad57-5678dea2f8a5';
                    $Warehouse['Password'] = 'x0b0885a26392b9bdc60';
                    $Warehouse['customerNumber'] = '3010387570';
                } else {
                    $Warehouse['username'] = 'bb174c1e-da3f-4bac-909e-fce17cd1620c';
                    $Warehouse['Password'] = 'x837e796a7973b080e0c';
                    $Warehouse['customerNumber'] = '0005879470';
                }
                break;
            case "WH16":
                if (substr($_SERVER['HTTP_HOST'], 0, 4) == 'dev.' || substr($_SERVER['HTTP_HOST'], 0, 6) == 'stage.') {
...
$Warehouse['customerNumber'] = '0004720960';
    ...
    function get_shipments($orders) {
        $in_orders = rtrim($orders, ',');
        $q = "SELECT shipment_id,order_id FROM jos_order_auspost_ondemand_json WHERE order_id in ($in_orders)";
        $result = mysqli_query($this->link, $q);
        if (!$result) {
            echo $q . "<br/>";
            die('Invalid query: ' . mysqli_error($this->link));
    ...
    function get_shipment_id($order_id) {
        $q = "SELECT shipment_id,json_label,json_manifest,manifest_id,warehouse FROM jos_order_auspost_ondemand_json WHERE order_id='$order_id'";
        $result = mysqli_query($this->link, $q);
        if (!$result) {
            echo $q . "<br/>";
            die('Invalid query: ' . mysqli_error($this->link));
    ...
    function attachlabel($order_id, $label_url, $sender) {
        ...
        $query = "SELECT id FROM `jos_order_auspost_ondemand_json` WHERE label_created='1' and `order_id`=" . $order_id;
        ...
        $query = "UPDATE `jos_order_auspost_ondemand_json` SET label_created='1'  WHERE `order_id`=" . $order_id;
        $result = mysqli_query($this->link, $query);
        if (!$result) {
            echo $query . "<br/>";
            die('Invalid query: ' . mysqli_error($this->link));
    ...
    function addlabeljson($order_id, $curl_response_label) {
        $query = "UPDATE `jos_order_auspost_ondemand_json` SET json_label='" . $curl_response_label . "'  WHERE `order_id`=" . $order_id;
        $result = mysqli_query($this->link, $query);
        if (!$result) {
            echo $query . "<br/>";
            die('Invalid query: ' . mysqli_error($this->link));
    ...
    function addshipment($order_id, $shipment_id, $tracking_pin, $sender, $response_json) {
        ...
        $query = "INSERT INTO `jos_order_auspost_ondemand_json`
                (
                    `order_id`, 
                    `json_shipment`,
                    `shipment_id`,
    ...
    function deleteshipment($order_id, $sender, $shipment_id) {
        ...
        $query = "DELETE  FROM `jos_order_auspost_ondemand_json`
        WHERE order_id='$order_id'";
    ...
    function addmanifest($order_id, $sender, $manifest, $warehouse, $curl_response) {
        ...
        $query = "UPDATE `jos_order_auspost_ondemand_json` SET json_manifest='" . $curl_response . "',manifest_id='" . htmlspecialchars($manifest) . "',warehouse='" . $warehouse . "'  WHERE `order_id`=" . $order_id;
        $result = mysqli_query($this->link, $query);
        if (!$result) {
            echo $query . "<br/>";
            die('Invalid query: ' . mysqli_error($this->link));
