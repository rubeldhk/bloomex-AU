...
class BloomexOrder
{
    ...
    function __construct()
    {
        ...
        $this->status_sent = $mosConfig_status_sent_auspost;
        $this->status_cancel = $mosConfig_status_cancel_auspost;
    }
    ...
    function getsender()
    {
        ...
        switch ($this->_WH) {
            case "WH12":
                ...
                break;
            case "WH14":
                $Warehouse['customerNumber'] = '0007860077';
                break;
            case "WH15":
                $Warehouse['customerNumber'] = '0008348363';
                break;
            case "WH16":
                $Warehouse['customerNumber'] = '0004720960';
                break;
            case "p01":
                $Warehouse['customerNumber'] = '0008348371';
                break;
            default :
...
//        }
    ...
    {
        $in_orders = rtrim($orders, ',');
        $q = "SELECT shipment_id,order_id FROM jos_order_auspost_json WHERE order_id in ($in_orders)";
        $result = mysqli_query($this->link, $q);
        if (!$result) {
            echo $q . "<br/>";
            die('Invalid query: ' . mysqli_error($this->link));
    ...
    {
        $q = "SELECT shipment_id,json_label,json_manifest,manifest_id,warehouse FROM jos_order_auspost_json WHERE order_id='$order_id'";
        $result = mysqli_query($this->link, $q);
        if (!$result) {
            echo $q . "<br/>";
            die('Invalid query: ' . mysqli_error($this->link));
    ...
    function attachlabel($order_id, $label_url, $sender)
    {
        ...
        $query = "SELECT id FROM `jos_order_auspost_json` WHERE label_created='1' and `order_id`=" . $order_id;
        ...
        $query = "UPDATE `jos_order_auspost_json` SET label_created='1'  WHERE `order_id`=" . $order_id;
        $result = mysqli_query($this->link, $query);
        if (!$result) {
            echo $query . "<br/>";
            die('Invalid query: ' . mysqli_error($this->link));
    ...
    {
        $query = "UPDATE `jos_order_auspost_json` SET json_label='" . $curl_response_label . "'  WHERE `order_id`=" . $order_id;
        $result = mysqli_query($this->link, $query);
        if (!$result) {
            echo $query . "<br/>";
            die('Invalid query: ' . mysqli_error($this->link));
    ...
    function addshipment($order_id, $shipment_id, $tracking_pin, $sender, $response_json)
    {
        ...
        $query = "INSERT INTO `jos_order_auspost_json`
                (
                    `order_id`, 
                    `json_shipment`,
                    `shipment_id`,
    ...
    function deleteshipment($order_id, $sender, $shipment_id)
    {
        ...
        $query = "DELETE  FROM `jos_order_auspost_json`
        WHERE order_id='$order_id'";
    ...
    function addmanifest($order_id, $sender, $manifest, $warehouse, $curl_response)
    {
        ...
        $query = "UPDATE `jos_order_auspost_json` SET json_manifest='" . $curl_response . "',manifest_id='" . htmlspecialchars($manifest) . "',warehouse='" . $warehouse . "'  WHERE `order_id`=" . $order_id;
        $result = mysqli_query($this->link, $query);
        if (!$result) {
            echo $query . "<br/>";
            die('Invalid query: ' . mysqli_error($this->link));
