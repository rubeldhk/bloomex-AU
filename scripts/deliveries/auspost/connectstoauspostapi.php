...
class AusPost {
    ...
    function __construct($order_id = '', $warehouse = '') {
        ...
        if (!is_dir($_SERVER["DOCUMENT_ROOT"] . "/auspost_request_response/" . $this->order_id)) {
            mkdir($_SERVER["DOCUMENT_ROOT"] . "/auspost_request_response/" . $this->order_id, 0777);
        }

//        if (substr($_SERVER['HTTP_HOST'], 0, 4) == 'dev3' || substr($_SERVER['HTTP_HOST'], 0, 6) == 'stage1') {
...
//        }
        ...
        if ($labels->labels) {
            ...
            foreach ($labels->labels as $label) {
                ...
                if (!$curl_response_print_label) {
                    ...
                    //print label
                    $shipment_label_print_response = fopen($_SERVER["DOCUMENT_ROOT"] . "/auspost_request_response/" . $this->order_id . "/shipment_label_print_response.json", "w") or die("Unable to open file!");
    ...
    function checkSuburbStateZipValid($state, $subarb, $zip) {
        ...
        $request_file = fopen($_SERVER["DOCUMENT_ROOT"] . "/auspost_request_response/" . $this->order_id . "/validate_suburb_request.json", "w") or die("Unable to open file!");
        fwrite($request_file, $service_url);
        $response_file = fopen($_SERVER["DOCUMENT_ROOT"] . "/auspost_request_response/" . $this->order_id . "/validate_suburb_response.json", "w") or die("Unable to open file!");
...
//validate shipment
        $request_file = fopen($_SERVER["DOCUMENT_ROOT"] . "/auspost_request_response/" . $this->order_id . "/validate_shipment_request.json", "w") or die("Unable to open file!");
        fwrite($request_file, $data_string);
        $response_file = fopen($_SERVER["DOCUMENT_ROOT"] . "/auspost_request_response/" . $this->order_id . "/validate_shipment_response.json", "w") or die("Unable to open file!");
        fwrite($response_file, $curl_response);
        fclose($response_file);
        fclose($request_file);
...
//create shipment
        $request_file = fopen($_SERVER["DOCUMENT_ROOT"] . "/auspost_request_response/" . $this->order_id . "/create_shipment_request.json", "w") or die("Unable to open file!");
        fwrite($request_file, $data_string);
        $response_file = fopen($_SERVER["DOCUMENT_ROOT"] . "/auspost_request_response/" . $this->order_id . "/create_shipment_response.json", "w") or die("Unable to open file!");
        fwrite($response_file, $curl_response);
        fclose($response_file);
        fclose($request_file);
//end create shipment
        ...
        if ($response->shipments) {
            ...
            foreach ($response->shipments as $shipment) {
                ...
                if (!$curl_response_label) {
                    ...
                    if ($response_label && isset($response_label->errors) && is_array($response_label->errors)) {
...
//create shipment label
                    $request_file = fopen($_SERVER["DOCUMENT_ROOT"] . "/auspost_request_response/" . $this->order_id . "/create_label_request.json", "w") or die("Unable to open file!");
                    fwrite($request_file, $data_string_label);
                    $response_file = fopen($_SERVER["DOCUMENT_ROOT"] . "/auspost_request_response/" . $this->order_id . "/create_label_response.json", "w") or die("Unable to open file!");
...
//end create shipment label
    ...
    function deleteshipment($request) {
        ...
        if ($checkshipment['shipment_id']) {
            ...
            //delete shipment
            $request_file = fopen($_SERVER["DOCUMENT_ROOT"] . "/auspost_request_response/" . $this->order_id . "/delete_shipment_request.json", "w") or die("Unable to open file!");
            fwrite($request_file, $service_url);
            $response_file = fopen($_SERVER["DOCUMENT_ROOT"] . "/auspost_request_response/" . $this->order_id . "/delete_shipment_response.json", "w") or die("Unable to open file!");
    ...
    function createmanifest($request) {
        $res = array();
        $order_reference = $request['warehouse'] . " " . date('d-m-Y');
...
//create manifest
                $request_file = fopen($_SERVER["DOCUMENT_ROOT"] . "/auspost_request_response/" . $this->order_id . "/create_order_from_shipment_request.json", "w") or die("Unable to open file!");
                fwrite($request_file, $data_order_string);
                $response_file = fopen($_SERVER["DOCUMENT_ROOT"] . "/auspost_request_response/" . $this->order_id . "/create_order_from_shipment_response.json", "w") or die("Unable to open file!");
...
//print manifest
        $request_file = fopen($_SERVER["DOCUMENT_ROOT"] . "/auspost_request_response/" . $this->order_id . "/get_order_summary_request.json", "w") or die("Unable to open file!");
        fwrite($request_file, $service_url_print_manifest);
        $response_file = fopen($_SERVER["DOCUMENT_ROOT"] . "/auspost_request_response/" . $this->order_id . "/get_order_summary_response.json", "w") or die("Unable to open file!");
        fwrite($response_file, $curl_response_print_manifest);
        fclose($response_file);
        fclose($request_file);
//end print manifest
